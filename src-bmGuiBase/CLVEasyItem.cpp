...
//******************************************************************************************************
CLVEasyItem::CLVEasyItem(uint32 level, bool superitem, bool expanded, ColumnListView* lv)
	: CLVListItem((lv && lv->fAvoidColPushing) ? 0 : level, superitem, expanded),
...
void
CLVEasyItem::PrepListsForSet(int column_index)
{
...
void
CLVEasyItem::SetColumnContent(int column_index, const char* text)
{
...
void
CLVEasyItem::SetColumnContent(
	int column_index, const BmBitmapHandle* bitmap, int8 horizontal_offset)
{
	PrepListsForSet(column_index);
...
const char*
CLVEasyItem::GetColumnContentText(int column_index)
{
	CLVColumn* column = (CLVColumn*)fOwner->ColumnAt(column_index);
	if (column) {
...
const BmBitmapHandle*
CLVEasyItem::GetColumnContentBitmap(int column_index)
{
	CLVColumn* column = (CLVColumn*)fOwner->ColumnAt(column_index);
	if (column) {
		int32 type = column->Type();
...
bool
CLVEasyItem::ColumnFitsText(int column_index, const char* text) const
{
	CLVColumn* column = (CLVColumn*)fOwner->ColumnAt(column_index);
	if (!column)
		return false;
...
CLVDrawingContext*
CLVEasyItem::CreateDrawingContext()
{
	return new CLVEasyItemDrawingContext;
}

void
CLVEasyItem::SetupDrawingContext(CLVDrawingContext* drawingContext)
{
	CLVEasyItemDrawingContext* ctx = dynamic_cast<CLVEasyItemDrawingContext*>(drawingContext);
	if (ctx) {
		...
		if (Highlight()) {
			...
			ctx->tintedBackColor = tint_color(ctx->tintedBackColor, highlight_tint);
		}
		if (Bold()) {
...
void
CLVEasyItem::DrawColumn(
	BRect item_column_rect, int32 column_index, CLVDrawingContext* drawingContext)
{
	CLVEasyItemDrawingContext* ctx = dynamic_cast<CLVEasyItemDrawingContext*>(drawingContext);
	if (!ctx)
		return;
	...
	if (HighlightTop()) {
		BRect highlightRect(item_column_rect.left, 0, item_column_rect.right, 1);
		fOwner->SetHighColor(ui_color(B_CONTROL_HIGHLIGHT_COLOR));
		fOwner->FillRect(highlightRect);
	}

	if (HighlightBottom()) {
		BRect highlightRect(item_column_rect.left, item_column_rect.bottom - 1,
			item_column_rect.right, item_column_rect.bottom);
		fOwner->SetHighColor(ui_color(B_CONTROL_HIGHLIGHT_COLOR));
		fOwner->FillRect(highlightRect);
	}
	...
	if (type == CLV_COLTYPE_STATICTEXT || type == CLV_COLTYPE_USERTEXT) {
		...
		if (text != NULL) {
			...
			if (!right_justify)
				draw_point.Set(
					item_column_rect.left + ctx->hOffs, item_column_rect.top + ctx->vOffs);
			else {
		...
		if (bitmap != NULL) {
			...
			if (!right_justify) {
				...
				item_column_rect.left
					= item_column_rect.right - horizontal_offset - (bounds.right - bounds.left);
				item_column_rect.right -= horizontal_offset;
			}
			item_column_rect.top += ceilf(
...
void
CLVEasyItem::Update(BView* fOwner, const BFont* font)
{
