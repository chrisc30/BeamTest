...
BetterScrollView::BetterScrollView(
	minimax minmax, BView* target, BmScrollViewFlags svFlags, const char* captionMaxText)
	: BScrollView("BetterScrollView", target, B_FOLLOW_NONE, B_FRAME_EVENTS | B_WILL_DRAW,
		  svFlags & BM_SV_H_SCROLLBAR, svFlags & BM_SV_V_SCROLLBAR, B_FANCY_BORDER),
	  mCaption(NULL),
	  mCaptionWidth(0),
	  mBusyView(NULL),
	  mTarget(target)
{
	...
	if (BeamOnDano) {
		...
		if (mVScroller) {
			float hOffs = Bounds().right - B_V_SCROLL_BAR_WIDTH - 1 - mVScroller->Frame().left;
	...
	if (svFlags & BM_SV_CORNER) {
		BRect bounds = Bounds();
		mScrollViewCorner = new ScrollViewCorner(
	...
	if (svFlags & BM_SV_CAPTION) {
		...
		mCaption
			= new BmCaption(BRect(LT.x, LT.y, LT.x + mCaptionWidth - 1, LT.y + frame.Height()), "");
		AddChild(mCaption);
	}
}
...
\*------------------------------------------------------------------------------*/
void
BetterScrollView::AddActionView(BView* actionView)
{
	if (!actionView)
		return;
	...
	if (mVScroller) {
		mVScroller->ResizeTo(B_V_SCROLL_BAR_WIDTH, mVScroller->Bounds().Height() - top);
...
\*------------------------------------------------------------------------------*/
void
BetterScrollView::SetDataRect(BRect data_rect, bool scrolling_allowed)
{
	mDataRect = data_rect;
...
\*------------------------------------------------------------------------------*/
void
BetterScrollView::FrameResized(float new_width, float new_height)
{
...
\*------------------------------------------------------------------------------*/
void
BetterScrollView::AttachedToWindow()
{
...
\*------------------------------------------------------------------------------*/
status_t
BetterScrollView::SetBorderHighlighted(bool highlighted)
{
	Draw(Bounds());
	return B_OK;
}
...
\*------------------------------------------------------------------------------*/
void
BetterScrollView::WindowActivated(bool active)
{
	if (mScrollViewCorner) {
		bool scEnabled = ((mHScroller && mHScroller->Proportion() < 1.0)
						  || (mVScroller && mVScroller->Proportion() < 1.0));
		mScrollViewCorner->SetEnabled(scEnabled && active);
		mScrollViewCorner->Draw(BRect(0.0, 0.0, B_V_SCROLL_BAR_WIDTH, B_H_SCROLL_BAR_HEIGHT));
	}
...
\*------------------------------------------------------------------------------*/
void
BetterScrollView::Draw(BRect rect)
{
	if (BeamOnDano)
	...
	if (mTarget) {
		BRect bounds = Bounds();
		rgb_color color = IsFocus() || mTarget->IsFocus() ? keyboard_navigation_color()
...
\*------------------------------------------------------------------------------*/
void
BetterScrollView::UpdateScrollBars(bool scrolling_allowed)
{
	...
	// Figure out the ratio of the bounds rectangle width or height to the page rectangle width or
	// height
...
\*------------------------------------------------------------------------------*/
void
BetterScrollView::SetCaptionText(const char* text)
{
	if (mCaption)
...
\*------------------------------------------------------------------------------*/
void
BetterScrollView::SetErrorText(const BmString& text)
{
	if (mBusyView)
...
\*------------------------------------------------------------------------------*/
void
BetterScrollView::PulseBusyView()
{
	if (mBusyView)
		mBusyView->Pulse();
}
...
\*------------------------------------------------------------------------------*/
BRect
BetterScrollView::layout(BRect r)
{
	MoveTo(r.LeftTop());
