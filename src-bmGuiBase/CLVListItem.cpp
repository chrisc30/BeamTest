...
CLVColumn*
CLVListItem::ColumnAt(int32 column_index)
{
	return fOwner->ColumnAt(column_index);
}

BRect
CLVListItem::ItemColumnFrame(int32 column_index)
{
...
void
CLVListItem::InvalidateColumn(int32 column_index)
{
	BRect updateRect;
	if (column_index < 0) {
...
bool
CLVListItem::ExpanderRectContains(BPoint where)
{
	bool contains = false;
	...
	for (int32 Counter = 0; Counter < NumberOfColumns; Counter++) {
		...
		if (ThisColumn->fFlags & CLV_EXPANDER) {
			...
			if (IsSuperItem()) {
				...
				float TopOffset
					= ceilf((ThisColumnRect.bottom - ThisColumnRect.top - 10.0f) / 2.0f);
				...
				if (LeftOffset <= ThisColumnRect.right) {
					BRect expanderButtonRect(LeftOffset, ThisColumnRect.top + TopOffset,
...
CLVDrawingContext*
CLVListItem::CreateDrawingContext()
{
	return NULL;
}

void
CLVListItem::SetupDrawingContext(CLVDrawingContext* drawingContext)
{
}

void
CLVListItem::DrawItem(BView*, BRect itemRect, bool complete)
{
	BList* DisplayList = &((ColumnListView*)fOwner)->fColumnDisplayList;
	int32 NumberOfColumns = DisplayList->CountItems();
	float PushMax = itemRect.right;
	...
	for (int32 Counter = 0; Counter < NumberOfColumns; Counter++) {
		...
		if (ThisColumn->fFlags & CLV_EXPANDER) {
			...
			if (ClippingRegion.Intersects(ThisColumnRect)) {
				// Give the programmer a chance to do his kind of highlighting if the item is
				// selected
				DrawColumn(ThisColumnRect, fOwner->fColumnList.IndexOf(ThisColumn), ctx);
				if (IsSuperItem()) {
					...
					if (Arrow) {
						...
						float LeftOffset = ThisColumn->fColumnEnd + Shift - r;
						float RightClip = LeftOffset + r - ThisColumnRect.right;
						if (RightClip < 0.0f)
							RightClip = 0.0f;
						if (LeftOffset <= ThisColumnRect.right) {
							BRect expanderButtonRect(LeftOffset, ThisColumnRect.top + TopOffset,
								LeftOffset + r - RightClip, ThisColumnRect.top + TopOffset + b);
							fOwner->SetDrawingMode(B_OP_OVER);
							fOwner->DrawBitmap(
								Arrow, BRect(0.0, 0.0, r - RightClip, b), expanderButtonRect);
			...
			if (Shift > 0.0 && ThisColumnRect.right > PushMax)
				ThisColumnRect.right = PushMax;
			if (ThisColumnRect.right >= ThisColumnRect.left
				&& ClippingRegion.Intersects(ThisColumnRect))
				DrawColumn(ThisColumnRect, fOwner->fColumnList.IndexOf(ThisColumn), ctx);
		}
	}
...
float
CLVListItem::ExpanderShift(int32 column_index)
{
...
void
CLVListItem::ColumnWidthChanged(int32 /*column_index*/, float /*column_width*/)
{
}


void
CLVListItem::FrameChanged(int32 /*column_index*/, BRect /*new_frame*/)
{
}

void
CLVListItem::SetStyleFlag(uint8 style, bool on)
{
	if (on)
		fItemFlags |= style;
	else
...
void
CLVListItem::SetDefaultExpanderBitmaps(BmBitmapHandle* expanded, BmBitmapHandle* unexpanded)
{
...
BmBitmapHandle*
CLVListItem::GetExpanderBitmap(bool expanded)
{
	if (expanded)
