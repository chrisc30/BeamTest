...
BmMenuControl::BmMenuControl(
	const char* label, BMenu* menu, float weight, float maxWidth, const char* fitText)
	  ...
	  mMenuBar(MenuBar())
{
	...
	if (fitText) {
		...
		ct_mpm = minimax(int(fixedWidth), int(minHeight), int(fixedWidth), int(minHeight));
	} else {
		ct_mpm = minimax(int(StringWidth("1234567890") + Divider() + 27), int(minHeight),
			int(maxWidth), int(minHeight), weight);
	}
...
\*------------------------------------------------------------------------------*/
void
BmMenuControl::MarkItem(const char* label)
{
	BmMenuControllerBase* menuContr = dynamic_cast<BmMenuControllerBase*>(Menu());
...
\*------------------------------------------------------------------------------*/
void
BmMenuControl::SetEnabled(bool enabled)
{
...
\*------------------------------------------------------------------------------*/
void
BmMenuControl::SetDivider(float divider)
{
	if (BeamOnDano)
...
\*------------------------------------------------------------------------------*/
BRect
BmMenuControl::layout(BRect inFrame)
{
	if (inFrame == Frame())
		return inFrame;
	BRect frame = inFrame;
	...
	float occupiedSpace = BeamOnDano ? Divider() + 1 + 13 : Divider() + 1;

	mMenuBar->MoveTo(occupiedSpace, BeamOnDano ? 5 : 3);
	mMenuBar->ResizeTo(frame.Width() - occupiedSpace - 2, mMenuBar->Frame().Height());
	if (BeamOnDano) {
