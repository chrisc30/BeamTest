...
UserResizeSplitView::UserResizeSplitView(MView* top_or_left, MView* right_or_bottom,
	...
	bool should_resize_left_or_top, bool should_resize_right_or_bottom, bool can_hide_left_or_top,
	bool can_hide_right_or_bottom, bool move_slider_on_frame_resize, uint32 resize_mask,
	uint32 flags)
...
float
UserResizeSplitView::LeftOrTopMinSize()
{
	return (m_posture == B_VERTICAL ? m_left_or_top->mpm.mini.x : m_left_or_top->mpm.mini.y);
}

float
UserResizeSplitView::LeftOrTopMaxSize()
{
	return (m_posture == B_VERTICAL ? m_left_or_top->ct_mpm.maxi.x : m_left_or_top->ct_mpm.maxi.y);
}

float
UserResizeSplitView::RightOrBottomMinSize()
{
	return (
		m_posture == B_VERTICAL ? m_right_or_bottom->mpm.mini.x : m_right_or_bottom->mpm.mini.y);
}

float
UserResizeSplitView::RightOrBottomMaxSize()
{
	return (m_posture == B_VERTICAL ? m_right_or_bottom->ct_mpm.maxi.x
									: m_right_or_bottom->ct_mpm.maxi.y);
}

void
UserResizeSplitView::SetPreferredDividerLeftOrTop(float divider_left_or_top)
{
	m_divider_left_or_top = divider_left_or_top;
}

void
UserResizeSplitView::SetDividerLeftOrTop(float divider_left_or_top, bool force)
{
	...
	if (m_posture == B_HORIZONTAL) {
		...
		invalid.Set(bounds.left, MIN(m_divider_left_or_top - 1, divider_left_or_top - 1),
			bounds.right, MAX(m_divider_left_or_top, divider_left_or_top) + Thickness);
	} else {
		if (m_should_resize_left_or_top && m_left_or_top)
			ResizeLeftOrTopChildTo(divider_left_or_top);
		if (m_right_or_bottom) {
...
void
UserResizeSplitView::ResizeLeftOrTopChildTo(float width_or_height)
{
	BRect bounds = Bounds();
...
void
UserResizeSplitView::ResizeRightOrBottomChildTo(float width_or_height)
{
	BRect bounds = Bounds();
	...
	if (m_posture == B_HORIZONTAL)
		m_right_or_bottom->layout(
			BRect(0, bounds.bottom - width_or_height, bounds.right, bounds.bottom));
	else
		m_right_or_bottom->layout(
			BRect(bounds.right - width_or_height, 0, bounds.right, bounds.bottom));
}

void
UserResizeSplitView::FrameResized(float new_width, float new_height)
{
	...
	if (m_move_slider_on_frame_resize) {
		...
		if (m_posture == B_HORIZONTAL)
			invalid.Set(bounds.right - dw - drawMargin,
				MIN(m_divider_left_or_top, m_divider_left_or_top + dh), bounds.right,
				MAX(m_divider_left_or_top, m_divider_left_or_top + dh) + Thickness);
		else
			invalid.Set(MIN(m_divider_left_or_top, m_divider_left_or_top + dw),
				bounds.bottom - dh - drawMargin,
				MAX(m_divider_left_or_top, m_divider_left_or_top + dw) + Thickness, bounds.bottom);
		Invalidate(invalid);
		SetDividerLeftOrTop(m_divider_left_or_top + (m_posture == B_HORIZONTAL ? dh : dw));
		...
		if (m_posture == B_HORIZONTAL)
			...
			invalid.Set(MIN(m_divider_left_or_top, m_divider_left_or_top + dw),
				bounds.bottom - dh - drawMargin,
				MAX(m_divider_left_or_top, m_divider_left_or_top + dh) + Thickness, bounds.bottom);
		Invalidate(invalid);
...
void
UserResizeSplitView::Draw(BRect updateRect)
{
	BRect b = Bounds();
	const float dist = 3;
	const float count = floorf((m_posture == B_HORIZONTAL ? b.Width() : b.Height()) / dist);
...
void
UserResizeSplitView::MouseDown(BPoint where)
{
...
void
UserResizeSplitView::MouseUp(BPoint where)
{
...
void
UserResizeSplitView::MouseMoved(BPoint where, uint32 code, const BMessage* message)
{
...
minimax
UserResizeSplitView::layoutprefs()
{
	minimax mmLeft;
	minimax mmRight;
	if (m_left_or_top)
...
BRect
UserResizeSplitView::layout(BRect frame)
{
	if (frame == Frame())
		return frame;
	...
	if (m_posture == B_HORIZONTAL) {
		if (m_left_or_top)
			m_left_or_top->layout(
				BRect(rect.left, rect.top, rect.right, m_divider_left_or_top - 1));
		if (m_right_or_bottom)
			m_right_or_bottom->layout(
				BRect(rect.left, m_divider_left_or_top + Thickness, rect.right, rect.bottom));
	} else {
		if (m_left_or_top)
			m_left_or_top->layout(
				BRect(rect.left, rect.top, m_divider_left_or_top - 1, rect.bottom));
		if (m_right_or_bottom)
			m_right_or_bottom->layout(
				BRect(m_divider_left_or_top + Thickness, rect.top, rect.right, rect.bottom));
	}
