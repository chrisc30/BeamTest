...
public:
	UserResizeSplitView(MView* top_or_left_child, MView* right_or_bottom_child, const char* name,
		float divider_left_or_top, orientation posture = B_HORIZONTAL,
		bool should_resize_left_or_top = true, bool should_resize_right_or_bottom = true,
		bool can_hide_left_or_top = false, bool can_hide_right_or_bottom = false,
		bool move_slider_on_frame_resize = false, uint32 resize_mask = B_FOLLOW_LEFT | B_FOLLOW_TOP,
	...
	inline MView* LeftChild()
	{
		assert(m_posture == B_HORIZONTAL);
		return m_left_or_top;
	}
	inline MView* TopChild()
	{
		assert(m_posture == B_VERTICAL);
		return m_left_or_top;
	}
	inline MView* RightChild()
	{
		assert(m_posture == B_HORIZONTAL);
		return m_right_or_bottom;
	}
	inline MView* BottomChild()
	{
		assert(m_posture == B_VERTICAL);
		return m_right_or_bottom;
	}
	void SetMinimumChildAreaSizes(
		float left_or_top_minimum_size, float right_or_bottom_minimum_size);

	virtual void SetPreferredDividerLeftOrTop(float divider_left_or_top);
	virtual void SetDividerLeftOrTop(float divider_left_or_top, bool force = false);
