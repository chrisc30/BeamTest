...
//******************************************************************************************************
CLVColumn::CLVColumn(
	const char* label, float width, uint32 flags, float min_width, const char* labelname)
{
...
BRect
CLVColumn::TruncateText(float column_width)
// Returns whether the truncated text has changed
{
	column_width -= 1 + 8 + 5 + 1;
	// Because when I draw the text I start drawing 8 pixels to the right from the text area's left
	// edge, which is in turn 1 pixel smaller than the column at each edge, and I want 5 trailing
	// pixels.
...
void
GetTruncatedString(const char* full_string, char* truncated, float width, int32 truncate_buf_size,
	const BFont* font)
{
...
void
CLVColumn::DrawColumnHeader(BView* view, BRect header_rect, bool sort_key, bool focus,
	float font_ascent, CLVSortMode sortMode, int32 sortPrio)
{
	...
	if (label) {
		...
		if (!(fFlags & CLV_RIGHT_JUSTIFIED))
			...
			text_point.Set(
				header_rect.right - offs - string_width, header_rect.top + 1.0f + font_ascent);
		}
