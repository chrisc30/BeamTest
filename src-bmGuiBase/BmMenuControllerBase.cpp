...
/*------------------------------------------------------------------------------*\
	BmMenuControllerBase()
		-
\*------------------------------------------------------------------------------*/
BmMenuControllerBase::BmMenuControllerBase(const char* label, BHandler* msgTarget,
	BMessage* msgTemplate, BmRebuildMenuFunc func, int32 flags)
...
\*------------------------------------------------------------------------------*/
void
BmMenuControllerBase::UpdateItemList(void)
{
	if (mRebuildMenuFunc) {
		// menu is created by a dedicated function:
		(BeamGuiRoster->*mRebuildMenuFunc)(this);
		if (mFlags & BM_MC_ADD_NONE_ITEM) {
			BMenuItem* item = new BMenuItem(BM_NoItemLabel.String(), new BMessage(*mMsgTemplate));
...
\*------------------------------------------------------------------------------*/
void
BmMenuControllerBase::MarkItem(const char* label)
{
	mMarkedLabel = label;
}
...
\*------------------------------------------------------------------------------*/
void
BmMenuControllerBase::ClearMark()
{
...
\*------------------------------------------------------------------------------*/
BMenuItem*
BmMenuControllerBase::MarkItemInMenu(BMenu* menu, const char* label)
{
	...
	for (uint32 i = 0; currMenu && i < itemVect.size(); ++i) {
		...
		currMenu = item ? item->Submenu() : NULL;
	}
	if (item)
		item->SetMarked(true);
	else
...
\*------------------------------------------------------------------------------*/
void
BmMenuControllerBase::ClearMarkInMenu(BMenu* menu)
{
	if (!menu)
		return;
	BMenuItem* item;
...
\*------------------------------------------------------------------------------*/
void
BmMenuControllerBase::AttachedToWindow(void)
{
	UpdateItemList();
...
\*------------------------------------------------------------------------------*/
BPoint
BmMenuControllerBase::ScreenLocation()
{
...
\*------------------------------------------------------------------------------*/
BHandler*
BmMenuControllerBase::MsgTarget() const
{
	if (!mMsgTarget) {
...
\*------------------------------------------------------------------------------*/
void
BmMenuControllerBase::Shortcuts(const BmString s)
{
	mShortcuts = s;
	UpdateItemList();
}
