...
public:
	TextEntryAlert(const char* title, const char* info_text, const char* initial_entry_text,
		const char* button_0_label, const char* button_1_label = NULL, bool inline_label = true,
		float min_text_box_width = 120, int min_text_box_rows = 1,
...
private:
	...
	static filter_result KeyDownFilterStatic(
	...
	BmString*
		m_text_entry_buffer;  // Buffer to store the user-entered text when the user hits a button
