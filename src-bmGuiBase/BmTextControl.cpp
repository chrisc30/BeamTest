...
BmTextControl::BmTextControl(
	const char* label, bool labelIsMenu, int32 fixedTextLen, int32 minTextLen)
...
\*------------------------------------------------------------------------------*/
BmTextControl::BmTextControl(
	const char* label, BmMenuControllerBase* menu, int32 fixedTextLen, int32 minTextLen)
	...
	const char* label, int32 fixedTextLen, int32 minTextLen, BmMenuControllerBase* popup)
{
	...
	if (fixedTextLen) {
		...
		ct_mpm = minimax(int(width), int(b.Height() + 4), int(width), int(b.Height() + 4));
	} else {
		if (minTextLen)
			ct_mpm = minimax(int(divPos + font.StringWidth("W") * float(minTextLen)),
				int(b.Height() + 4), 100000, int(b.Height() + 4));
		else
			ct_mpm = minimax(int(divPos + font.StringWidth("W") * 10), int(b.Height() + 4), 100000,
				int(b.Height() + 4));
	}
	if (mLabelIsMenu) {
		float width, height;
		...
		if (BeamOnDano)
			mMenuField = new BMenuField(BRect(2, 2, Divider(), height), NULL, label, popup, true,
				B_FOLLOW_NONE, B_WILL_DRAW);
		else
			mMenuField = new BMenuField(BRect(2, 0, Divider(), height), NULL, label, popup, true,
				B_FOLLOW_NONE, B_WILL_DRAW);
...
\*------------------------------------------------------------------------------*/
void
BmTextControl::SetDivider(float divider)
{
...
\*------------------------------------------------------------------------------*/
void
BmTextControl::SetEnabled(bool enabled)
{
...
\*------------------------------------------------------------------------------*/
void
BmTextControl::SetText(const char* text)
{
...
\*------------------------------------------------------------------------------*/
void
BmTextControl::SetTextSilently(const char* text)
{
...
\*------------------------------------------------------------------------------*/
void
BmTextControl::FrameResized(float new_width, float new_height)
{
	BRect curr = Bounds();
...
BmTextControl::Menu() const
{
	return mMenuField ? dynamic_cast<BmMenuControllerBase*>(mMenuField->Menu()) : NULL;
}
...
\*------------------------------------------------------------------------------*/
BRect
BmTextControl::layout(BRect frame)
{
	if (frame == Frame())
		return frame;
	...
	mTextView->ResizeTo(frame.Width() - occupiedSpace - 4, mTextView->Frame().Height());
#endif	// __HAIKU__
	return frame;
}
