...
void
CLVColumnLabelView::SetLayoutLocked(bool layoutIsLocked)
{
...
bool
CLVColumnLabelView::LayoutLocked() const
{
...
void
CLVColumnLabelView::Draw(BRect update_rect)
{
	...
	for (int32 ColumnDraw = 0; ColumnDraw < NumberOfColumns; ColumnDraw++) {
		...
		if (ThisColumn->IsShown()) {
			...
			if (update_rect.Intersects(
					BRect(ColumnBegin, ViewBounds.top, ColumnEnd, ViewBounds.bottom))) {
				...
				if (!MergeWithLeft)
					AddLine(BPoint(ColumnBegin, ViewBounds.top + 1.0f),
						BPoint(ColumnBegin, ViewBounds.bottom), ui_color(B_SHINE_COLOR));
				// Bottom line
				...
				BRect TextRect(
					ColumnBegin + 1.0f, ViewBounds.top, ColumnEnd - 1.0f, ViewBounds.bottom - 1.0f);
				BRegion TextRegion;
				...
				ThisColumn->DrawColumnHeader(
					this, TextRect, sort_key, focus, fFontAscent, ThisColumn->fSortMode, sortPrio);
	...
	if (ColumnEnd < ViewBounds.right) {
		...
		if (update_rect.Intersects(
				BRect(ColumnEnd + 1.0f, ViewBounds.top, ViewBounds.right, ViewBounds.bottom))) {
			BeginLineArray(3);
			// Top line
	...
	if (fColumnClicked && fColumnDragging) {
		...
		BeginLineArray(4);
		AddLine(BPoint(DragOutlineLeft, ViewBounds.top), BPoint(DragOutlineRight, ViewBounds.top),
			ui_color(B_NAVIGATION_BASE_COLOR));
		AddLine(BPoint(DragOutlineLeft, ViewBounds.bottom),
			BPoint(DragOutlineRight, ViewBounds.bottom), ui_color(B_NAVIGATION_BASE_COLOR));
		AddLine(BPoint(DragOutlineLeft, ViewBounds.top + 1.0f),
			BPoint(DragOutlineLeft, ViewBounds.bottom - 1.0f), ui_color(B_NAVIGATION_BASE_COLOR));
		AddLine(BPoint(DragOutlineRight, ViewBounds.top + 1.0f),
			BPoint(DragOutlineRight, ViewBounds.bottom - 1.0f), ui_color(B_NAVIGATION_BASE_COLOR));
		EndLineArray();
...
void
CLVColumnLabelView::MouseDown(BPoint Point)
{
	...
	if (Buttons == B_PRIMARY_MOUSE_BUTTON && !fLayoutIsLocked) {
		...
		// Make sure no other column was already clicked.  If so, just discard the old one and
		// redraw the view
				...
				if (Point.x >= ColumnBegin && Point.x <= ColumnEnd) {
					...
					if (Point.x <= ColumnBegin + 2.0f) {
						...
						fDragBoxMouseHoldOffset
							= Point.x - ((CLVDragGroup*)fDragGroups.ItemAt(fDragGroup))->GroupBegin;
						Invalidate(BRect(ColumnBegin + 1.0f, ViewBounds.top + 1.0f,
							ColumnEnd - 1.0f, ViewBounds.bottom - 1.0f));
...
void
CLVColumnLabelView::MouseMoved(BPoint where, uint32 code, const BMessage*)
{
	if (fLayoutIsLocked)
		return;
			...
			if (where.x < fPreviousMousePos.x || where.x > fPreviousMousePos.x) {
				...
				do {
					...
					if (fSnapMin != -1.0f && where.x < fSnapMin) {
						...
						else if (fTheShownGroupBefore->LastColumnShown->fFlags
					...
					if (fSnapMax != -1.0f && where.x > fSnapMax) {
						...
						if (fTheDragGroup->LastColumnShown->fFlags & CLV_MERGE_WITH_RIGHT)
							ColumnsUpdateRight += 1.0f;
						else if (fTheShownGroupAfter->LastColumnShown->fFlags
								 & CLV_MERGE_WITH_RIGHT)
							ColumnsUpdateRight += 1.0f;
...
void
CLVColumnLabelView::MouseUp(BPoint)
{
	if (fColumnClicked == NULL)
		return;
	BRect ViewBounds = Bounds();
	if (!fColumnDragging && !fColumnResizing) {
		...
		Invalidate(BRect(fPrevDragOutlineLeft, ViewBounds.top + 1.0f, fPrevDragOutlineLeft,
			ViewBounds.bottom - 1.0f));
		Invalidate(BRect(fPrevDragOutlineRight, ViewBounds.top + 1.0f, fPrevDragOutlineRight,
			ViewBounds.bottom - 1.0f));
		Invalidate(
			BRect(fPrevDragOutlineLeft, ViewBounds.top, fPrevDragOutlineRight, ViewBounds.top));
		Invalidate(BRect(
			fPrevDragOutlineLeft, ViewBounds.bottom, fPrevDragOutlineRight, ViewBounds.bottom));
	} else
...
void
CLVColumnLabelView::ShiftDragGroup(int32 NewPos)
...
CLVColumnLabelView::UpdateDragGroups()
{
	...
	for (int32 Counter = 0; Counter < NumberOfColumns; Counter++) {
		...
		CurrentGroup->Flags
			|= CurrentColumn->fFlags & (CLV_NOT_MOVABLE | CLV_LOCK_AT_BEGINNING | CLV_LOCK_AT_END);
...
void
CLVColumnLabelView::MessageReceived(BMessage* msg)
{
