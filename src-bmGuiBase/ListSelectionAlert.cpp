...
ListSelectionAlert::ListSelectionAlert(const char* title, const char* info_text, BList& items,
	const char* initial_selection, const char* button_0_label, const char* button_1_label,
	float min_list_box_width, int min_list_box_height, button_width width_style, const BRect* frame,
	...
	: BWindow((frame ? *frame : BRect(50, 50, 60, 60)), title, look, feel, flags)
{
	...
	const char* strings[2]
		= {info_text ? info_text : "", initial_selection ? initial_selection : ""};
	...
	info_box.bottom
		= info_box.top + (ceilf(plain_font_height.ascent) + ceilf(plain_font_height.descent)) * 4;
	sel_list_rect = result_rects[1];
	sel_list_rect.bottom = sel_list_rect.top + float(min_list_box_height);
	sel_list_rect.InsetBy(
	...
	if (info_text) {
		...
		info_box.right += 1;
		m_label_view = new BTextView(
			info_box, NULL, info_box.OffsetToCopy(0, 0), B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW);
	...
	BetterScrollView* container = new BetterScrollView(
		minimax(0, 0, 1e5, 1e5), m_list_view, BM_SV_H_SCROLLBAR | BM_SV_V_SCROLLBAR | BM_SV_CORNER);
	m_list_view->AddColumn(new CLVColumn(title, 300.0, CLV_TELL_ITEMS_WIDTH, 300.0));
	...
	if (right_button != NULL) {
		...
		right_button->MoveTo(
			sel_list_rect.right + B_V_SCROLL_BAR_WIDTH - right_button->Frame().Width() + 1,
	...
	if (m_buttons[0] != NULL && m_buttons[1] != NULL) {
		...
		m_buttons[0]->MoveTo(
			button_left, sel_list_rect.bottom + B_H_SCROLL_BAR_HEIGHT + c_item_spacing);
	}
	...
	ResizeTo(sel_list_rect.right + c_item_spacing + B_V_SCROLL_BAR_WIDTH,
...
int32
ListSelectionAlert::Go(char* selection_buffer, int32 buffer_size)
{
...
status_t
ListSelectionAlert::Go(BInvoker* invoker)
{
	m_invoker = invoker;
	m_list_view->MakeFocus(true);
	Show();
...
void
ListSelectionAlert::SetShortcut(int32 index, char shortcut)
{
	if (index >= 0 && index <= 1)
...
char
ListSelectionAlert::Shortcut(int32 index) const
{
...
BTextView*
ListSelectionAlert::LabelView(void) const
{
	return m_label_view;
}
...
ColumnListView*
ListSelectionAlert::ListView(void) const
{
	return m_list_view;
}
...
void
ListSelectionAlert::MessageReceived(BMessage* message)
{
...
//******************************************************************************************************
ListSelectionAlertBackgroundView::ListSelectionAlertBackgroundView(
	BRect frame, BRect entry_text_rect)
...
void
ListSelectionAlertBackgroundView::FrameResized(float, float)
{
