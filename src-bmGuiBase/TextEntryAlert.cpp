...
TextEntryAlert::TextEntryAlert(const char* title, const char* info_text,
	const char* initial_entry_text, const char* button_0_label, const char* button_1_label,
	bool inline_label, float min_text_box_width, int min_text_box_rows, button_width width_style,
	bool multi_line, const BRect* frame, window_look look, window_feel feel, uint32 flags)
	: BWindow((frame ? *frame : BRect(50, 50, 60, 60)), title, look, feel, flags)
{
	...
	const char* strings[2]
		= {info_text ? info_text : "", initial_entry_text ? initial_entry_text : ""};
	escapement_delta zero_escapements[2] = {{0, 0}, {0, 0}};
	BRect result_rects[2];
	be_plain_font->GetBoundingBoxesForStrings(
		strings, 2, B_SCREEN_METRIC, zero_escapements, result_rects);
	...
	entry_text_rect
		= result_rects[1].IsValid() ? result_rects[1] : BRect(0, 0, min_text_box_width - 1, 0);
	entry_text_rect.bottom = entry_text_rect.top
							 + (ceilf(plain_font_height.ascent) + ceilf(plain_font_height.descent))
								   * float(min_text_box_rows)
							 + ceilf(plain_font_height.leading) * float(min_text_box_rows - 1);
	entry_text_rect.InsetBy(
		0 - (c_text_margin + c_text_border_width), 0 - (c_text_margin + c_text_border_width));
	...
	if (button_0_label != NULL) {
		...
		m_buttons[0] = new BButton(BRect(0, 0, 0, 0), button_0_label, button_0_label, message,
	...
	if (button_1_label != NULL) {
		...
		m_buttons[1] = new BButton(BRect(0, 0, 0, 0), button_1_label, button_1_label, message,
	...
	m_text_entry_view = new TextEntryAlertTextEntryView(
		entry_text_rect.InsetByCopy(c_text_border_width, c_text_border_width), multi_line);
	...
	if (right_button != NULL) {
		if (width_style == B_WIDTH_AS_USUAL)
			...
			right_button->ResizeTo(
				right_button == m_buttons[1] ? button_1_width : button_0_width, buttons_height);
...
int32
TextEntryAlert::Go(BmString& text_entry_buffer)
{
...
status_t
TextEntryAlert::Go(BInvoker* invoker)
{
	m_invoker = invoker;
	m_text_entry_view->MakeFocus(true);
	Show();
...
void
TextEntryAlert::SetShortcut(int32 index, char shortcut)
{
	if (index >= 0 && index <= 1)
...
char
TextEntryAlert::Shortcut(int32 index) const
{
...
filter_result
TextEntryAlert::KeyDownFilterStatic(BMessage* message, BHandler**, BMessageFilter* filter)
{
...
filter_result
TextEntryAlert::KeyDownFilter(BMessage* message)
{
...
BTextView*
TextEntryAlert::LabelView(void) const
{
	return m_label_view;
}
...
BTextView*
TextEntryAlert::TextEntryView(void) const
{
...
void
TextEntryAlert::SetTabAllowed(bool allow)
{
...
void
TextEntryAlert::MessageReceived(BMessage* message)
{
	if (message->what == c_button_pressed) {
		...
		if (message->FindInt32("which", &which) == B_NO_ERROR) {
			...
			if (m_done_mutex < B_NO_ERROR) {
				...
				if (message
					&& (message->AddInt32("which", which) == B_NO_ERROR
				...
				// Synchronous version: set the result button and text buffer, then release the
				// thread that created me
...
void
TextEntryAlertBackgroundView::Draw(BRect update_rect)
{
...
void
TextEntryAlertBackgroundView::FrameResized(float, float)
{
...
void
TextEntryAlertTextEntryView::SetTabAllowed(bool allow)
{
	m_tab_allowed = allow;
}
...
void
TextEntryAlertTextEntryView::KeyDown(const char* bytes, int32 num_bytes)
{
	if (num_bytes == 1
		&& ((bytes[0] == B_TAB && !m_tab_allowed) || (bytes[0] == B_ENTER && !m_multi_line))) {
...
void
TextEntryAlertTextEntryView::AttachedToWindow()
{
