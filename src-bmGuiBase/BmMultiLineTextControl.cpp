...
BmMultiLineTextControl::BmMultiLineTextControl(
	const char* label, bool labelIsMenu, int32 lineCount, int32 minTextLen, bool fixedHeight)
	  ...
	  mLabelIsMenu(labelIsMenu)
{
	...
	if (minTextLen)
		ct_mpm = minimax(int(divPos + font.StringWidth("W") * float(minTextLen)), int(height + 4),
			100000, fixedHeight ? int(height + 4) : 100000);
	else
		ct_mpm = minimax(int(divPos + font.StringWidth("W") * 10), int(height + 4), 100000,
	...
	if (labelIsMenu) {
		...
		mMenuField = new BMenuField(
			BRect(2, 0, Divider(), height), NULL, label, popup, true, B_FOLLOW_NONE, B_WILL_DRAW);
...
\*------------------------------------------------------------------------------*/
void
BmMultiLineTextControl::SetDivider(float divider)
{
...
\*------------------------------------------------------------------------------*/
float
BmMultiLineTextControl::Divider() const
{
...
\*------------------------------------------------------------------------------*/
void
BmMultiLineTextControl::SetEnabled(bool enabled)
{
...
\*------------------------------------------------------------------------------*/
void
BmMultiLineTextControl::SetText(const char* text)
{
...
\*------------------------------------------------------------------------------*/
void
BmMultiLineTextControl::SetTextSilently(const char* text)
{
...
\*------------------------------------------------------------------------------*/
void
BmMultiLineTextControl::FrameResized(float new_width, float new_height)
{
	BRect curr = Bounds();
...
\*------------------------------------------------------------------------------*/
minimax
BmMultiLineTextControl::layoutprefs()
{
	return mpm = ct_mpm;
}
...
\*------------------------------------------------------------------------------*/
BRect
BmMultiLineTextControl::layout(BRect frame)
{
	if (frame == Frame())
		return frame;
	...
	m_text_view->ResizeTo(
		frame.Width() - occupiedSpace - 4, frame.Height() - m_text_view->Frame().top - 4 - 2);
	return frame;
}
