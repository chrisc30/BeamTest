...
ImageAboutWindow::ImageAboutWindow(const char* window_title, const char* app_title,
	const BBitmap* bmap, float icon_sidebar_offset, const char* body_text, const char* email,
	const char* web, const char* credits)
	  ...
	  m_credits_pos(0)
{
	...
	if (app_version_resource) {
		...
		const char* app_version_data = (const char*)app_version_resource->LoadResource(
			'APPV', "BEOS:APP_VERSION", &resource_size);
	...
	if (m_num_lines > 0) {
		...
		body_width
			= GetStringsMaxWidth((const char**)m_lines, m_num_lines, be_plain_font, body_lengths);
	...
	m_title_rect.top
		= m_logo_rect.top + (floorf(((m_logo_rect.bottom - m_logo_rect.top) - bold_height) / 2));
	...
	for (int i = 0; i < m_num_lines; i++) {
		if (m_lines[i]) {
			m_text_rects[i].left
				= logo_right + 10 + floorf((string_widths - body_lengths[i]) / 2.0f);
	...
	if (credits) {
		...
		m_credits_view
			= new BTextView(credits_rect, "credits", text_rect, B_FOLLOW_NONE, B_WILL_DRAW);
...
void
ImageAboutWindow::MessageReceived(BMessage* message)
{
...
void
ImageAboutWindow::MouseDown(BPoint point)
{
...
void
ImageAboutWindow::ScrollCredits()
{
...
void
ImageAboutWindow::DrawContent(BView* view, BRect update_rect)
{
	...
	if (m_version && update_rect.Intersects(m_version_rect))
		view->DrawString(
			m_version, BPoint(m_version_rect.left, m_version_rect.top + m_plain_font_ascent));
	if (m_email && update_rect.Intersects(m_email_rect)) {
		view->SetHighColor(ui_color(B_CONTROL_HIGHLIGHT_COLOR));
		view->DrawString(
			m_email, BPoint(m_email_rect.left, m_email_rect.top + m_plain_font_ascent));
	...
	for (int i = 0; i < m_num_lines; i++)
		if (m_lines[i] && update_rect.Intersects(m_text_rects[i]))
			view->DrawString(m_lines[i],
				BPoint(m_text_rects[i].left, m_text_rects[i].top + m_plain_font_ascent));
}
...
void
AboutView::Draw(BRect update_rect)
{
