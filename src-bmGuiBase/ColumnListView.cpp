...
//******************************************************************************************************
ColumnListView::ColumnListView(BRect Frame, const char* Name, uint32 flags, list_view_type Type,
...
status_t
ColumnListView::UISettingsChanged(const BMessage* changes, uint32 flags)
{
	fSelectedItemColorWindowActive = ui_color(B_MENU_SELECTED_BACKGROUND_COLOR);
	fSelectedItemColorWindowInactive = ui_color(B_MENU_SELECTED_BACKGROUND_COLOR);
	fSelectedItemColorTintedWindowActive = BmWeakenColor(B_MENU_SELECTED_BACKGROUND_COLOR, 1);
...
void
ColumnListView::UpdateDataRect(bool scrolling_allowed)
{
...
void
ColumnListView::AssertWindowLocked() const
{
#ifdef DEBUG
...
// Adds a BList of CLVColumn's to the ColumnListView at the position specified, or at the end of the
// list if AtIndex == -1.  Returns true if successful.
{
	AssertWindowLocked();
	...
	for (Counter = 0; Counter < NumberOfColumnsToAdd; Counter++) {
		...
		// Check if this should be locked at the beginning or end, and adjust its position if
		// necessary
		int32 DisplayIndex = NumberOfColumns;
		if (!(Column->Flags() & CLV_LOCK_AT_END)) {
			bool Repeat;
...
CLVColumn*
ColumnListView::ColumnAt(int32 column_index) const
{
	AssertWindowLocked();
		...
		for (int32 Counter = 0; Counter < number_of_items; Counter++) {
			...
			if (ThisColumn->fPushedByExpander || (ThisColumn->fFlags & CLV_EXPANDER)) {
				...
				if (ExpanderShift > 0) {
					...
					if (DisplayIndex >= 0) {
						// The column scrolling scrolled some content that is encroaching into the
						// next column and needs to be invalidated now
...
void
ColumnListView::GetDisplayOrder(int32* order) const
{
	AssertWindowLocked();
...
ColumnListView::SetSortKey(int32 ColumnIndex)
{
	...
	if (fSortKeyList.ItemAt(0) != Column || Column == NULL) {
		...
		if (Column) {
			...
			fColumnLabelView->Invalidate(BRect(
				Column->fColumnBegin, LabelBounds.top, Column->fColumnEnd, LabelBounds.bottom));
		}
	}
...
ColumnListView::AddSortKey(int32 ColumnIndex)
{
	...
	if (Column && !fSortKeyList.HasItem(Column)) {
		...
		fColumnLabelView->Invalidate(
			BRect(Column->fColumnBegin, LabelBounds.top, Column->fColumnEnd, LabelBounds.bottom));
	}
	SortingChanged();
}

bool
ColumnListView::IsSortKey(int32 ColumnIndex)
{
	AssertWindowLocked();
...
void
ColumnListView::SetSortMode(int32 ColumnIndex, CLVSortMode Mode, bool doSort)
{
	AssertWindowLocked();
	CLVColumn* Column;
	if (ColumnIndex >= 0) {
...
int32
ColumnListView::GetSorting(int32* SortKeys, CLVSortMode* SortModes) const
{
	AssertWindowLocked();
...
void
ColumnListView::SetSorting(int32 NumberOfKeys, int32* SortKeys, CLVSortMode* SortModes)
{
	AssertWindowLocked();
...
void
ColumnListView::MouseDown(BPoint where)
{
	// BListView::MouseDown gives undesirable behavior during click-and drag (it reselects instead
	// of doing nothing to allow for initiating drag & drop), so I need to replace the selection
	// behavior, and while I'm at it, I'll just do an InitiateDrag hook function.
			...
			if (!clicked_item->IsSelected()) {
				...
				if (modifier_keys & fToggleSelMask)
					...
					if (clicks > 1)
						...
						// If neither key held down, watch for drag, but if no drag and just a click
						// on this item, select it alone at mouse up.
						fLastMouseDown = where;
...
void
ColumnListView::MouseUp(BPoint where)
{
...
void
ColumnListView::MouseMoved(BPoint where, uint32, const BMessage*)
{
	if (fWatchingForDrag
		&& (where.x < fLastMouseDown.x - 4 || where.x > fLastMouseDown.x + 4
...
void
ColumnListView::WindowActivated(bool active)
{
...
bool
ColumnListView::AddUnder(BListItem* a_item, BListItem* a_superitem)
{
	AssertWindowLocked();
...
int32
ColumnListView::DetermineSortedPosHierarchical(CLVListItem* item, uint32 startIndex)
{
...
bool
ColumnListView::AddItem(BListItem* a_item, int32 fullListIndex)
{
	// Get the CLVListItems
	CLVListItem* item = cast_as(a_item, CLVListItem);
	if (item == NULL)
...
bool
ColumnListView::AddItem(BListItem* a_item)
{
	// Get the CLVListItems
	CLVListItem* item = cast_as(a_item, CLVListItem);
	if (item == NULL)
...
void
ColumnListView::ReSortItem(CLVListItem* item)
{
	if (!item)
		return;
...
int32
ColumnListView::DetermineSortedPos(CLVListItem* item)
{
	int32 SortDepth = fSortKeyList.CountItems();
...
bool
ColumnListView::ReplaceItemsWith(BList* newItems, bool sort)
{
	AssertWindowLocked();
	MakeEmptyPrivate();
...
bool
ColumnListView::RemoveItem(BListItem* a_item)
{
	AssertWindowLocked();
...
void
ColumnListView::FullListDoForEach(bool (*func)(CLVListItem*, void*), void* arg2)
{
	AssertWindowLocked();
	int32 NumberOfItems = fFullItemList.CountItems();
	for (int32 Counter = 0; Counter < NumberOfItems; Counter++)
...
int
ColumnListView::CompareItems(const CLVListItem* item1, const CLVListItem* item2)
{
	int32 SortDepth = fSortKeyList.CountItems();
	int CompareResult = 0;
	for (int32 SortIteration = 0; SortIteration < SortDepth && CompareResult == 0;
...
ColumnListView::SortItems()
{
	...
	if (!fHierarchical) {
		...
		// Do the actual sort
		BListView::SortItems(
			(int (*)(const void*, const void*))ColumnListView::HierarchicalBListSortFunc);
	}
	if (selectedItem)
...
int
ColumnListView::PlainBListSortFunc(BListItem** a_item1, BListItem** a_item2)
{
...
int
ColumnListView::HierarchicalBListSortFunc(BListItem** a_item1, BListItem** a_item2)
{
	CLVListItem* item1 = (CLVListItem*)*a_item1;
	CLVListItem* item2 = (CLVListItem*)*a_item2;
...
void
ColumnListView::SortFullListSegment(
	int32 OriginalListStartIndex, int32 InsertionPoint, BList* NewList)
{
	...
	for (int32 Counter = InsertionPoint; Counter < NewItemsStopIndex; Counter++) {
		...
		CLVListItem* NextItem
			= (CLVListItem*)fFullItemList.ItemAt(fFullItemList.IndexOf(ThisItem) + 1);
		if (ThisItem->IsSuperItem() && NextItem
			&& ThisItem->OutlineLevel() < NextItem->OutlineLevel()) {
...
BList*
ColumnListView::SortItemsInThisLevel(int32 OriginalListStartIndex)
{
...
void
ColumnListView::SortListArray(CLVListItem** SortArray, int32 NumberOfItems)
{
	if (fCompare == NULL)
		// No sorting function
		return;
...
void
ColumnListView::SetDisconnectScrollView(bool disconnect)
{
...
\*------------------------------------------------------------------------------*/
void
ColumnListView::DrawColumn(BRect updateRect, int32 column_index)
{
...
\*------------------------------------------------------------------------------*/
void
ColumnListView::ShowColumn(int32 col_index)
{
	AssertWindowLocked();
...
\*------------------------------------------------------------------------------*/
void
ColumnListView::HideColumn(int32 col_index)
{
	AssertWindowLocked();
...
\*------------------------------------------------------------------------------*/
void
ColumnListView::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		case 'PSTE':
		{
...
\*------------------------------------------------------------------------------*/
status_t
ColumnListView::Archive(BMessage* archive, bool) const
{
	AssertWindowLocked();
	status_t ret = B_OK;
	int i;
	...
	if (ret == B_OK) {
		...
		for (i = 0; i < numCols && ret == B_OK; ++i) {
			ret = archive->AddFloat(MSG_COLWIDTH, ((CLVColumn*)fColumnList.ItemAt(i))->Width());
		}
	}
...
\*------------------------------------------------------------------------------*/
status_t
ColumnListView::Unarchive(const BMessage* archive, bool)
{
	AssertWindowLocked();
	int i;

	if (!archive || archive->IsEmpty())
		return B_OK;
...
void
ColumnListView::SetExtendedSelectionPolicy(bool likeTracker)
{
	if (likeTracker) {
