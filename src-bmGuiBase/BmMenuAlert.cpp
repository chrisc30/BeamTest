...
BmMenuAlert::BmMenuAlert(float width, float height, const char* title, const char* info_text,
	...
	: MWindow(BRect(1, 1, width, height), title, look, feel, flags),
	  m_menu_field(menu)
{
	VGroup* vg = new VGroup(m_label_view = new MTextView(), menu, new Space(minimax(0, 5, 1e5, 5)),
		new HGroup(new Space(),
			m_buttons[0] = new MButton(button_0_label, (ulong)0, minimax(74, -1, 74, -1)),
			m_buttons[1] = new MButton(button_1_label, (ulong)1, minimax(74, -1, 74, -1)), 0),
		0);
...
int32
BmMenuAlert::Go(char* selection_buffer, int32 buffer_size)
{
...
status_t
BmMenuAlert::Go(BInvoker* invoker)
{
	m_invoker = invoker;
	m_menu_field->MakeFocus(true);
	Show();
...
void
BmMenuAlert::SetShortcut(int32 index, char shortcut)
{
	if (index >= 0 && index <= 1)
...
char
BmMenuAlert::Shortcut(int32 index) const
{
...
filter_result
BmMenuAlert::KeyDownFilterStatic(BMessage* message, BHandler**, BMessageFilter* filter)
{
...
filter_result
BmMenuAlert::KeyDownFilter(BMessage* message)
{
...
void
BmMenuAlert::MessageReceived(BMessage* message)
{
	switch (message->what) {
				...
				if (m_done_mutex < B_NO_ERROR) {
					...
					if (message
						...
						&& (message->AddString("selection", m_selection.String()) == B_NO_ERROR
							|| message->ReplaceString("selection", m_selection.String())
								   == B_NO_ERROR))
						m_invoker->Invoke();
				} else {
		...
		case BM_MENUITEM_SELECTED:
		{
			...
			if (item) {
				...
				BmString path;
				while (currMenu && currItem && currItem != m_menu_field->MenuItem()) {
					if (!path.Length())
