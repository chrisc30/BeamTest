...
WrappingTextView::UndoInfo::UndoInfo(
	bool ins, const char* t, int32 len, uint32 offs, const text_run_array* runs, bool delToRight)
	: isInsertion(ins),
	  text(t, len),
	  offset(offs),
	  text_runs(NULL),
...
WrappingTextView::UndoInfo
WrappingTextView::UndoInfo::operator=(const UndoInfo& ui)
{
	isInsertion = ui.isInsertion;
	text = ui.text;
	offset = ui.offset;
...
void
WrappingTextView::UndoInfo::JoinTextRuns(const text_run_array* a_runs, int32 len, bool atFront)
{
	if (a_runs) {
		if (text_runs) {
...
WrappingTextView::WrappingTextView(
	BRect a_frame, const char* a_name, int32 a_resize_mode, int32 a_flags)
	: BTextView(a_frame, a_name,
		  BRect(
			  4.0f, 4.0f, a_frame.right - a_frame.left - 4.0f, a_frame.bottom - a_frame.top - 4.0f),
...
void
WrappingTextView::DetachedFromWindow()
{
	// This is sort of what the destructor should do, but...  Derived class's destructors get called
	// before WrappingTextView's destructor so StoreChange won't get to the derived class's version
	// of it.
	if (m_modified) {
		StoreChange();
		m_modified = false;
...
void
WrappingTextView::FrameResized(float a_width, float a_height)
{
...
void
WrappingTextView::MakeFocus(bool a_focused)
{
...
void
WrappingTextView::InsertText(
	const char* a_text, int32 a_length, int32 a_offset, const text_run_array* a_runs)
{
	...
	if (IsEditable()) {
		if (!m_in_undo_redo) {
			...
			if (canConcat) {
				...
				m_undo_vect[m_curr_undo_index++]
					= UndoInfo(true, a_text, a_length, a_offset, a_runs);
...
void
WrappingTextView::DeleteText(int32 start, int32 finish)
{
	if (IsEditable()) {
		if (!m_in_undo_redo) {
			...
			if (canConcat) {
				...
				m_undo_vect[m_curr_undo_index++]
					= UndoInfo(false, Text() + start, finish - start, start, runs, deleteToRight);
			if (runs)
				free(runs);
			m_max_undo_index = MAX(m_max_undo_index, m_curr_undo_index);
		}
...
void
WrappingTextView::ResetUndoBuffer()
{
...
void
WrappingTextView::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		case B_UNDO:
		{
...
void
WrappingTextView::SetModificationMessage(BMessage* msg)
{
	delete m_modification_msg;
...
void
WrappingTextView::SetText(const char* text, int32 length, const text_run_array* runs)
{
...
void
WrappingTextView::SetText(const char* text, const text_run_array* runs)
{
	m_modified_disabled = true;
...
void
WrappingTextView::SetText(BFile* file, int32 offset, int32 length, const text_run_array* runs)
{
	m_modified_disabled = true;
...
void
WrappingTextView::StoreChange()
{
}


void
WrappingTextView::ResetTextRect()
{
	BRect bounds = Bounds();
...
bool
WrappingTextView::HasBeenModified()
{
	return m_modified;
}

void
WrappingTextView::CalculateVerticalOffset()
{
	float offset = 0;
...
void
WrappingTextView::KeyDown(const char* bytes, int32 numBytes)
{
...
void
WrappingTextView::SetFixedWidth(int32 i)
{
	m_fixed_width = i;
	ResetTextRect();
}
