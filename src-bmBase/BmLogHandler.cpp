...
\*------------------------------------------------------------------------------*/
void
ShowAlert(const BmString& text)
{
	BAlert* alert
		= new BAlert(NULL, text.String(), "OK", NULL, NULL, B_WIDTH_AS_USUAL, B_STOP_ALERT);
	alert->Go();
}
...
\*------------------------------------------------------------------------------*/
void
ShowAlertWithType(const BmString& text, alert_type type)
{
	BAlert* alert = new BAlert(NULL, text.String(), "OK", NULL, NULL, B_WIDTH_AS_USUAL, type);
	alert->Go();
}
...
\*------------------------------------------------------------------------------*/
BmString
TimeToString(time_t utc, const char* format)
{
	BmString s;
...
\*------------------------------------------------------------------------------*/
void
BmLogHandler::Log(const BmString logname, const BmString& msg)
{
	if (TheLogHandler)
...
\*------------------------------------------------------------------------------*/
void
BmLogHandler::Log(const char* const logname, const char* msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmLogHandler::Shutdown(bool sync)
{
	if (TheLogHandler)
...
\*------------------------------------------------------------------------------*/
void
BmLogHandler::FinishLog(const BmString& logname)
{
	if (TheLogHandler)
...
\*------------------------------------------------------------------------------*/
void
BmLogHandler::StartWatchingLogfile(BHandler* handler, const char* logfileName)
{
	mLocker.Lock();
...
\*------------------------------------------------------------------------------*/
void
BmLogHandler::StopWatchingLogfile(BHandler* handler, const char* logfileName)
{
	mLocker.Lock();
	BmWatcherInfo* info = WatcherInfoFor(logfileName);
	if (!info) {
...
\*------------------------------------------------------------------------------*/
BmLogHandler*
BmLogHandler::CreateInstance(uint32 logLevels, node_ref* appFolder)
{
	if (TheLogHandler)
	...
	if (find_directory(B_SYSTEM_LOG_DIRECTORY, &logPath, true) == B_OK) {
		...
		if (!mLogFolder.Contains("beam")) {
			...
			if (appFolder.Contains("logs")) {
				...
				BmString mvCmd = BmString("mv ")
								 << oldLogPath.Path() << " " << logPath.Path() << "/" << "beam";
				system(mvCmd.String());
			}
		}
	}
...
\*------------------------------------------------------------------------------*/
void
BmLogHandler::LogLevels(uint32 loglevels, int32 minFileSize, int32 maxFileSize)
{
	mLoglevels = loglevels;
...
\*------------------------------------------------------------------------------*/
BmLogHandler::BmLogfile*
BmLogHandler::LogfileFor(const BmString& logname)
{
	BmLogfile* log = NULL;
	int32 count = mActiveLogs.CountItems();
	for (int i = 0; i < count; ++i) {
...
\*------------------------------------------------------------------------------*/
BmLogHandler::BmLogfile*
BmLogHandler::FindLogfile(const BmString& ln)
{
	BmString logFolderName = BeamInTestMode ? "beam_test"  // use another log-folder in testmode
											: "beam";
	...
	if (!log) {
		...
		BFile* logfile
			= new BFile(&mLogFolder, name.String(), B_READ_WRITE | B_CREATE_FILE | B_OPEN_AT_END);
		...
		if (logfile->Position() > mMaxFileSize) {
			...
			newSize = logfile->ReadAt(logfile->Position() - newSize, buf, size_t(newSize));
			buf[newSize] = '\0';
			int32 offs = 0;
			char* pos = strchr(buf, '\n');
			if (pos != NULL)
...
\*------------------------------------------------------------------------------*/
bool
BmLogHandler::CheckLogLevel(uint32 terrain, int8 minlevel) const
{
...
\*------------------------------------------------------------------------------*/
void
BmLogHandler::LogToFile(const BmString& logname, const BmString& msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmLogHandler::LogToFile(const BmString& logname, const char* msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmLogHandler::CloseLog(const BmString& logname)
{
	BAutolock lock(mLocker);
	if (lock.IsLocked()) {
		BmLogfile* log = LogfileFor(logname);
...
\*------------------------------------------------------------------------------*/
void
BmLogHandler::BmLogfile::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		case BM_LOG_MSG:
		{
...
\*------------------------------------------------------------------------------*/
void
BmLogHandler::BmLogfile::Write(const char* const msg, int32 threadId)
{
	BmString s(msg);
	s.ReplaceAll("\r", "<CR>");
	s.ReplaceAll("\n\n", "\n");
	...
	char buf[40];
	sprintf(buf, "<%6" B_PRId32 "|%s.%03" B_PRId32 ">: ", threadId,
		TimeToString(now, "%Y-%m-%d|%H:%M:%S").String(), nowMSecs);
	s.Prepend(buf);
	ssize_t result;
