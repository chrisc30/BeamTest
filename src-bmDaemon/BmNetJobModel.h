...
public:
	...
	// getters:
	inline const BmString& BottomStatusText() const { return mBottomStatusText; }
...
public:
	...
	inline const BmString& AnswerText() const { return mAnswerText; }
	inline const BmString& StatusText() const { return mStatusFilter->StatusText(); }
	inline const BmString& BottomStatusText() const { return mStatusFilter->BottomStatusText(); }
...
protected:
	...
	virtual bool CheckForPositiveAnswer(uint32 expectedSize = 4096, bool dotstuffDecoding = false,
		bool update = false, BMessage* infoMsg = NULL);
	virtual void GetAnswer(uint32 expectedSize = 4096, bool dotstuffDecoding = false,
		bool update = false, BMessage* infoMsg = NULL);
	virtual void SendCommand(const BmString& cmd, const BmString& secret = BM_DEFAULT_STRING,
		bool dotstuffEncoding = false, bool update = false);
	virtual void SendCommandBuf(BmStringIBuf& cmd, const BmString& secret = BM_DEFAULT_STRING,
		bool dotstuffEncoding = false, bool update = false);
...
public:
	BmTrafficLogger(BmMemIBuf* input, BmNetJobModel* job, int32 logLimit, const char* prefix,
...
public:
	BmDotstuffDecoder(BmMemIBuf* input, BmNetJobModel* job, uint32 blockSize = nBlockSize);
...
public:
	BmDotstuffEncoder(BmMemIBuf* input, BmNetJobModel* job, uint32 blockSize = nBlockSize);
...
public:
	...
	// getters:
	BmNetEndpoint* Connection() { return mJob ? mJob->Connection() : NULL; }
