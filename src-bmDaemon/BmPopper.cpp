...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmPopStatusFilter::BmPopStatusFilter(BmMemIBuf* input, BmNetJobModel* job, uint32 blockSize)
...
\*------------------------------------------------------------------------------*/
void
BmPopStatusFilter::Filter(const char* srcBuf, uint32& srcLen, char* destBuf, uint32& destLen)
{
	...
	if (mHaveStatus) {
		...
		if (src < srcEnd) {
			...
			if (statusSize > 0) {
				...
				if (!needData || (mStatusText.Length() && mStatusText.ByteAt(0) != '+'))
...
\*------------------------------------------------------------------------------*/
bool
BmPopStatusFilter::CheckForPositiveAnswer()
{
...
BmPopper::PopState BmPopper::PopStates[BmPopper::POP_FINAL] = {
	PopState("connect...", &BmPopper::StateConnect), PopState("capa...", &BmPopper::StateCapa),
	PopState("starttls...", &BmPopper::StateStartTLS), PopState("auth...", &BmPopper::StateAuth),
	PopState("check...", &BmPopper::StateCheck), PopState("cleanup...", &BmPopper::StateCleanup),
	PopState("get...", &BmPopper::StateRetrieve), PopState("quit...", &BmPopper::StateDisconnect),
	PopState("done", NULL)};
...
BmPopper::BmPopper(const BmString& name, BmPopAccount* account)
	: inherited(BmString("POP_") << name, BM_LogRecv, new BmPopStatusFilter(NULL, this)),
	  mPopAccount(account),
	  mCurrMailNr(0),
	  mMsgCount(0),
	  mNewMsgCount(0),
...
\*------------------------------------------------------------------------------*/
bool
BmPopper::ShouldContinue()
{
	if (mConnection && mConnection->IsStopRequested())
		return false;
	return CurrentJobSpecifier() == BM_AUTH_ONLY_JOB
		   || CurrentJobSpecifier() == BM_CHECK_CAPABILITIES_JOB || inherited::ShouldContinue();
}
...
\*------------------------------------------------------------------------------*/
void
BmPopper::SetupAdditionalInfo(BMessage* additionalInfo)
{
	additionalInfo->AddString(
		BmNetEndpoint::MSG_CLIENT_CERT_NAME, mPopAccount->ClientCertificate().String());
	additionalInfo->AddString(BmNetEndpoint::MSG_SERVER_NAME, mPopAccount->Server().String());
	additionalInfo->AddString(
		BmNetEndpoint::MSG_ACCEPTED_CERT_ID, mPopAccount->AcceptedCertID().String());
}
...
\*------------------------------------------------------------------------------*/
void
BmPopper::UpdatePOPStatus(const float delta, const char* detailText, bool failed, bool stopped)
{
...
\*------------------------------------------------------------------------------*/
void
BmPopper::UpdateMailStatus(const float delta, const char* detailText, int32 currMsg)
{
	BmString text;
	if (mNewMsgCount) {
...
\*------------------------------------------------------------------------------*/
void
BmPopper::UpdateCleanupStatus(const float delta, int32 currMsg)
{
	BmString text;
...
\*------------------------------------------------------------------------------*/
void
BmPopper::UpdateProgress(uint32 numBytes)
{
...
\*------------------------------------------------------------------------------*/
void
BmPopper::StateConnect()
{
	BNetAddress addr;
	if (addr.SetTo(mPopAccount->Server().String(), mPopAccount->PortNr()) != B_OK) {
...
BmPopper::StateCapa()
{
	...
	try {
		...
		Regexx rx;
		if (rx.exec(mAnswerText, "^\\s*SASL\\s+(.*?)$", Regexx::newline)) {
			mSupportedAuthTypes = rx.match[0].atom[0];
		} else if (rx.exec(mAnswerText, "^\\s*AUTH\\s+(.*?)$", Regexx::newline)) {
			mSupportedAuthTypes = rx.match[0].atom[0];
		}
...
\*------------------------------------------------------------------------------*/
bool
BmPopper::StartEncryption(const char* encType)
{
...
\*------------------------------------------------------------------------------*/
void
BmPopper::ExtractBase64(const BmString& text, BmString& base64)
{
	...
	while (!pwdOK) {
		...
		if (authMethod == BmPopAccount::AUTH_APOP) {
			...
			if (mServerTimestamp.Length()) {
				...
				BM_THROW_RUNTIME(
					"Server did not supply a timestamp, "
...
BmPopper::StateCheck()
{
	...
	try {
		...
		int numLines
			= rx.exec(mAnswerText, "\\s*(\\d+)\\s+(.+?)\\s*$", Regexx::newline | Regexx::global);
		if (numLines < mMsgCount)
			throw BM_network_error(
				BmString("answer to UIDL has unknown format"
	...
	if (count != listAnswerVect.size()) {
		BM_LOG(BM_LogRecv, BmString("Strange: server indicated ")
							   << mMsgCount << " mails, but LIST received " << listAnswerVect.size()
							   << " lines!");
		if (count > listAnswerVect.size())
	...
	for (uint32 i = 0; i < count; i++) {
		...
		if (!mPopAccount->IsUIDDownloaded(mMsgUIDs[i])) {
			...
			if (msgNum != i + 1)
				throw BM_network_error(BmString("answer to LIST has unexpected msg-nr. ")
									   << msgNum << " in line " << i + 1);
			...
			BmString log;
			bool shouldBeRemoved = mPopAccount->ShouldUIDBeDeletedFromServer(mMsgUIDs[i], log);
...
BmPopper::StateRetrieve()
{
	...
	for (int32 i = 0; mNewMsgCount > 0 && i < mMsgCount; ++i) {
		...
		BmString log;
		bool shouldBeDeleted = mPopAccount->ShouldUIDBeDeletedFromServer(mMsgUIDs[i], log);
		BM_LOG2(BM_LogRecv, log);
		if (shouldBeDeleted) {
...
\*------------------------------------------------------------------------------*/
void
BmPopper::Quit(bool WaitForAnswer)
{
	BmString cmd("QUIT");
	try {
		SendCommand(cmd);
...
\*------------------------------------------------------------------------------*/
BmString
BmPopper::SuggestAuthType() const
{
