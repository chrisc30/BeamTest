...
\*------------------------------------------------------------------------------*/
void
MailMonitor::Run(dev_t mailboxDevice)
{
	mMailboxDevice = mailboxDevice;
	mShouldRun = true;
	// start new thread for worker:
	mThreadId = spawn_thread(MailMonitor::ThreadEntry, "BeamMonitor", B_NORMAL_PRIORITY, this);
	if (mThreadId >= 0)
...
\*------------------------------------------------------------------------------*/
int32
MailMonitor::ThreadEntry(void* data)
{
...
MailMonitor::StartQuery()
{
	...
	if (mNewMailQuery.SetVolume(&mboxVolume) == B_OK
		...
		&& mNewMailQuery.SetTarget(target) == B_OK && mNewMailQuery.Fetch() == B_OK) {
...
\*------------------------------------------------------------------------------*/
void
MailMonitor::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		case B_NODE_MONITOR:
		{
...
\*------------------------------------------------------------------------------*/
void
MailMonitor::AddMessage(BMessage* msg)
{
	if (mLocker.Lock()) {
...
\*------------------------------------------------------------------------------*/
void
MailMonitor::HandleQueryUpdateMsg(BMessage* msg)
{
	...
	switch (opcode) {
		case B_ENTRY_CREATED:
		{
			...
			if (msg->FindInt64("directory", &eref.directory) != B_OK
				...
				|| msg->FindInt64("node", &node) != B_OK || msg->FindString("name", &name) != B_OK)
				return;
			eref.set_name(name);
...
\*------------------------------------------------------------------------------*/
void
MailMonitor::HandleMailMonitorMsg(BMessage* msg)
{
	int32 opcode = msg->FindInt32("opcode");
	int64 node;
	switch (opcode) {
...
BmDeskbarView::BmDeskbarView(BRect frame)
	: BView(frame, BM_DeskbarItemName, B_FOLLOW_NONE, B_WILL_DRAW | B_PULSE_NEEDED),
	  mCurrIcon(NULL),
	  mNewMailCount(0),
...
\*------------------------------------------------------------------------------*/
BmDeskbarView*
BmDeskbarView::Instantiate(BMessage* data)
{
...
\*------------------------------------------------------------------------------*/
status_t
BmDeskbarView::Archive(BMessage* data, bool deep) const
{
...
\*------------------------------------------------------------------------------*/
void
BmDeskbarView::AttachedToWindow()
{
...
\*------------------------------------------------------------------------------*/
void
BmDeskbarView::DetachedFromWindow()
{
...
\*------------------------------------------------------------------------------*/
void
BmDeskbarView::Draw(BRect /*updateRect*/)
{
...
\*------------------------------------------------------------------------------*/
void
BmDeskbarView::ChangeIcon(const char* iconName)
{
...
\*------------------------------------------------------------------------------*/
void
BmDeskbarView::SendToBeam(BMessage* msg, BHandler* replyHandler)
{
...
\*------------------------------------------------------------------------------*/
void
BmDeskbarView::MouseDown(BPoint pos)
{
...
\*------------------------------------------------------------------------------*/
void
BmDeskbarView::ShowMenu(BPoint point)
{
	BMenuItem* item;
...
\*------------------------------------------------------------------------------*/
void
BmDeskbarView::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
...
\*------------------------------------------------------------------------------*/
bool
BmDeskbarView::LivesInMailbox(const entry_ref& inref)
{
	BEntry entry(&inref);
	entry_ref eref;
...
\*------------------------------------------------------------------------------*/
void
BmDeskbarView::AddRef(int64 node, const entry_ref& eref)
{
...
\*------------------------------------------------------------------------------*/
void
BmDeskbarView::RemoveRef(int64 node)
{
...
\*------------------------------------------------------------------------------*/
void
BmDeskbarView::UpdateRef(int64 node, const entry_ref& eref)
{
	NewMailMap::iterator iter = mNewMailMap.find(node);
	if (iter != mNewMailMap.end()) {
