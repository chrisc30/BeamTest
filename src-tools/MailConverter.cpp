...
public:
	...
	void SetTo(const BmString& msgText) { BmMail::SetTo(msgText, "dummy-account"); }
	void StoreAttributes(BFile& mailFile, const BmString& status, bigtime_t whenCreated)
	{
...
\*------------------------------------------------------------------------------*/
void
MailConverter(const char* folder)
{
	...
	while (dir.GetNextRef(&eref) == B_OK) {
		...
		if (sz != size) {
			fprintf(stderr,
				"unable to read %" B_PRIdOFF " bytes from file (only got %" B_PRIdOFF ")\n", size,
				sz);
			errorCount++;
			continue;
		}
...
main(int argc, char** argv)
{
	...
	if (argc != 2) {
		fprintf(stderr,
			"This program converts all files in a given path\n"
