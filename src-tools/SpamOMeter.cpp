...
struct ResInfo {
	...
	double corrSpam, corrTofu, reinfSpam, reinfTofu, falsePos, falseNeg, unsure, totalFalsePos,
		totalOverall;
};
...
\*------------------------------------------------------------------------------*/
status_t
FindTrainingMails(BmString& msgFileName)
{
	int32 count;
	status_t err;
	dirent* dent;
	...
	TrainCount = uint16(std::min(TrainingMailsSpam.size(), TrainingMailsTofu.size()));
...
\*------------------------------------------------------------------------------*/
void
SpamOMeter(const char* pathfileName, ResInfo& ri)
{
	status_t res;
	off_t size;
	off_t sz;
	...
	if (perfs) {
		...
		fprintf(stderr, "%s done (%d messages in %ld secs)\n", pathfileName, pvs,
			time(NULL) - starttime);
		fprintf(stderr,
			"\tsetup: reinforce=(S:%" B_PRId32 "/T:%" B_PRId32 ") unsure=(S:%" B_PRId32
			"/T:%" B_PRId32 ") dehtml=%s keep-atags=%s\n",
			ThresholdForSpam, ThresholdForTofu, UnsureForSpam, UnsureForTofu, DeHtml ? "yes" : "no",
			KeepATags ? "yes" : "no");
...
main(int argc, char** argv)
{
	...
	BmApplication* app = new BmApplication(APP_SIG, TrainingMode ? false : true);
	...
	if (argc - as > 1) {
		...
		fprintf(stderr,
			"\tsetup: reinf=(S:%" B_PRId32 "/T:%" B_PRId32 ") unsure=(S:%" B_PRId32 "/T:%" B_PRId32
			") dehtml=%s keep-atags=%s\n",
			ThresholdForSpam, ThresholdForTofu, UnsureForSpam, UnsureForTofu, DeHtml ? "yes" : "no",
			KeepATags ? "yes" : "no");
		fprintf(stderr, "\tsimple spam:      %6.2f   simple tofu:       %6.2f\n", ri.corrSpam,
			ri.corrTofu);
...
out:
	...
	delete[] resInfo;
	BFile resultFile(
		"/boot/var/tmp/bm_spamometer_results", B_READ_WRITE | B_ERASE_FILE | B_CREATE_FILE);
