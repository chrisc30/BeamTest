...
\*------------------------------------------------------------------------------*/
static void
EncodeBase64AndCheck(BmString input, BmString result)
{
	BmString encodedStr;
	int32 blockSize = 128;
	...
	BmBase64Encoder encoder(
		&srcBuf, blockSize, SingleLineMode ? BmBase64Encoder::nTagOnSingleLine : "");
...
Base64EncoderTest::SimpleTest()
{
	...
	// check 1 byte:
	NextSubTest();
	EncodeBase64AndCheck("a", "YQ==");
	// check 2 bytes:
	NextSubTest();
	EncodeBase64AndCheck("ab", "YWI=");
	// check 3 bytes:
	NextSubTest();
	EncodeBase64AndCheck("abc", "YWJj");
	// check 4 bytes:
	NextSubTest();
	EncodeBase64AndCheck("abcd", "YWJjZA==");

	// check normal encoding:
	NextSubTest();
	EncodeBase64AndCheck("Simple encoding äöüß", "U2ltcGxlIGVuY29kaW5nIMOkw7bDvMOf");
