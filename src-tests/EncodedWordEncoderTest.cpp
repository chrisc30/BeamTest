...
static void EncodeQpAndCheck(BmString input, BmString result, BmString srcCharset = "utf-8",
	int32 firstDiscardedPos = -1, bool hasError = false);
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
	...
	BmString input, BmString result, BmString srcCharset, int32 firstDiscardedPos, bool hasError)
{
	...
	try {
		...
		CPPUNIT_ASSERT(
			firstDiscardedPos != -1 || qpEncoder.FirstDiscardedPos() == firstDiscardedPos);
...
EncodedWordEncoderTest::SimpleTest()
{
	...
	NextSubTest();
	EncodeQpAndCheck("", "=?utf-8?q??=");
	// check what happens if nothing is to be encoded:
	NextSubTest();
	EncodeQpAndCheck("goof", "=?utf-8?q?goof?=");
	// check normal encoding (note that spaces become underlines):
	NextSubTest();
	EncodeQpAndCheck(
		"Simple encoding äöüß", "=?utf-8?q?Simple_encoding_=C3=A4=C3=B6=C3=BC=C3=9F?=");
	// underlines should be encoded:
	NextSubTest();
	EncodeQpAndCheck("Simple_encoding_", "=?utf-8?q?Simple=5Fencoding=5F?=");
	// equal-signs must be encoded:
	NextSubTest();
	EncodeQpAndCheck("Simple encoding =", "=?utf-8?q?Simple_encoding_=3D?=");
	// seemingly already qp, just the equal-signs should be encoded:
	NextSubTest();
	EncodeQpAndCheck("Simple encoding =C3=9F", "=?utf-8?q?Simple_encoding_=3DC3=3D9F?=");
	// check that chars unknown in src-charset are discarded properly:
	NextSubTest();
	EncodeQpAndCheck(
		"Simple encoding äöüß xxx", "=?us-ascii?q?Simple_encoding__xxx?=", "us-ascii", 17);
	// broken utf-8, bytes missing at end (whole string will be dropped):
	NextSubTest();
	EncodeQpAndCheck("text is broken \xFC", "", "utf-8", -1, true);
...
EncodedWordEncoderTest::MultiLineTest()
{
	...
	NextSubTest();
	EncodeQpAndCheck("1234567890123456789012345678901234567890123456789012345678901234",
		//
	...
	NextSubTest();
	EncodeQpAndCheck("12345678901234567890123456789012345678901234567890123456789012345",
		//
		"=?utf-8?q?123456789012345678901234567890123456789012345678901234567890"
		"1234?=\r\n"
		" =?utf-8?q?5?=");
	...
	NextSubTest();
	EncodeQpAndCheck("1234567890123456789012345678901234567890123456789012345678ä012345678",
		//
	...
	NextSubTest();
	EncodeQpAndCheck("12345678901234567890123456789012345678901234567890123456789ä12345678",
		//
		"=?utf-8?q?1234567890123456789012345678901234567890123456789012345678"
		"9?=\r\n"
	...
	// check encoding near wrapping border (doesn't fit on current line):
	NextSubTest();
	EncodeQpAndCheck("123456789012345678901234567890123456789012345678901234567890ä2345678",
		//
		"=?utf-8?q?1234567890123456789012345678901234567890123456789012345678"
		"90?=\r\n"
	...
	// check encoding near wrapping border (doesn't fit on current line):
	NextSubTest();
	EncodeQpAndCheck("1234567890123456789012345678901234567890123456789012345678901ä345678",
		//
		"=?utf-8?q?1234567890123456789012345678901234567890123456789012345678"
		"901?=\r\n"
		" =?utf-8?q?=C3=A4345678?=");
	// check encoding near wrapping border (doesn't fit on current line):
	NextSubTest();
	EncodeQpAndCheck("12345678901234567890123456789012345678901234567890123456789012ä45678",
		//
		"=?utf-8?q?1234567890123456789012345678901234567890123456789012345678"
		"9012?=\r\n"
		" =?utf-8?q?=C3=A445678?=");
	...
	// check encoding near wrapping border (fits on current line):
	NextSubTest();
	EncodeQpAndCheck("12345678901234567890123456789012345678901234567890123456ä89012345678",
		//
	...
	// check encoding near wrapping border (doesn't fit on current line):
	NextSubTest();
	EncodeQpAndCheck("123456789012345678901234567890123456789012345678901234567ä9012345678",
		//
		"=?iso-8859-1?q?12345678901234567890123456789012345678901234567890123"
		"4567?=\r\n"
