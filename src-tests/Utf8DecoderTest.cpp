...
static void DecodeUtf8AndCheck(BmString input, BmString result,
	BmString destCharset = DefaultCharset, int32 firstDiscardedPos = -1, bool hasError = false);
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
static void
DecodeUtf8AndCheck(
	BmString input, BmString result, BmString destCharset, int32 firstDiscardedPos, bool hasError)
{
	BmString decodedStr;
	int32 blockSize = 128;
	...
	try {
		...
		CPPUNIT_ASSERT(firstDiscardedPos != -1 || decoder.FirstDiscardedPos() == firstDiscardedPos);
...
Utf8DecoderTest::SimpleTest()
{
	...
	NextSubTest();
	DecodeUtf8AndCheck("äöüß", "\xe4\xf6\xfc\xdf");
	// check decoding of special characters which can't be found in dest-charset:
	NextSubTest();
	DecodeUtf8AndCheck("äöü - The €-sign is only contained in iso-8859-15",
		"\xe4\xf6\xfc - The -sign is only contained in iso-8859-15", "iso-8859-1", 14);
	// broken utf-8, bytes missing at end (should be dropped):
	NextSubTest();
	DecodeUtf8AndCheck("text is broken \xFC", "text is broken ", DefaultCharset, -1, true);
}
