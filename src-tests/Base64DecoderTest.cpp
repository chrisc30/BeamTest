...
\*------------------------------------------------------------------------------*/
static void
DecodeBase64AndCheck(BmString input, BmString result)
{
	BmString decodedStr;
	int32 blockSize = 128;
...
Base64DecoderTest::SimpleTest()
{
	...
	// check 1 byte:
	NextSubTest();
	DecodeBase64AndCheck("YQ==", "a");
	// check 2 bytes:
	NextSubTest();
	DecodeBase64AndCheck("YWI=", "ab");
	// check 3 bytes:
	NextSubTest();
	DecodeBase64AndCheck("YWJj", "abc");
	// check 4 bytes:
	NextSubTest();
	DecodeBase64AndCheck("YWJjZA==", "abcd");
	...
	NextSubTest();
	DecodeBase64AndCheck("U2ltcGxlIGVuY29kaW5nIMOkw7bDvMOf", "Simple encoding äöüß");

	// check that decoding skips whitespace:
	NextSubTest();
	DecodeBase64AndCheck("U2 l tcG xl IGVuY2\t\n\r9ka W5	nIMOkw7bDvMOf", "Simple encoding äöüß");

	// check that decoding skips illegal chars (not in base64-alphabet):
	NextSubTest();
	DecodeBase64AndCheck("U2ltcGxlIGVu!§$%&()Y29kaW5nIMOkw7bDvMOf", "Simple encoding äöüß");
	...
	NextSubTest();
	DecodeBase64AndCheck(
		"W1lhaG9vIRskQiVHJWolUCE8GyhCXQ==IBskQiJjMEIbKEI="
