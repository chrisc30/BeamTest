...
");
	CPPUNIT_ASSERT(!filter.CompileScript()
				   && filter.ErrorString().FindFirst("fileinto not required") != B_ERROR);
...
");
	CPPUNIT_ASSERT(!filter.CompileScript()
				   && filter.ErrorString().FindFirst("reject not required") != B_ERROR);
...
");
	...
	CPPUNIT_ASSERT(!filter.CompileScript()
				   && filter.ErrorString().FindFirst("notify not required") != B_ERROR);
...
");
	CPPUNIT_ASSERT(filter.CompileScript());
	CPPUNIT_ASSERT(filter.Execute(msgContext));
	CPPUNIT_ASSERT(Result() == RES_IDENTITY && targetIdentity == "test-identity");
...
");
	CPPUNIT_ASSERT(filter.CompileScript());
	CPPUNIT_ASSERT(filter.Execute(msgContext));
	CPPUNIT_ASSERT(Result() == RES_STATUS && targetStatus == "test-status");
...
SieveTest::AddressTestTest(void)
{
	...
	filter.Content(
		...
		{ keep; } else { discard; }");
	CPPUNIT_ASSERT(!filter.CompileScript()
				   && filter.ErrorString().FindFirst("subaddress not required") != B_ERROR);
...
SieveTest::HeaderTestTest(void)
{
	...
	filter.Content(
		...
		{ keep; } else { discard; }");
	CPPUNIT_ASSERT(!filter.CompileScript()
				   && filter.ErrorString().FindFirst("unknown comparator tag") != B_ERROR);
	...
	filter.Content(
		...
		{ keep; } else { discard; }");
	CPPUNIT_ASSERT(
		!filter.CompileScript() && filter.ErrorString().FindFirst("regex not required") != B_ERROR);
	// :regex comparison
	NextSubTest();
	filter.Content(
		"\
...
SieveTest::RelationalValueTestsTest(void)
{
	...
	filter.Content(
		...
		{ keep; } else { discard; }");
	CPPUNIT_ASSERT(!filter.CompileScript()
				   && filter.ErrorString().FindFirst("relational not required") != B_ERROR);
...
SieveTest::NumericRelationalValueTestsTest(void)
{
	...
	filter.Content(
		...
		{ keep; } else { discard; }");
	CPPUNIT_ASSERT(!filter.CompileScript()
				   && filter.ErrorString().FindFirst("i;ascii-numeric not required") != B_ERROR);
	// value eq
	NextSubTest();
	filter.Content(
		"\
