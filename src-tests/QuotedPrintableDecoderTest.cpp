...
\*------------------------------------------------------------------------------*/
static void
DecodeQpAndCheck(BmString input, BmString result)
{
	BmString decodedStr;
	int32 blockSize = 128;
	...
	BmQuotedPrintableDecoder qpDecoder(
		&srcBuf, blockSize, IsEncodedWord ? BmQuotedPrintableDecoder::nTagIsEncodedWord : "");
...
QuotedPrintableDecoderTest::SimpleTest()
{
	...
	NextSubTest();
	DecodeQpAndCheck("A sentence with no encoded chars.", "A sentence with no encoded chars.");
	// check normal decoding:
	NextSubTest();
	DecodeQpAndCheck("Simple decoding =C3=A4=C3=B6=C3=BC=C3=9F", "Simple decoding äöüß");
	// case-insensitivity:
	NextSubTest();
	DecodeQpAndCheck("Simple decoding =c3=A4=c3=B6=C3=bC=C3=9f", "Simple decoding äöüß");
	// underline in bodypart-mode should just be copied:
	NextSubTest();
	DecodeQpAndCheck("Simple_decoding_", "Simple_decoding_");
	{
		// underline in encoded-word-mode should be converted to space:
		Activator activate(IsEncodedWord);
		NextSubTest();
		DecodeQpAndCheck("Simple_decoding_", "Simple decoding ");
	}
	// unneccessary encoding:
	NextSubTest();
	DecodeQpAndCheck("Simple=20decoding=20=C3=A4=C3=B6=C3=BC=C3=9F", "Simple decoding äöüß");
	// broken encoding, non-hex after '=' (should be copied):
	NextSubTest();
	DecodeQpAndCheck("Simple decoding =xy=C3=B6=C3=BC=C3=9F", "Simple decoding =xyöüß");
	// broken encoding, whitespace after '=' (should be copied):
	NextSubTest();
	DecodeQpAndCheck("Simple decoding =C3=B6= =C3=BC=C3=9F", "Simple decoding ö= üß");
	// equal-sign massacre (should be copied):
	NextSubTest();
	DecodeQpAndCheck("Simple decoding ==========C3=9F", "Simple decoding =========ß");
	// broken encoding, chars missing at end (they should be copied):
	NextSubTest();
	DecodeQpAndCheck("Simple decoding =C3=B6=C3=BC=C", "Simple decoding öü=C");
}
...
QuotedPrintableDecoderTest::MultiLineTest()
{
	...
	NextSubTest();
	DecodeQpAndCheck("Ich war dabei mir eine Art von Verschwinden, =",
		//
		"Ich war dabei mir eine Art von Verschwinden, =");
	// faked folded line, no \r\n following:
	NextSubTest();
	DecodeQpAndCheck("Ich war dabei mir eine Art von Verschwinden, =\r",
		//
		"Ich war dabei mir eine Art von Verschwinden, =");
