...
public:
	...
	bool CheckSpecialCount()
	{
			...
			while (count-- > 0) {
				if (!(!strcmp(dent->d_name, ".") || !strcmp(dent->d_name, ".."))) {
					...
					if (S_ISREG(st.st_mode)) {
						...
						if (status == BM_MAIL_STATUS_NEW || status == BM_MAIL_STATUS_PENDING)
							specialCount++;
					}
				}
		...
		printf("<CheckSpecial: %ld>", inFolder->SpecialMailCount());
		fflush(stdout);
		if (!val)
	...
	bool CheckStoredRefList()
	{
		...
		printf("<CheckStored: %ld>", refList->size());
		fflush(stdout);
		if (!val) {
	...
	void Sync() { BAutolock lock(&mLock); }
...
MailMonitorTest::setUp()
{
	...
	inFolder = dynamic_cast<BmMailFolder*>(TheMailFolderList->FindItemByKey(BM_REFKEY(nref)).Get());
...
MailMonitorTest::suite()
{
	...
	suite->addTest(new CppUnit::TestCaller<MailMonitorTest>(
		"MailMonitorTest::BasicMailRefTest", &MailMonitorTest::BasicMailRefTest));

	// simple test for mail-folder-tracking:
	suite->addTest(new CppUnit::TestCaller<MailMonitorTest>(
		"MailMonitorTest::BasicMailFolderTest", &MailMonitorTest::BasicMailFolderTest));

	// massive mail-ref-tracking test with multiple threads:
	test = new MailMonitorTest;
	caller = new BThreadedTestCaller<MailMonitorTest>("MailMonitorTest::MassiveMailRefTest", test);
	...
	test = new MailMonitorTest;
	caller
		= new BThreadedTestCaller<MailMonitorTest>("MailMonitorTest::MassiveSpecialMailTest", test);
	...
	// and loaded:
	test = new MailMonitorTest;
	caller = new BThreadedTestCaller<MailMonitorTest>("MailMonitorTest::RefListStorageTest", test);
...
MailMonitorTest::BasicMailRefTest(void)
{
	...
	CPPUNIT_ASSERT(node.GetNodeRef(&nref) == B_OK);
	SyncWithMailMonitor();
	ref = dynamic_cast<BmMailRef*>(folder1List->FindItemByKey(BM_REFKEY(nref)).Get());
	...
	SyncWithMailMonitor();
	ref = dynamic_cast<BmMailRef*>(inList->FindItemByKey(BM_REFKEY(nref)).Get());
	CPPUNIT_ASSERT(ref != NULL);
