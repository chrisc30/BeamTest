...
StringTest::StringBeamExtensionsTest(void)
{
	...
	int64test << ll;
	CPPUNIT_ASSERT(strcmp(int64test.String(), "4000000000000000") == 0);

	NextSubTest();
	BmString crlf("this\r\n is a small\r test \nof linebreak-conversion\r\n");
	crlf.ConvertLinebreaksToLF();
	CPPUNIT_ASSERT(
		strcmp(crlf.String(), "this\n is a small\r test \nof linebreak-conversion\n") == 0);

	NextSubTest();
	BmString lf("this\n is a small\r test\r\n of linebreak-conversion\n");
	lf.ConvertLinebreaksToCRLF();
	CPPUNIT_ASSERT(
		strcmp(lf.String(), "this\r\n is a small\r test\r\n of linebreak-conversion\r\n") == 0);

	NextSubTest();
	BmString tabs("this\t is a small\r test of\t\ttabs-conversion\r\n");
	tabs.ConvertTabsToSpaces(4);
	CPPUNIT_ASSERT(
		strcmp(tabs.String(), "this     is a small\r test of        tabs-conversion\r\n") == 0);

	NextSubTest();
	BmString url("http://www.test.org?foo=%20%%%20ximul&bar=%Fa%xxdigit");
	url.DeUrlify();
	CPPUNIT_ASSERT(strcmp(url.String(), "http://www.test.org?foo= % ximul&bar=\xfa%xxdigit") == 0);
