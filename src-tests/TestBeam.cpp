...
\*------------------------------------------------------------------------------*/
void
SlurpFile(const char* filename, BmString& str)
{
	if (!FetchFile(filename, str))
		throw CppUnit::Exception((BmString("couldn't open file ") + filename).String());
}
...
\*------------------------------------------------------------------------------*/
BTestSuite*
CreateBmBaseTestSuite()
{
	BTestSuite* suite = new BTestSuite("BmBase");

	// ##### Add test suites here #####
	suite->addTest("BmBase::MemIo", MemIoTest::suite());
	//	suite->addTest("BmBase::MultiLocker",
	//						MultiLockerTest::suite());
	suite->addTest("BmBase::String", StringTest::suite());
	return suite;
}
...
\*------------------------------------------------------------------------------*/
BTestSuite*
CreateMailTrackerTestSuite()
{
	BTestSuite* suite = new BTestSuite("MailTracker");

	// ##### Add test suites here #####
	suite->addTest("MailTracker::MailMonitor", MailMonitorTest::suite());
	return suite;
}
...
\*------------------------------------------------------------------------------*/
BTestSuite*
CreateMailParserTestSuite()
{
	BTestSuite* suite = new BTestSuite("MailParser");

	// ##### Add test suites here #####
	suite->addTest("Encoding::Base64Decoder", Base64DecoderTest::suite());
	suite->addTest("Encoding::Base64Encoder", Base64EncoderTest::suite());
	suite->addTest("Encoding::BinaryDecoder", BinaryDecoderTest::suite());
	suite->addTest("Encoding::BinaryEncoder", BinaryEncoderTest::suite());
	suite->addTest("Encoding::EncodedWordEncoder", EncodedWordEncoderTest::suite());
	suite->addTest("Encoding::FoldedLineEncoder", FoldedLineEncoderTest::suite());
	suite->addTest("Encoding::LinebreakDecoder", LinebreakDecoderTest::suite());
	suite->addTest("Encoding::LinebreakEncoder", LinebreakEncoderTest::suite());
	suite->addTest("Encoding::QuotedPrintableDecoder", QuotedPrintableDecoderTest::suite());
	suite->addTest("Encoding::QuotedPrintableEncoder", QuotedPrintableEncoderTest::suite());
	suite->addTest("Encoding::Utf8Decoder", Utf8DecoderTest::suite());
	suite->addTest("Encoding::Utf8Encoder", Utf8EncoderTest::suite());
	return suite;
}
...
\*------------------------------------------------------------------------------*/
BTestSuite*
CreateFilterAddonsTestSuite()
{
	BTestSuite* suite = new BTestSuite("FilterAddons");

	// ##### Add test suites here #####
	suite->addTest("FilterAddons::Sieve", SieveTest::suite());
	return suite;
}
...
StartTests(void* args)
{
	try {
		...
		if (!access(testPath.String(), R_OK)) {
			...
			printf("cleaning up remains from last run...");
			fflush(stdout);
			system("rm -rf >/dev/null mail");
			printf("done\ncreating initial test setup...");
			fflush(stdout);
...
main(int argc, char** argv)
{
	...
	thread_id tid = spawn_thread(StartTests, "Beam_Test", B_NORMAL_PRIORITY, &argsInfo);
	resume_thread(tid);
