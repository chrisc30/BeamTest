...
\*------------------------------------------------------------------------------*/
static status_t
setNodeMonitorLimit(int newLimit)
{
	struct rlimit rl;
...
\*------------------------------------------------------------------------------*/
status_t
WatchNode(const node_ref* node, uint32 flags, BHandler* handler)
{
	...
	if (result == ENOMEM) {
		...
		if (lastCount == gNodeMonitorCount) {
			...
			BM_LOG(BM_LogApp, BmString("Failed to add monitor, trying to bump limit to ")
								  << gNodeMonitorCount << " nodes.");
			result = setNodeMonitorLimit(gNodeMonitorCount);
			if (result != B_OK) {
				BM_LOGERR(
					BmString("Failed to allocate more node monitors, error: ") << strerror(result));
				return result;
			}
		}
...
\*------------------------------------------------------------------------------*/
bool
MoveToTrash(const entry_ref* refs, int32 count)
{
...
\*------------------------------------------------------------------------------*/
bool
LivesInMailbox(const entry_ref& eref)
{
	node_ref nref;
...
\*------------------------------------------------------------------------------*/
bool
CheckMimeType(const entry_ref* eref, const char* type)
{
...
\*------------------------------------------------------------------------------*/
BmString
DetermineMimeType(const entry_ref* inref, bool doublecheck)
{
...
\*------------------------------------------------------------------------------*/
void
EnsureIndexExists(const char* attrName, int32 type)
{
	struct index_info idxInfo;
	if (fs_stat_index(ThePrefs->MailboxVolume.Device(), attrName, &idxInfo) != 0) {
		status_t res = fs_create_index(ThePrefs->MailboxVolume.Device(), attrName, type, 0);
		if (res == -1)
...
\*------------------------------------------------------------------------------*/
bool
FetchFile(BmString fileName, BmString& contents)
{
...
\*------------------------------------------------------------------------------*/
void
BmTempFileList::AddFile(BmString fileWithPath)
{
...
\*------------------------------------------------------------------------------*/
void
BmTempFileList::RemoveFile(BmString fileWithPath)
{
...
\*------------------------------------------------------------------------------*/
BmString
BmTempFileList::NextTempFilenameWithPath()
{
	BPath tempPath;
	status_t err;
...
\*------------------------------------------------------------------------------*/
bool
BmReadStringAttr(const BNode* node, const char* attrName, BmString& outStr)
{
	attr_info attrInfo;
	BmString tmpStr;
...
\*------------------------------------------------------------------------------*/
BmBackedFile::BmBackedFile(const char* filename, const char* mimetype, const BEntry* backupEntry)
{
...
\*------------------------------------------------------------------------------*/
BmBackedFile::BmBackedFile(const BEntry& entry, const char* mimetype, const BEntry* backupEntry)
{
...
\*------------------------------------------------------------------------------*/
status_t
BmBackedFile::SetTo(const char* filename, const char* mimetype, const BEntry* backupEntry)
{
...
\*------------------------------------------------------------------------------*/
status_t
BmBackedFile::SetTo(const BEntry& entry, const char* mimetype, const BEntry* backupEntry)
{
...
BmBackedFile::Init()
{
	...
	if (mBackupEntry.Exists()) {
		...
		if (mMimeType.Length()) {
			...
			if ((err = nodeInfo.SetTo(&node)) != B_OK)
				BM_THROW_RUNTIME(BmString("Could not set node-info for file\n\t<")
								 << mBackupName << ">\n\n Result: " << strerror(err));
			nodeInfo.SetType((mMimeType + nBackupExt).String());
		}
		// now rename the backup:
		if ((err = mBackupEntry.Rename(mBackupName.String(), true)) != B_OK) {
			BM_LOGERR(BmString("Could not rename file <")
					  << mFileName << "> to <" << mBackupName << ">\n\n Result: " << strerror(err));
			return err;
		}
	} else
		mBackupEntry.Unset();
	...
	if (err != B_OK) {
		BM_LOGERR(BmString("Could not create file\n\t<")
				  << mFileName << ">\n\n Result: " << strerror(err));
		return err;
	}
	return B_OK;
...
\*------------------------------------------------------------------------------*/
ssize_t
BmBackedFile::Write(const void* buffer, size_t size)
{
...
\*------------------------------------------------------------------------------*/
status_t
SetupFolder(const BmString& name, BDirectory* dir)
{
	...
	if (dir) {
		...
		if (res != B_OK) {
			if ((res = create_directory(name.String(), 0755)) != 0
				...
				BM_SHOWERR(BmString("Sorry, could not create folder ")
						   << name << ".\n\t Error:" << strerror(res));
			}
		}
	}
