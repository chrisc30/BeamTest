...
static BmString BmIdentificationFieldNames = "<Message-ID><In-Reply-To><References>";

static BmString BmNoEncodingFieldNames
	= "<Received><Message-ID><Resent-Message-ID><In-Reply-To><References><Date>"
	  "<Resent-Date>";

static BmString BmNoStrippingFieldNames = "<Received><Subject><UserAgent>";
...
\*------------------------------------------------------------------------------*/
bool
BmAddress::SetTo(const BmString& fullText)
{
	Regexx rx;
...
\*------------------------------------------------------------------------------*/
BmString
BmAddress::QuotedPhrase(const BmString& phrase)
{
...
\*------------------------------------------------------------------------------*/
bool
BmAddress::IsHandledByIdentity(BmIdentity* ident, bool needExactMatch) const
{
	if (!ident)
		return false;
...
\*------------------------------------------------------------------------------*/
void
BmAddress::ConstructRawText(BmString& header, const BmString& charset, int32 fieldNameLength) const
{
	BmString convertedAddrSpec
		= ConvertUTF8ToHeaderPart(mAddrSpec, charset, false, fieldNameLength);
	if (mPhrase.Length()) {
		BmString convertedPhrase
			= ConvertUTF8ToHeaderPart(QuotedPhrase(mPhrase), charset, true, fieldNameLength);
...
\*------------------------------------------------------------------------------*/
bool
BmAddressList::Set(BmString strippedFieldVal)
{
	mGroupName = "";
	mAddrList.clear();
...
\*------------------------------------------------------------------------------*/
bool
BmAddressList::Add(BmString strippedFieldVal)
{
	BmString addrText;
	Regexx rx;
	bool res = true;
	...
	if (rx.exec(strippedFieldVal, "^\\s*(.+?):\\s*(.+?)?;\\s*$")) {
		// it's a group list:
		mIsGroup = true;
		strippedFieldVal.CopyInto(
			mGroupName, rx.match[0].atom[0].start(), rx.match[0].atom[0].Length());
		if (rx.match[0].atom.size() > 1)
			strippedFieldVal.CopyInto(
				addrText, rx.match[0].atom[1].start(), rx.match[0].atom[1].Length());
	} else {
...
\*------------------------------------------------------------------------------*/
void
BmAddressList::Remove(BmString singleAddress)
{
...
\*------------------------------------------------------------------------------*/
const BmString&
BmAddressList ::FindAddressMatchingIdentity(BmIdentity* ident, bool needExactMatch) const
{
...
\*------------------------------------------------------------------------------*/
bool
BmAddressList::ContainsAddrSpec(const BmString addrSpec) const
{
	int32 count = (int32)mAddrList.size();
	for (int i = 0; i < count; ++i) {
...
\*------------------------------------------------------------------------------*/
BmStringList
BmAddressList::SplitIntoAddresses(BmString addrListText)
{
	mAddrString.Truncate(0);
...
\*------------------------------------------------------------------------------*/
const BmString&
BmAddressList::AddrString() const
{
	mAddrString.Truncate(0);
	if (mIsGroup) {
...
\*------------------------------------------------------------------------------*/
void
BmAddressList::ConstructRawText(
	BmStringOBuf& header, const BmString& charset, int32 fieldNameLength) const
{
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::BmHeaderList::Set(const BmString& fieldName, const BmString value)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::BmHeaderList::Add(const BmString& fieldName, const BmString value)
{
	BmValueList& valueList = mHeaders[fieldName];
	valueList.push_back(value);
}
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::BmHeaderList::Remove(const BmString& fieldName)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::BmHeaderList::RemoveFieldVal(const BmString& fieldName, const BmString& val)
{
	BmHeaderMap::iterator pos = mHeaders.find(fieldName);
	if (pos != mHeaders.end()) {
		BmValueList& valueList = pos->second;
		BmValueList::iterator valPos = find(valueList.begin(), valueList.end(), val);
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::BmHeaderList::GetAllValues(BmMsgContext& msgContext) const
{
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::BmHeaderList ::GetAllNames(vector<BmString>& fieldNamesVect) const
{
...
\*------------------------------------------------------------------------------*/
uint32
BmMailHeader::BmHeaderList::CountValuesFor(const BmString& fieldName) const
{
...
\*------------------------------------------------------------------------------*/
const BmString&
BmMailHeader::BmHeaderList ::ValueAt(const BmString& fieldName, uint32 idx) const
{
	BmHeaderMap::const_iterator iter = mHeaders.find(fieldName);
...
\*------------------------------------------------------------------------------*/
const BmString&
BmMailHeader::BmHeaderList ::operator[](const BmString& fieldName) const
{
...
#endif
	inline subpart(int32 p, int32 l)
		: pos(p),
		  len(l)
	{
	}
...
\*------------------------------------------------------------------------------*/
bool
BmMailHeader::IsAddressField(BmString fieldName)
{
...
\*------------------------------------------------------------------------------*/
bool
BmMailHeader::IsIdentificationField(BmString fieldName)
{
	BmString fname = BmString("<") << fieldName.CapitalizeEachWord() << ">";
...
\*------------------------------------------------------------------------------*/
bool
BmMailHeader::IsEncodingOkForField(BmString fieldName)
{
...
\*------------------------------------------------------------------------------*/
bool
BmMailHeader::IsStrippingOkForField(BmString fieldName)
{
...
\*------------------------------------------------------------------------------*/
bool
BmMailHeader::IsFieldEmpty(BmString fieldName)
{
	fieldName.CapitalizeEachWord();
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::GetAllFieldValues(BmMsgContext& msgContext) const
{
...
\*------------------------------------------------------------------------------*/
const BmString&
BmMailHeader::GetFieldVal(BmString fieldName, uint32 idx)
{
	fieldName.CapitalizeEachWord();
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::GetAllFieldNames(vector<BmString>& fieldNamesVect) const
{
...
\*------------------------------------------------------------------------------*/
uint32
BmMailHeader::CountFieldVals(BmString fieldName)
{
	fieldName.CapitalizeEachWord();
...
\*------------------------------------------------------------------------------*/
bool
BmMailHeader::AddressFieldContainsAddrSpec(BmString fieldName, const BmString addrSpec)
{
	fieldName.CapitalizeEachWord();
	if (!IsAddressField(fieldName))
		BM_THROW_RUNTIME(
...
\*------------------------------------------------------------------------------*/
bool
BmMailHeader::AddressFieldContainsAddress(BmString fieldName, const BmString& address)
{
	fieldName.CapitalizeEachWord();
	if (!IsAddressField(fieldName))
		BM_THROW_RUNTIME(
...
\*------------------------------------------------------------------------------*/
const BmAddressList&
BmMailHeader::GetAddressList(BmString fieldName)
{
	fieldName.CapitalizeEachWord();
	if (!IsAddressField(fieldName))
		BM_THROW_RUNTIME("BmMailHeader.GetAddressList(): Field is not an address-field.");
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::SetFieldVal(BmString fieldName, const BmString value)
{
	fieldName.CapitalizeEachWord();
	BmString strippedVal = IsStrippingOkForField(fieldName) ? StripField(value) : value;
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::AddFieldVal(BmString fieldName, const BmString value)
{
	fieldName.CapitalizeEachWord();
	BmString strippedVal = IsStrippingOkForField(fieldName) ? StripField(value) : value;
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::RemoveFieldVal(BmString fieldName, const BmString& value)
{
	fieldName.CapitalizeEachWord();
	BmString strippedVal = IsStrippingOkForField(fieldName) ? StripField(value) : value;
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::RemoveField(BmString fieldName)
{
	fieldName.CapitalizeEachWord();
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::RemoveAddrFieldVal(BmString fieldName, const BmString value)
{
	fieldName.CapitalizeEachWord();
	if (IsAddressField(fieldName))
...
\*------------------------------------------------------------------------------*/
BmAddressList
BmMailHeader::DetermineOriginator(bool bypassReplyTo)
{
...
\*------------------------------------------------------------------------------*/
BmAddressList
BmMailHeader::DetermineListAddress(bool bypassSanityTest)
{
	...
	if (!bypassSanityTest && listAddr.AddrCount() == 1) {
		...
		if (!(AddressFieldContainsAddrSpec(BM_FIELD_TO, firstAddr.AddrSpec())
				...
				|| AddressFieldContainsAddrSpec(BM_FIELD_RESENT_BCC, firstAddr.AddrSpec())
				|| AddressFieldContainsAddrSpec(BM_FIELD_RESENT_FROM, firstAddr.AddrSpec()))) {
...
\*------------------------------------------------------------------------------*/
BmString
BmMailHeader::DetermineReceivingAddrFor(
	const BmIdentityVect& identities, BmRef<BmIdentity>* identRefOut)
{
	BmString addr;
	...
	for (int i = 0; !addr.Length() && i < 2; ++i) {
		...
		for (iter = identities.begin(); iter != identities.end() && !addr.Length(); ++iter) {
			addr = mAddrMap[BM_FIELD_TO].FindAddressMatchingIdentity(iter->Get(), needExactMatch);
			if (!addr.Length()) {
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::PlugDefaultHeader(const BmMailHeader* defaultHeader)
{
	if (!defaultHeader)
		return;
	BmHeaderMap::const_iterator iter;
	for (iter = defaultHeader->mHeaders.begin(); iter != defaultHeader->mHeaders.end(); ++iter) {
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::UnplugDefaultHeader(const BmMailHeader* defaultHeader)
{
	if (!defaultHeader)
		return;
	BmHeaderMap::const_iterator iter;
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::AddParsingError(const BmString& errStr)
{
	if (errStr.Length()) {
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::ParseHeader(const BmString& header)
{
	Regexx rxUnfold, rx;
	...
	int32 pos = -1;
	int32 lastpos = 0;
	for (int32 offset = 0; (pos = header.FindFirst("\r\n", offset)) != B_ERROR; offset = pos + 2) {
	...
	if (!nm && mMail) {
		BM_LOGERR(BmString("Could not find any header-fields in this header: \n") << header);
	}
	...
	for (i = subparts.begin(); i != subparts.end(); ++i) {
		...
		if (pos == B_ERROR) {
			BmString errStr = BmString(
								  "Could not determine field-name of "
								  "mail-header-part:\n   ")
		...
		if (IsEncodingOkForField(fieldName)) {
			...
			AddFieldVal(fieldName,
				ConvertHeaderPartToUTF8(fieldBody,
					mMail ? mMail->DefaultCharset() : ThePrefs->GetString("DefaultCharset"),
					hadConversionError));
			if (hadConversionError) {
				BmString errStr = BmString("Autodetected charset of header-field '")
...
\*------------------------------------------------------------------------------*/
BmString
BmMailHeader::StripField(BmString fieldValue, BmString* commentBuffer)
{
	BmString stripped;
...
\*------------------------------------------------------------------------------*/
void
BmMailHeader::StoreAttributes(BFile& mailFile)
{
	//
	...
	//
	mailFile.WriteAttr(BM_MAIL_ATTR_SUBJECT, B_STRING_TYPE, 0, mHeaders[BM_FIELD_SUBJECT].String(),
		mHeaders[BM_FIELD_SUBJECT].Length() + 1);
	//
	mailFile.WriteAttr(BM_MAIL_ATTR_MIME, B_STRING_TYPE, 0, mHeaders[BM_FIELD_MIME].String(),
	...
	if (outbound && recipients.Length()) {
		...
		mailFile.WriteAttr(BM_MAIL_ATTR_RECIPIENTS, B_STRING_TYPE, 0, recipients.String(),
			recipients.Length() + 1);
	}
	...
	if (!priority.Length()) {
		...
		if (!prio.ICompare("Highest"))
			priority = "1";
		else if (!prio.ICompare("High"))
			priority = "2";
		else if (!prio.ICompare("Normal"))
			priority = "3";
		else if (!prio.ICompare("Low"))
			priority = "4";
		else if (!prio.ICompare("Lowest"))
			priority = "5";
	}
	if (!priority.Length()) {
	...
	mailFile.WriteAttr(
		BM_MAIL_ATTR_PRIORITY, B_STRING_TYPE, 0, priority.String(), priority.Length() + 1);
...
\*------------------------------------------------------------------------------*/
bool
BmMailHeader::ConstructRawText(BmStringOBuf& msgText, const BmString& charset)
{
	mParsingErrors.Truncate(0);
	...
	if (ThePrefs->GetBool("GenerateOwnMessageIDs")) {
		...
		if (accName.Length()) {
			BmSmtpAccount* account
				= dynamic_cast<BmSmtpAccount*>(TheSmtpAccountList->FindItemByKey(accName).Get());
			if (account)
		...
		SetFieldVal(mMail->IsRedirect() ? BM_FIELD_RESENT_MESSAGE_ID : BM_FIELD_MESSAGE_ID,
			BmString("<") << TimeToString(time(NULL), "%Y%m%d%H%M%S.") << find_thread(NULL) << "."
						  << ++nCounter << "@" << domain << ">");
	}
	...
	try {
		...
		if (mMail->IsRedirect()) {
			...
			for (iter = mHeaders.begin(); iter != mHeaders.end(); ++iter) {
				...
				BM_LOG2(BM_LogMailParse, BmString("ConstructRawText(): dealing with field ")
											 << fieldName);
				...
				if (IsAddressField(fieldName)) {
					...
					for (int i = 0; i < count; ++i) {
						headerIO << fieldName << ": "
								 << ConvertUTF8ToHeaderPart(
										valueList[i], charset, encodeIfNeeded, fieldName.Length())
								 << "\r\n";
					}
				}
			}
		...
		for (iter = mHeaders.begin(); iter != mHeaders.end(); ++iter) {
			fieldName = iter->first;
			BM_LOG2(
				BM_LogMailParse, BmString("ConstructRawText(): dealing with field ") << fieldName);
			if (fieldName.ICompare("Content-", 8) == 0) {
