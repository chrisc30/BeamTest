...
\*------------------------------------------------------------------------------*/
void
BmContentField::SetTo(const BmString cfString)
{
	Regexx rx;
	...
	if (rx.exec(cfString, "^\\s*([^\\s;]+)\\s*([;\\s].*)?\\s*$", Regexx::newline)) {
		...
		if (rx.match[0].atom.size() > 0) {
			if (cfString[rx.match[0].atom[0].start()] == '"') {
				...
				cfString.CopyInto(
					mValue, rx.match[0].atom[0].start() + 1, rx.match[0].atom[0].Length() - 2);
			} else {
		...
		if (rx.exec(params, ";?\\s*(\\w+)\\s*=\\s*((?:\\\"[^\"]+\\\"?)|(?:[^;\\s]+))",
			...
			for (uint32 i = 0; i < rx.match.size(); ++i) {
				...
				if (rx.match[0].atom.size() > 0) {
					...
					key.ToLower();
					mParams[key] = val;
					BM_LOG2(BM_LogMailParse, BmString("...found param: ")
												 << key << " with value: " << val);
				}
			}
		}
		...
		BM_LOG(
			BM_LogMailParse, BmString("field-value <") << cfString << "> has unknown structure!");
		return;
	}
	mInitCheck = B_OK;
}
...
\*------------------------------------------------------------------------------*/
const BmString&
BmContentField::Param(BmString key) const
{
...
\*------------------------------------------------------------------------------*/
void
BmContentField::SetParam(BmString key, BmString value)
{
	key.ToLower();
	mParams[key] = value;
}
...
/*------------------------------------------------------------------------------*\
	...
	-	c'tor
\*------------------------------------------------------------------------------*/
BmBodyPart::BmBodyPart(BmBodyPartList* model, const BmString& msgtext, int32 start, int32 length,
	const BmString& defaultCharset, BmRef<BmMailHeader> header, BmListModelItem* parent)
	  ...
	  mHadErrorDuringConversion(false)
{
	if (!mSuggestedCharset.Length())
		mCurrentCharset = mSuggestedCharset = ThePrefs->GetBool("ImportExportTextAsUtf8", true)
												  ? BmString("utf-8")
...
/*------------------------------------------------------------------------------*\
	...
	-	c'tor
\*------------------------------------------------------------------------------*/
BmBodyPart::BmBodyPart(BmBodyPartList* model, const entry_ref* ref, const BmString& defaultCharset,
	...
	try {
		...
		if (mimetype.ICompare("text/", 5) == 0
			...
			if (!IsCompatibleWithText(nativeString)) {
				BM_SHOWERR(
					"The attachment has been advertised as text, "
		...
		if (mimetype.ICompare("text/", 5) == 0)
			mContentTransferEncoding
				= NeedsQuotedPrintableEncoding(mDecodedData, BM_MAX_BODY_LINE_LEN)
					  ? ThePrefs->GetBool("Allow8BitMime", false) ? "8bit" : "quoted-printable"
					  : "7bit";
		else if (mimetype.ICompare("message/", 8) == 0)
			mContentTransferEncoding
				= NeedsQuotedPrintableEncoding(mDecodedData, BM_MAX_BODY_LINE_LEN)
					  ? ThePrefs->GetBool("Allow8BitMime", false) ? "8bit" : "7bit"
					  : "7bit";
		else
...
\*------------------------------------------------------------------------------*/
void
BmBodyPart::SetTo(const BmString& msgtext, int32 start, int32 length,
	const BmString& defaultCharset, BmRef<BmMailHeader> header)
{
		...
		if (!length) {
			...
			if (pos < end + 1 && msgtext[pos] == '\r' && msgtext[pos + 1] == '\n') {
				...
				if (pos < 0 || pos + 4 > end) {
					...
					BmString s = BmString(
									 "Couldn't determine borderline between "
									 "MIME-header and body in string <")
								 << str << ">.";
	...
	if (mIsMultiPart) {
		...
		if (!startPos) {
			BmString errStr = BmString("Boundary <") << boundary << "> not found within message.";
			BM_LOG(BM_LogMailParse, errStr);
			AddParsingError(errStr);
			return;
		}
		...
		while (!isLastBoundary) {
			while (1) {
				...
				if (*(nPos + foundBoundaryLen) == '-' && *(nPos + foundBoundaryLen + 1) == '-')
					foundBoundaryLen += 2;
				// skip any space and tabs:
				while (*(nPos + foundBoundaryLen) == ' ' || *(nPos + foundBoundaryLen) == '\t')
					foundBoundaryLen++;
				// now skip over \r\n if present:
			...
			if (nPos) {
				...
				BmBodyPart* subPart = new BmBodyPart((BmBodyPartList*)ListModel().Get(), msgtext,
					startOffs, len, defaultCharset, NULL, this);
				...
				if (start + length > startOffs) {
					...
					if (nlPos != B_ERROR && nlPos < start + length) {
						BM_LOG2(
							BM_LogMailParse, "Subpart of multipart found will be added to array");
						BmBodyPart* subPart
							= new BmBodyPart((BmBodyPartList*)ListModel().Get(), msgtext, startOffs,
								start + length - startOffs, defaultCharset, NULL, this);
						BmAutolockCheckGlobal lock(ListModel()->ModelLocker());
						if (!lock.IsLocked())
							BM_THROW_RUNTIME("BmBodyPart::SetTo(): Unable to get lock");
						AddSubItem(subPart);
...
\*------------------------------------------------------------------------------*/
void
BmBodyPart::AddParsingError(const BmString& errStr) const
{
	if (errStr.Length()) {
...
\*------------------------------------------------------------------------------*/
bool
BmBodyPart::ShouldBeShownInline() const
{
...
\*------------------------------------------------------------------------------*/
void
BmBodyPart::DecodeText(const char* tryCharset)
{
	if (tryCharset)
	...
	if (count > 0) {
		BmString sigStr(
			mDecodedData.String() + rx.match[count - 1].start() + rx.match[count - 1].Length());
		...
		if (listModel) {
			...
			if (bodyPartList && (mail = bodyPartList->Mail()) != NULL) {
				if (IsText()) {
					...
					// charset into utf8:
					BM_LOG2(BM_LogMailParse, BmString("(re-)converting bodytext of ")
												 << mBodyLength << " bytes...");
					...
					for (uint32 i = 0; i < charsetVect.size(); ++i) {
						...
						if (!mHadErrorDuringConversion || i == charsetVect.size() - 1) {
							if (i > 0) {
								AddParsingError(BmString("Autodetected charset (")
					...
					BmMemFilterRef decoder = FindDecoderFor(&text, mContentTransferEncoding);
					BmStringOBuf tempIO(mBodyLength, 1.2f);
					BM_LOG2(BM_LogMailParse, BmString("decoding bodytext of ")
												 << mBodyLength << " bytes...");
...
\*------------------------------------------------------------------------------*/
bool
BmBodyPart::ContainsRef(const entry_ref& ref) const
{
	BmModelItemMap::const_iterator iter;
...
\*------------------------------------------------------------------------------*/
void
BmBodyPart::WriteToFile(BFile& file)
{
...
\*------------------------------------------------------------------------------*/
entry_ref
BmBodyPart::WriteToTempFile(BmString filename)
{
	BPath tempPath;
	entry_ref eref;
	if (!filename.Length()) {
	...
	if (find_directory(B_SYSTEM_TEMP_DIRECTORY, &tempPath, true) == B_OK) {
		...
		if ((err = tempFile.SetTo(
				 &tempDir, filename.String(), B_WRITE_ONLY | B_CREATE_FILE | B_ERASE_FILE))
			!= B_OK) {
...
\*------------------------------------------------------------------------------*/
void
BmBodyPart::SaveAs(const entry_ref& destDirRef, BmString filename)
{
	BDirectory destDir;
	status_t err;
	if ((err = destDir.SetTo(&destDirRef)) == B_OK) {
		...
		if ((err = destFile.SetTo(
				 &destDir, filename.String(), B_WRITE_ONLY | B_CREATE_FILE | B_ERASE_FILE))
			!= B_OK) {
...
\*------------------------------------------------------------------------------*/
void
BmBodyPart::SetBodyText(const BmString& utf8Text, const BmString& charset)
{
	...
	mContentTransferEncoding
		= needsQP ? (ThePrefs->GetBool("Allow8BitMime", false) ? "8bit" : "quoted-printable")
				  : "7bit";
}
...
\*------------------------------------------------------------------------------*/
void
BmBodyPart::PropagateHigherEncoding()
{
	if (!IsMultiPart())
		return;
...
\*------------------------------------------------------------------------------*/
int32
BmBodyPart::PruneUnneededMultiParts()
{
	int32 count = 0;
	BmModelItemMap::const_iterator iter;
...
\*------------------------------------------------------------------------------*/
int32
BmBodyPart::EstimateEncodedSize()
{
...
\*------------------------------------------------------------------------------*/
void
BmBodyPart::ConstructBodyForSending(BmStringOBuf& msgText)
{
	BmString boundary;
	if (IsMultiPart()) {
	...
	bool haveEncodedText
		= bodyPartList && mail && mStartInRawText && bodyPartList->EditableTextBody() != this;
	...
	if (IsText()) {
		if (!haveEncodedText) {
			...
			BM_LOG2(BM_LogMailParse, BmString("encoding/converting bodytext of ")
										 << DecodedLength() << " bytes to " << mCurrentCharset);
			BmCharsetVect charsetVect;
					...
					if (i + 1 == charsetVect.size()) {
						...
						BmString errText
							= (bodyPartList->EditableTextBody() == this)
								  ...
								  : BmString("The attachment ")
										<< FileName() << " contains characters that "
										<< "could not be converted to the selected charset ("
										<< mCurrentCharset << ").\n\n"
										<< "Please re-add the attachment with the correct "
										<< "charset.";
						throw BM_text_error(errText, "", textConverter.FirstDiscardedPos());
					}
				} else {
	...
	if (IsMultiPart())
		...
		if (haveEncodedText) {
			...
			BM_LOG2(
				BM_LogMailParse, BmString("copying bodytext of ") << mBodyLength << " bytes...");
			...
			if (IsText()) {
				...
				// encode buffer:
				BmStringIBuf text(DecodedData());
				BM_LOG2(BM_LogMailParse, BmString("encoding bodytext of ")
											 << DecodedLength() << " bytes...");
...
\*------------------------------------------------------------------------------*/
bool
BmBodyPart::MimeTypeIsPotentiallyHarmful(const BmString& realMT)
{
...
BmBodyPartList::BmBodyPartList(BmMail* mail)
	: inherited(BmString("BodyPartList_") << mail->ModelName(), BM_LogMailParse),
	  mMail(mail),
...
BmBodyPartList::ParseMail()
{
	...
	if (mMail && mMail->HeaderLength() >= 2) {
		...
		BmBodyPart* bodyPart = new BmBodyPart(this, msgText, mMail->HeaderLength() + 2,
			MAX(msgText.Length() - mMail->HeaderLength() - 2, 0), mMail->DefaultCharset(),
			mMail->Header());
...
\*------------------------------------------------------------------------------*/
bool
BmBodyPartList::HasAttachments() const
{
...
\*------------------------------------------------------------------------------*/
void
BmBodyPartList::AddAttachmentFromRef(const entry_ref* ref, const BmString& charset)
{
	BmAutolockCheckGlobal lock(ModelLocker());
	if (!lock.IsLocked())
		BM_THROW_RUNTIME(ModelNameNC() << ": Unable to get lock");
...
\*------------------------------------------------------------------------------*/
void
BmBodyPartList::RemoveItemFromList(BmListModelItem* item)
{
	BmAutolockCheckGlobal lock(ModelLocker());
	if (!lock.IsLocked())
		BM_THROW_RUNTIME(ModelNameNC() << ": Unable to get lock");
...
\*------------------------------------------------------------------------------*/
void
BmBodyPartList::SetEditableText(const BmString& utf8Text, const BmString& charset)
{
	BmRef<BmBodyPart> editableTextBody(EditableTextBody());
	if (editableTextBody)
...
\*------------------------------------------------------------------------------*/
const BmString&
BmBodyPartList::DefaultCharset() const
{
	BmRef<BmBodyPart> editableTextBody(EditableTextBody());
	if (editableTextBody)
...
\*------------------------------------------------------------------------------*/
bool
BmBodyPartList::IsMultiPart() const
{
	BmAutolockCheckGlobal lock(ModelLocker());
	if (!lock.IsLocked())
		BM_THROW_RUNTIME(ModelNameNC() << ": Unable to get lock");
...
\*------------------------------------------------------------------------------*/
void
BmBodyPartList::PruneUnneededMultiParts()
{
	vector<BmRef<BmListModelItem> > removedRefVect;
	BmAutolockCheckGlobal lock(mModelLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME(ModelNameNC() << ":PruneUnnededMultiParts(): Unable to get lock");
	BmModelItemMap::const_iterator iter;
	for (iter = begin(); iter != end();) {
		BmBodyPart* bodyPart = dynamic_cast<BmBodyPart*>(iter++->second.Get());
		if (bodyPart->IsMultiPart()) {
...
\*------------------------------------------------------------------------------*/
int32
BmBodyPartList::EstimateEncodedSize()
{
	BmAutolockCheckGlobal lock(mModelLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME(ModelNameNC() << ":EstimateEncodedSize(): Unable to get lock");
	int32 size = 0;
	BmModelItemMap::const_iterator iter;
	for (iter = begin(); iter != end();) {
		BmBodyPart* bodyPart = dynamic_cast<BmBodyPart*>(iter++->second.Get());
...
\*------------------------------------------------------------------------------*/
bool
BmBodyPartList::ConstructBodyForSending(BmStringOBuf& msgText)
{
	BmAutolockCheckGlobal lock(mModelLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME(ModelNameNC() << ":ConstructBodyForSending(): Unable to get lock");
	BmRef<BmBodyPart> editableTextBody(EditableTextBody());
	bool hasMultiPartTop = editableTextBody && editableTextBody->Parent();
	bool isMultiPart = hasMultiPartTop ? editableTextBody->Parent()->size() > 1 : size() > 1;
	BmString boundary;
