...
public:
	...
	bool ShouldUIDBeDeletedFromServer(const BmString& uid, BmString& logOutput) const;
	...
	inline uint16 DeleteMailDelay() const { return mDeleteMailDelay; }
	inline const BmString& DeleteMailDelayString() const { return mDeleteMailDelayString; }
	...
	inline const BmString& Server() const { return mServer; }
	...
	{
		mDeleteMailDelay = i;
		mDeleteMailDelayString = BmString() << (uint32)i;
		TellModelItemUpdated(UPD_ALL);
	}
	inline void MarkedAsDefault(bool b)
	{
		mMarkedAsDefault = b;
		TellModelItemUpdated(UPD_ALL);
	}
