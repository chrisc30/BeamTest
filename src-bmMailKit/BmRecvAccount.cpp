...
\*------------------------------------------------------------------------------*/
status_t
BmRecvAccount::Archive(BMessage* archive, bool deep) const
{
...
\*------------------------------------------------------------------------------*/
void
BmRecvAccount::ExecuteAction(BMessage* action)
{
	BmString uid;
	switch (action->what) {
		case BM_APPEND_UID:
...
\*------------------------------------------------------------------------------*/
BmString
BmRecvAccount::GetDomainName() const
{
...
\*------------------------------------------------------------------------------*/
bool
BmRecvAccount::IsUIDDownloaded(const BmString& uid) const
{
...
\*------------------------------------------------------------------------------*/
void
BmRecvAccount::MarkUIDAsDownloaded(const BmString& uid)
{
...
\*------------------------------------------------------------------------------*/
bool
BmRecvAccount::ShouldUIDBeDeletedFromServer(const BmString& uid, BmString& logOutput) const
{
...
\*------------------------------------------------------------------------------*/
BmString
BmRecvAccount ::AdjustToCurrentServerUids(const vector<BmString>& serverUids)
{
...
\*------------------------------------------------------------------------------*/
void
BmRecvAccount::CheckInterval(int16 i)
{
	mCheckInterval = i;
...
\*------------------------------------------------------------------------------*/
void
BmRecvAccount::SetupIntervalRunner()
{
	delete mIntervalRunner;
	mIntervalRunner = NULL;
	BM_LOG(BM_LogRecv, BmString("RecvAccount ") << Type() << ":" << Key()
	...
	if (mCheckInterval > 0) {
		...
		mIntervalRunner = new BMessageRunner(
			be_app_messenger, &msg, static_cast<bigtime_t>(mCheckInterval) * 60 * 1000 * 1000, -1);
...
\*------------------------------------------------------------------------------*/
bool
BmRecvAccount::SanityCheck(BmString& complaint, BmString& fieldName) const
{
	...
	if (!mServer.Length()) {
		complaint = BmString("Please enter the address of this account's ") << Type() << "-Server.";
		fieldName = "popserver";
		return false;
	}
	if (mPortNr <= 0) {
	...
	if (mCheckInterval > 0 && !mPwdStoredOnDisk) {
		complaint
			= "In order to check this account for mail automatically,\n"
...
\*------------------------------------------------------------------------------*/
const BmString
BmRecvAccountList::SettingsFileName()
{
	BmString name = BmString(BeamRoster->SettingsPath()) << "/" << "Receiving Accounts";
	// this file used to be called "Pop Accounts", so we automatically
	// rename, if only the old name exists:
	BEntry entry(name.String());
	if (!entry.Exists()) {
		BmString oldName = BmString(BeamRoster->SettingsPath()) << "/" << "Pop Accounts";
...
\*------------------------------------------------------------------------------*/
void
BmRecvAccountList::InstantiateItem(BMessage* archive)
{
	int16 version = 0;
	...
	BM_LOG3(BM_LogMailTracking, BmString("RecvAccount <") << newAcc->Type() << "," << newAcc->Name()
														  << "," << newAcc->Key() << "> read");
...
\*------------------------------------------------------------------------------*/
void
BmRecvAccountList::ResetToSaved()
{
	BM_LOG2(BM_LogMailTracking, BmString("Start of ResetToSaved() for RecvAccountList"));
	...
	BM_LOG2(BM_LogMailTracking, BmString("End of ResetToSaved() for RecvAccountList"));
}
...
\*------------------------------------------------------------------------------*/
void
BmRecvAccountList::ForeignKeyChanged(
...
\*------------------------------------------------------------------------------*/
void
BmRecvAccountList::CheckMail(bool allAccounts)
{
	BmAutolockCheckGlobal lock(ModelLocker());
	if (!lock.IsLocked())
		BM_THROW_RUNTIME(ModelNameNC() << ": Unable to get lock");
...
\*------------------------------------------------------------------------------*/
void
BmRecvAccountList::CheckMailFor(BmString accName, bool isAutoCheck)
{
