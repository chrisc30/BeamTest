...
public:
	...
	BmBodyPart(BmBodyPartList* model, const BmString& msgtext, int32 s, int32 l,
		const BmString& defaultCharset, BmRef<BmMailHeader> mHeader = NULL,
		BmListModelItem* parent = NULL);
	BmBodyPart(BmBodyPartList* model, const entry_ref* ref, const BmString& defaultCharset,
		BmListModelItem* parent = NULL);
	...
	// native methods:
	void SetTo(const BmString& msgtext, int32 s, int32 l, const BmString& defaultCharset,
	...
	{
		return mContentDisposition.Param(key);
	}
	...
	inline bool HadParsingErrors() const { return mParsingErrors.Length() > 0; }
...
public:
	...
	inline BmRef<BmBodyPart> EditableTextBody() const { return mEditableTextBody; }
