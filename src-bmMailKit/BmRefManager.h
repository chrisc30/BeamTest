...
public:
	...
	static BmRefObj* FetchObject(
		const char* objListName, const BmString& objName, BmRefObj* ptr = NULL);
...
public:
	...
	bool operator==(const BmRef<T>& ref) const { return mPtr == ref.mPtr; }
...
public:
	...
	{
		LogHelper(BmString("RefManager: weak-reference to <")
				  << mName << ":" << BmRefObj::RefPrintHex(mPtr) << "> created");
	}
	...
	{
		return (mPtr == ref.mPtr) && mName == ref.mName && mObjectListName == ref.mObjectListName;
	}
	inline bool operator!=(const T* p) const
	{
	...
	inline bool operator<(const BmWeakRef<T>& ref) const { return (mPtr < ref.mPtr); }
	inline operator bool() const { return Get(); }
	inline BmRef<T> Get() const
	{
		LogHelper(BmString("RefManager: weak-reference to <")
				  << mName << ":" << BmRefObj::RefPrintHex(mPtr) << "> dereferenced");
		BAutolock lock(BmRefObj::GlobalLocker());
		return dynamic_cast<T*>(BmRefObj::FetchObject(mObjectListName, mName, mPtr));
	}
