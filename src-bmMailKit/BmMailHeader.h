...
public:
	...
	{
		return mPhrase == a.Phrase() && mAddrSpec == a.AddrSpec();
	}

	// native methods:
	bool SetTo(const BmString& addrText);
	void ConstructRawText(BmString& header, const BmString& charset, int32 fieldNameLength) const;
	bool IsHandledByIdentity(BmIdentity* ident, bool needExactMatch = false) const;
...
public:
	...
	// native methods:
	inline bool SetTo(BmString strippedFieldVal) { return Set(strippedFieldVal); }
	...
	void ConstructRawText(
		BmStringOBuf& header, const BmString& charset, int32 fieldNameLength) const;
	const BmString& FindAddressMatchingIdentity(
		BmIdentity* ident, bool needExactMatch = false) const;
	...
	{
		return mAddrList.size() > 0 ? mAddrList[0] : BmAddress();
	}
...
public:
	...
	bool AddressFieldContainsAddress(BmString fieldName, const BmString& address);
	//
	BmString DetermineSender();
	BmString DetermineReceivingAddrFor(
		const BmIdentityVect& identities, BmRef<BmIdentity>* identRefOut = NULL);
	...
	inline const bool HasParsingErrors() const { return mParsingErrors.Length() > 0; }
	inline const BmString& ParsingErrors() const { return mParsingErrors; }
