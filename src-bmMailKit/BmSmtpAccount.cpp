...
\*------------------------------------------------------------------------------*/
status_t
BmSmtpAccount::Archive(BMessage* archive, bool deep) const
{
...
\*------------------------------------------------------------------------------*/
bool
BmSmtpAccount::NeedsAuthViaPopServer()
{
...
\*------------------------------------------------------------------------------*/
bool
BmSmtpAccount::SanityCheck(BmString& complaint, BmString& fieldName) const
{
	...
	if ((mAuthMethod != AUTH_NONE && mAuthMethod != AUTH_SMTP_AFTER_POP) && !mUsername.Length()) {
...
\*------------------------------------------------------------------------------*/
void
BmSmtpAccount::SendMail(const entry_ref& eref)
{
...
\*------------------------------------------------------------------------------*/
void
BmSmtpAccount::SendPendingMails()
{
...
\*------------------------------------------------------------------------------*/
void
BmSmtpAccount::GetSupportedAuthTypes(vector<BmString>& outList) const
{
...
\*------------------------------------------------------------------------------*/
const BmString
BmSmtpAccountList::SettingsFileName()
{
	BmString name = BmString(BeamRoster->SettingsPath()) << "/" << "Sending Accounts";
	// this file used to be called "Smtp Accounts", so we automatically
	// rename, if only the old name exists:
	BEntry entry(name.String());
	if (!entry.Exists()) {
		BmString oldName = BmString(BeamRoster->SettingsPath()) << "/" << "Smtp Accounts";
...
\*------------------------------------------------------------------------------*/
void
BmSmtpAccountList::InstantiateItem(BMessage* archive)
{
	BmSmtpAccount* newAcc = new BmSmtpAccount(archive, this);
	BM_LOG3(BM_LogMailTracking, BmString("SmtpAccount <")
...
\*------------------------------------------------------------------------------*/
void
BmSmtpAccountList::SendPendingMails()
{
	...
	for (iter = begin(); iter != end(); ++iter) {
		BmSmtpAccount* smtpAcc = dynamic_cast<BmSmtpAccount*>(iter->second.Get());
		if (smtpAcc)
...
\*------------------------------------------------------------------------------*/
void
BmSmtpAccountList::SendPendingMailsFor(const BmString accName)
{
