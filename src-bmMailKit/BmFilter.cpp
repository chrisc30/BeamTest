...
BmFilter::SetupAddonPart()
{
	if (mKind.Length()) {
		...
		if (instFunc && (mAddon = (*instFunc)(Key(), &mAddonArchive, mKind)) != NULL) {
			BM_LOG2(BM_LogFilter, BmString("Instantiated Filter-Addon <")
									  << mKind << ":" << Key() << ">");
			mAddon->Initialize();
		} else
			BM_LOG2(BM_LogFilter, BmString("Unable to instantiate Filter-Addon <")
									  << mKind << ":" << Key()
									  << ">. This filter will be disabled.");
	} else
		BM_LOG2(BM_LogFilter, BmString("Unable to instantiate Filter-Addon <")
								  << Key()
...
\*------------------------------------------------------------------------------*/
status_t
BmFilter::Archive(BMessage* archive, bool deep) const
{
	status_t ret
...
\*------------------------------------------------------------------------------*/
bool
BmFilter::SanityCheck(BmString& complaint, BmString& fieldName) const
{
	if (mAddon)
...
\*------------------------------------------------------------------------------*/
bool
BmFilter::Execute(BmMsgContext* msgContext)
{
	if (!mAddon)
		return false;
...
\*------------------------------------------------------------------------------*/
void
BmFilterList::ForeignKeyChanged(const BmString& key, const BmString& oldVal, const BmString& newVal)
{
...
BmFilterList::LoadAddons()
{
	...
	while (addonDir.GetNextEntry(&entry, true) == B_OK) {
		if (entry.IsFile()) {
			...
			if ((err = get_image_symbol(ao.image, "InstantiateFilter", B_SYMBOL_TYPE_ANY,
			...
			if ((err = get_image_symbol(ao.image, "InstantiateFilterPrefs", B_SYMBOL_TYPE_ANY,
			...
			if ((err = get_image_symbol(
					 ao.image, "FilterKinds", B_SYMBOL_TYPE_ANY, (void**)&filterKinds))
				!= B_OK) {
				BM_SHOWERR(BmString("Unable to load filter-addon\n\t")
			...
			if ((err = get_image_symbol(
					 ao.image, "DefaultFilterName", B_SYMBOL_TYPE_ANY, (void**)&defaultFilterName))
				== B_OK)
			...
			while (*filterKinds) {
				...
				if (kind.ICompare("Spam") == 0) {
					...
					mLearnAsSpamFilter = new BmFilter(LEARN_AS_SPAM_NAME, "Spam", NULL);
					BMessage learnAsSpamJob;
					learnAsSpamJob.AddString("jobSpecifier", "LearnAsSpam");
					mLearnAsSpamFilter->JobSpecifier(learnAsSpamJob);
					mLearnAsTofuFilter = new BmFilter(LEARN_AS_TOFU_NAME, "Spam", NULL);
...
\*------------------------------------------------------------------------------*/
BmString
BmFilterList::DefaultNameForFilterKind(const BmString& filterKind)
{
...
\*------------------------------------------------------------------------------*/
bool
BmFilterList::HaveSpamFilter() const
{
...
\*------------------------------------------------------------------------------*/
void
BmFilterList::InstantiateItem(BMessage* archive)
{
