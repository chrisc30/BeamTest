...
protected:
	void CopyMailParts(BmRef<BmMail>& newMail, BmRef<BmMail>& oldMail, bool withAttachments,
		bool isForward, const BmString& intro, const BmString& selectedText = BM_DEFAULT_STRING);
	void ExpandIntroMacros(BmRef<BmMail> mail, BmString& intro, bool usePersonalPhrase);
	...
	static int32 QuoteTextWithReWrap(
		const BmString& in, BmString& out, BmString quoteString, int maxLineLen);
	static int32 AddQuotedText(const BmString& text, BmString& out, const BmString& quote,
...
private:
	...
	BmRef<BmMail> CreateReplyTo(
		BmRef<BmMail>& oldMail, const BmString& selectedText, bool demandNonPersonal = false);
...
public:
	BmForwardFactory(
		BmForwardMode forwardMode, bool join, const BmString& selectedText = BM_DEFAULT_STRING);
	~BmForwardFactory();
...
private:
	...
	BmRef<BmMail> CreateInlineForward(
		BmRef<BmMail>& oldMail, bool withAttachments, const BmString& selectedText);
