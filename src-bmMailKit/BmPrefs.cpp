...
BmPrefs::CreateInstance()
{
	...
	prefsFilename = BmString(BeamRoster->SettingsPath()) << "/" << PREFS_FILENAME;
	if ((err = prefsFile.SetTo(prefsFilename.String(), B_READ_ONLY)) == B_OK) {
		...
		try {
			...
			if (!buf)
				BM_THROW_RUNTIME(BmString("Could not alloc attribute buffer for prefs-file\n\t<")
								 << prefsFilename << ">\n");
			...
			while (prefsFile.GetNextAttrName(name) == B_OK) {
				...
				if (err != B_OK)
					BM_THROW_RUNTIME(BmString("Could not fetch setting <")
									 << name << "> from file\n\t<" << prefsFilename
									 << ">\n\n Result: " << strerror(err));
				count++;
			}
	  ...
	  mLocker("PrefsLock")
{
	...
	if (version < 4) {
		...
		mPrefsMsg.AddString("DefaultForwardType", mDefaultsMsg.FindString("DefaultForwardType"));
	}
	if (version < 5) {
	...
	if (version < 7) {
		...
		const char* ll[] = {"Loglevel_Pop", "Loglevel_JobWin", "Loglevel_MailParse", "Loglevel_App",
			"Loglevel_MailTracking", "Loglevel_Gui", "Loglevel_ModelController", "Loglevel_Smtp",
			"Loglevel_Filter", "Loglevel_RefCount", "Loglevel_Util", "Loglevel_FolderView",
			"Loglevel_RefView", "Loglevel_MailEditWin", NULL};
		for (int i = 0; ll[i]; ++i) {
	...
	if (version < 14) {
		...
		mShortcutsMsg.AddString(
			"Send All Pending Messages", mShortcutsMsg.FindString("Send Pending Messages"));
		mShortcutsMsg.RemoveName("Send Pending Messages");
		mShortcutsMsg.AddString("Reply (Auto)", mShortcutsMsg.FindString("Reply"));
...
\*------------------------------------------------------------------------------*/
void
BmPrefs::InitDefaults(BMessage& defaultsMsg)
{
	defaultsMsg.MakeEmpty();
	defaultsMsg.AddInt16(MSG_VERSION, nPrefsVersion);
	int32 loglevels = BM_LOGLVL1(BM_LogRecv) + BM_LOGLVL0(BM_LogJobWin)
					  + BM_LOGLVL0(BM_LogMailParse) + BM_LOGLVL1(BM_LogApp)
					  + BM_LOGLVL0(BM_LogMailTracking) + BM_LOGLVL0(BM_LogGui)
					  + BM_LOGLVL0(BM_LogModelController) + BM_LOGLVL1(BM_LogSmtp)
					  + BM_LOGLVL1(BM_LogFilter) + BM_LOGLVL0(BM_LogRefCount);
	defaultsMsg.AddInt32("Loglevel_Recv", BM_LOGLVL_FOR(loglevels, BM_LogRecv));
	defaultsMsg.AddInt32("Loglevel_JobWin", BM_LOGLVL_FOR(loglevels, BM_LogJobWin));
	defaultsMsg.AddInt32("Loglevel_MailParse", BM_LOGLVL_FOR(loglevels, BM_LogMailParse));
	defaultsMsg.AddInt32("Loglevel_App", BM_LOGLVL_FOR(loglevels, BM_LogApp));
	defaultsMsg.AddInt32("Loglevel_MailTracking", BM_LOGLVL_FOR(loglevels, BM_LogMailTracking));
	defaultsMsg.AddInt32("Loglevel_Gui", BM_LOGLVL_FOR(loglevels, BM_LogGui));
	defaultsMsg.AddInt32(
		"Loglevel_ModelController", BM_LOGLVL_FOR(loglevels, BM_LogModelController));
	defaultsMsg.AddInt32("Loglevel_Smtp", BM_LOGLVL_FOR(loglevels, BM_LogSmtp));
	defaultsMsg.AddInt32("Loglevel_Filter", BM_LOGLVL_FOR(loglevels, BM_LogFilter));
	defaultsMsg.AddInt32("Loglevel_RefCount", BM_LOGLVL_FOR(loglevels, BM_LogRefCount));
	...
	defaultsMsg.AddString("AutoCharsetsInbound", "us-ascii,utf-8,default");
	defaultsMsg.AddString("AutoCharsetsOutbound", "us-ascii,default,iso-2022-jp,utf-8");
	...
	defaultsMsg.AddString("DefaultCharset", BmEncoding::DefaultCharset.String());
	...
	defaultsMsg.AddString("ForwardSubjectRX", "^\\s*\\[?\\s*Fwd(\\[\\d+\\])?:");
	...
	defaultsMsg.AddString("HeaderListLarge", "Subject,From,Date,To,Cc,User-Agent/X-Mailer");
	...
	defaultsMsg.AddString(
		"MimeTypeTrustInfo", "application/pdf:T,application/zip:T,application:W,:T");
	...
	defaultsMsg.AddString("QuotingLevelRX", "^((?:\\w?\\w?\\w?[>|]|[ \\t]*)*)(.*?)$");
...
\*------------------------------------------------------------------------------*/
BmString
BmPrefs::GetShortcutFor(const char* _shortcutID)
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefs::SetShortcutIfNew(BMessage* msg, const char* name, const BmString val)
{
	type_code tc;
...
\*------------------------------------------------------------------------------*/
void
BmPrefs::SetShortcutFor(const char* name, const BmString val)
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("Prefs: Unable to get lock!");
...
\*------------------------------------------------------------------------------*/
BMessage*
BmPrefs::GetShortcutDefaults(BMessage* shortcutsMsg)
{
	if (!shortcutsMsg)
...
\*------------------------------------------------------------------------------*/
uint32
BmPrefs::GetNumericLogLevelFor(uint32 terrain)
{
	uint32 level = 0;
...
\*------------------------------------------------------------------------------*/
const char*
BmPrefs::GetLogLevelFor(uint32 terrain)
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefs::SetLogLevelForTo(uint32 terrain, BmString loglevel)
{
	int32 level;
...
BmPrefs::SetLoglevels()
{
	...
	TheLogHandler->LogLevels(
		loglevels, GetInt("MinLogfileSize", 50 * 1024), GetInt("MaxLogfileSize", 200 * 1024));
	...
	BM_LOG3(BM_LogApp, BmString("Initialized log levels to binary value ") << s);
}
...
BmPrefs::SetupMailboxVolume()
{
	...
	if (entry.SetTo(mailboxPath.String(), true) != B_OK)
		BM_THROW_RUNTIME(
			BmString("Sorry, could not get entry for mailbox <") << mailboxPath << "> !");
	node_ref nref;
...
BmPrefs::Store()
{
	...
	try {
		BmString prefsFilename = BmString(BeamRoster->SettingsPath()) << "/" << PREFS_FILENAME;
		...
		uint32 type;
		int32 count;
		for (int32 i = 0; mPrefsMsg.GetInfo(B_ANY_TYPE, i, &name, &type, &count) == B_OK; ++i) {
			for (int32 j = 0; j < count; ++j) {
				...
				if (err < B_OK)
					BM_THROW_RUNTIME(BmString("Could not store setting <")
									 << name << "> into file\n\t<" << prefsFilename
									 << ">\n\n Result: " << strerror(err));
			}
		}
...
\*------------------------------------------------------------------------------*/
BmString
BmPrefs::GetString(const char* name)
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("Prefs: Unable to get lock!");
...
\*------------------------------------------------------------------------------*/
BmString
BmPrefs::GetString(const char* name, const BmString defaultVal)
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("Prefs: Unable to get lock!");
...
\*------------------------------------------------------------------------------*/
bool
BmPrefs::GetBool(const char* name)
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("Prefs: Unable to get lock!");
...
\*------------------------------------------------------------------------------*/
bool
BmPrefs::GetBool(const char* name, const bool defaultVal)
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("Prefs: Unable to get lock!");
...
\*------------------------------------------------------------------------------*/
int32
BmPrefs::GetInt(const char* name)
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("Prefs: Unable to get lock!");
...
\*------------------------------------------------------------------------------*/
int32
BmPrefs::GetInt(const char* name, const int32 defaultVal)
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("Prefs: Unable to get lock!");
...
\*------------------------------------------------------------------------------*/
BMessage*
BmPrefs::GetMsg(const char* name)
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("Prefs: Unable to get lock!");
...
\*------------------------------------------------------------------------------*/
void
BmPrefs::SetBool(const char* name, const bool val)
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("Prefs: Unable to get lock!");
...
\*------------------------------------------------------------------------------*/
void
BmPrefs::SetInt(const char* name, const int32 val)
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("Prefs: Unable to get lock!");
...
\*------------------------------------------------------------------------------*/
void
BmPrefs::SetMsg(const char* name, const BMessage* val)
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("Prefs: Unable to get lock!");
...
\*------------------------------------------------------------------------------*/
void
BmPrefs::SetString(const char* name, const BmString val)
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("Prefs: Unable to get lock!");
