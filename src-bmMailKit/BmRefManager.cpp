...
\*------------------------------------------------------------------------------*/
BmObjectList*
BmObjectList::GetObjectList(const char* const objListName)
{
...
\*------------------------------------------------------------------------------*/
BmRefObj*
BmObjectList::FetchObject(const BmString& key, BmRefObj* ptr)
{
...
BmObjectList::CleanupObjectLists()
{
	BAutolock lock(BmRefObj::GlobalLocker());
	if (!lock.IsLocked())
		throw BM_runtime_error("CleanupObjectLists(): Could not acquire global lock!");
...
BmRefObj::AddRef()
{
	...
	if (lastCount == 0) {
		objList->ObjectMap.insert(std::pair<const BmString, BmRefObj*>(RefName(), this));
	}
#ifdef BM_REF_DEBUGGING
	// check again to ensure no-one has clobbered with ref-count...
	BM_ASSERT(mRefCount > 0 && mRefCount == lastCount + 1);
	BM_LOG2(BM_LogRefCount, BmString("RefManager: reference to <")
								<< typeid(*this).name() << ":" << RefName() << ":" << RefPrintHex()
								<< "> added, ref-count is " << mRefCount);
#else
	BM_LOG2(BM_LogRefCount, BmString("RefManager: reference to <")
								<< RefName() << ":" << RefPrintHex() << "> added, ref-count is "
								<< mRefCount);
#endif
}
...
\*------------------------------------------------------------------------------*/
void
BmRefObj::RenameRef(const char* newName)
{
	BAutolock lock(GlobalLocker());
	if (!lock.IsLocked())
...
#else
	BM_LOG2(BM_LogRefCount, BmString("RefManager: reference to <")
								<< RefName() << ":" << RefPrintHex() << "> renamed to " << newName);
#endif
	// find object...
	...
	{  // scope for lock
		...
		if (lastCount == 1) {
			...
			if (pos != objList->ObjectMap.end())
				objList->ObjectMap.erase(pos);
#ifdef BM_REF_DEBUGGING
			BM_LOG(BM_LogRefCount, BmString("RefManager: ... object <")
									   << typeid(*this).name() << ":" << RefName() << ":"
			...
			mRefCount = -1;
#else
			BM_LOG2(BM_LogRefCount, BmString("RefManager: ... object <")
										<< RefName() << ":" << RefPrintHex()
										<< "> will be deleted");
#endif
			needsDelete = true;
...
\*------------------------------------------------------------------------------*/
BmRefObj*
BmRefObj::FetchObject(const char* objListName, const BmString& objName, BmRefObj* ptr)
{
...
BmRefObj::PrintRefsLeft()
{
	...
	try {
		...
		BmObjectList::BmObjectListMap::const_iterator end = BmObjectList::nObjectListMap.end();
		for (iter = BmObjectList::nObjectListMap.begin(); iter != end; ++iter) {
			...
			for (iter2 = objList->ObjectMap.begin(); iter2 != end2; ++iter2, ++count) {
				...
				BM_LOG(BM_LogRefCount, BmString("\t<")
										   << typeid(*ref).name() << " " << ref->RefName() << ":"
										   << ref->RefPrintHex() << "> alive, ref-count is "
										   << ref->mRefCount);
			}
		}
		BM_LOG(BM_LogRefCount, BmString("--------------------\n(")
...
\*------------------------------------------------------------------------------*/
BmString
BmRefObj::RefPrintHex(const void* ptr)
{
	char buf[20];
	sprintf(buf, "%p", ptr);
	return buf;
...
\*------------------------------------------------------------------------------*/
bool
BmAutolockCheckGlobal::IsLocked()
{
	return (mLocker && mLocker->IsLocked()) || (mLooper && mLooper->IsLocked());
}
...
\*------------------------------------------------------------------------------*/
void
BmAutolockCheckGlobal::Init()
{
	if (BmRefObj::GlobalLocker()->IsLocked()) {
		DEBUGGER(
			"GlobalLocker must not be locked when using "
			"BmAutolockCheckGlobal!");
		mLocker = NULL;
		mLooper = NULL;
	} else {
