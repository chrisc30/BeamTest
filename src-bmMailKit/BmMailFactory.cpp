...
\*------------------------------------------------------------------------------*/
void
BmMailFactory::AddBaseMail(BmMail* mail)
{
	if (mail)
...
\*------------------------------------------------------------------------------*/
void
BmMailFactory::AddBaseMailRef(BmMailRef* ref)
{
	if (ref)
...
\*------------------------------------------------------------------------------*/
void
BmMailFactory::CopyMailParts(BmRef<BmMail>& newMail, BmRef<BmMail>& oldMail, bool withAttachments,
	bool isForward, const BmString& intro, const BmString& selectedText)
{
	...
	BmString quotedText;
	int32 newLineLen
		= QuoteText((selectedText.Length() || !textBody) ? selectedText : textBody->DecodedData(),
			quotedText, ThePrefs->GetString("QuotingString"), ThePrefs->GetInt("MaxLineLen"));
	if (newTextBody) {
		newBody->SetEditableText(
			newTextBody->DecodedData() + "\n" + intro + "\n" + quotedText, charset);
	} else
	...
	if (withAttachments && oldBody->HasAttachments()) {
		...
		if (!lock.IsLocked())
			BM_THROW_RUNTIME(oldBody->ModelNameNC() << ": Unable to get lock");
		...
		bool doNotAttachVCards = ThePrefs->GetBool("DoNotAttachVCardsToForward");
		for (; iter != end; ++iter) {
...
void
BmMailFactory::ExpandIntroMacros(BmRef<BmMail> mail, BmString& intro, bool usePersonalPhrase)
{
	BmMailRef* mailRef = mail->MailRef();
	Regexx rx;
	intro = rx.replace(intro, "%D", TimeToString(mailRef->When(), "%Y-%m-%d"),
		Regexx::nocase | Regexx::global | Regexx::noatom);
	intro = rx.replace(intro, "%T", TimeToString(mailRef->When(), "%X [%z]"),
		Regexx::nocase | Regexx::global | Regexx::noatom);
	intro = rx.replace(intro, "\\n", "\n", Regexx::nocase | Regexx::global | Regexx::noatom);
	intro = rx.replace(
		intro, "%S", mailRef->Subject(), Regexx::nocase | Regexx::global | Regexx::noatom);
...
\*------------------------------------------------------------------------------*/
int32
BmMailFactory::QuoteText(
	const BmString& in, BmString& out, const BmString inQuoteString, int maxLineLen)
{
	out = "";
	if (!in.Length())
		return maxLineLen;
	...
	BmString emptyLinesAtEndRX = BmString("(?:") << "\\Q" << quoteString << "\\E" << "(" << "\\Q"
												 << quote << "\\E" << ")?[ \\t]*\\n)+\\z";
...
\*------------------------------------------------------------------------------*/
int32
BmMailFactory::QuoteTextWithReWrap(
	const BmString& in, BmString& out, BmString quoteString, int maxLineLen)
{
	out = "";
	if (!in.Length())
		return maxLineLen;
	...
	// cache a few preference values
	int32 spacesPerTab = ThePrefs->GetInt("SpacesPerTab", 4);
	BmString quotingLevelEmptyLineRX = ThePrefs->GetString("QuotingLevelEmptyLineRX", "^[ \\t]*$");
	...
	for (int32 i = 0; i < count; ++i) {
		...
		if ((line.CountChars() < minLenForWrappedLine && lastWasSpecialLine)
			|| rxl.exec(line, quotingLevelEmptyLineRX) || rxl.exec(line, quotingLevelListLineRX)) {
			if (i != 0) {
...
\*------------------------------------------------------------------------------*/
int32
BmMailFactory::AddQuotedText(const BmString& inText, BmString& out, const BmString& quote,
	const BmString& quoteString, int maxTextLen)
{
...
BmReplyFactory::BmReplyFactory(
	BmReplyMode replyMode, bool join, bool joinIntoOne, const BmString& selectedText)
...
struct LessTimewise {
	bool operator()(const BmRef<BmMailRef>& left, const BmRef<BmMailRef>& right)
	{
	...
	if (mJoin) {
		...
		for (BmMailRefVect::const_iterator iter = mBaseRefVect.begin();
			...
			if (mail) {
				...
				BmString index = mJoinIntoOne ? BmString("single") : replyAddrSpec;
				...
				if (!newMail) {
					...
					bool usePersonalPhrase = demandNonPersonal ? false : IsReplyToPersonOnly(mail);
		...
		for (BmMailRefVect::const_iterator iter = mBaseRefVect.begin();
			...
			if (mail) {
				...
				newMail = CreateReplyTo(
					mail, iter == mBaseRefVect.begin() ? mSelectedText : BM_DEFAULT_STRING);
				if (newMail)
...
\*------------------------------------------------------------------------------*/
BmString
BmReplyFactory::DetermineReplyAddress(BmRef<BmMail>& mail)
{
...
\*------------------------------------------------------------------------------*/
bool
BmReplyFactory::IsReplyToPersonOnly(BmRef<BmMail>& mail)
{
...
\*------------------------------------------------------------------------------*/
BmString
BmReplyFactory::CreateReplySubjectFor(const BmString& subject)
{
	BmString isReplyRX = ThePrefs->GetString("ReplySubjectRX");
	Regexx rx;
	if (!rx.exec(subject, isReplyRX, Regexx::nocase | Regexx::nomatch)) {
		BmString subjectStr = ThePrefs->GetString("ReplySubjectStr");
		subjectStr = rx.replace(
			subjectStr, "%s", subject, Regexx::nocase | Regexx::global | Regexx::noatom);
		return subjectStr;
	}
	return subject;
}
...
\*------------------------------------------------------------------------------*/
BmString
BmReplyFactory::CreateReplyIntro(BmRef<BmMail>& mail, bool usePersonalPhrase)
{
...
\*------------------------------------------------------------------------------*/
BmRef<BmMail>
BmReplyFactory::CreateReplyTo(
	BmRef<BmMail>& oldMail, const BmString& selectedText, bool demandNonPersonal)
{
	...
	const BmAddressList& toAddrs = oldMail->Header()->GetAddressList(BM_FIELD_TO);
	const BmAddressList& ccAddrs = oldMail->Header()->GetAddressList(BM_FIELD_CC);
	if (mReplyMode == BM_REPLY_MODE_SMART) {
	...
	bool usePersonalPhrase = demandNonPersonal ? false : IsReplyToPersonOnly(oldMail);
...
BmForwardFactory::BmForwardFactory(
	BmForwardMode forwardMode, bool join, const BmString& selectedText)
			...
			if (mail) {
				...
				if (mForwardMode == BM_FORWARD_MODE_ATTACHED) {
					if (iter == mBaseRefVect.begin())
						...
						newMail->AddAttachmentFromRef(
							mailRef->EntryRefPtr(), mail->DefaultCharset());
				} else {
					if (iter == mBaseRefVect.begin())
						newMail = CreateInlineForward(
							mail, mForwardMode == BM_FORWARD_MODE_INLINE_ATTACH, mSelectedText);
					else {
						BmString intro = CreateForwardIntro(mail);
						CopyMailParts(newMail, mail, mForwardMode == BM_FORWARD_MODE_INLINE_ATTACH,
							BM_IS_FORWARD, intro);
					}
				}
				if (iter == mBaseRefVect.begin()) {
		...
		for (BmMailRefVect::const_iterator iter = mBaseRefVect.begin();
			...
			if (mail) {
				...
				if (mForwardMode == BM_FORWARD_MODE_ATTACHED) {
					...
					newMail
						= CreateInlineForward(mail, mForwardMode == BM_FORWARD_MODE_INLINE_ATTACH,
...
\*------------------------------------------------------------------------------*/
BmRef<BmMail>
BmForwardFactory::CreateInlineForward(
	BmRef<BmMail>& mail, bool withAttachments, const BmString& selectedText)
{
	...
	BmString intro = CreateForwardIntro(mail);
	CopyMailParts(newMail, mail, withAttachments, BM_IS_FORWARD, intro, selectedText);
...
\*------------------------------------------------------------------------------*/
BmRef<BmMail>
BmForwardFactory::CreateAttachedForward(BmRef<BmMail>& mail)
{
	BmRef<BmMail> newMail = new BmMail(true);
	if (mail->MailRef()->InitCheck() == B_OK)
		newMail->Body()->AddAttachmentFromRef(
			mail->MailRef()->EntryRefPtr(), mail->DefaultCharset());
	// massage subject, if neccessary:
	BmString subject = mail->GetFieldVal(BM_FIELD_SUBJECT);
	newMail->SetFieldVal(BM_FIELD_SUBJECT, CreateForwardSubjectFor(subject));
...
\*------------------------------------------------------------------------------*/
BmString
BmForwardFactory::CreateForwardSubjectFor(const BmString& subject)
{
	BmString isForwardRX
		= ThePrefs->GetString("ForwardSubjectRX", "^\\s*\\[?\\s*Fwd(\\[\\d+\\])?:");
	Regexx rx;
...
\*------------------------------------------------------------------------------*/
BmString
BmForwardFactory::CreateForwardIntro(BmRef<BmMail>& mail)
{
...
\*------------------------------------------------------------------------------*/
BmRef<BmMail>
BmRedirectFactory::CreateRedirect(BmRef<BmMail>& mail)
{
	BmRef<BmMail> newMail = new BmMail(true);
	...
	newMail->SetFieldVal(
		BM_FIELD_RESENT_DATE, TimeToString(time(NULL), "%a, %d %b %Y %H:%M:%S %z"));
...
\*------------------------------------------------------------------------------*/
BmRef<BmMail>
BmCopyMailFactory::CreateCopy(BmRef<BmMail>& oldMail)
{
	BmRef<BmMail> newMail = new BmMail(true);
	...
	BmRef<BmListModelItem> identRef = TheIdentityList->FindItemByKey(oldMail->IdentityName());
