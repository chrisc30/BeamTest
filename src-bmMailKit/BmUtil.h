...
IMPEXPBMMAILKIT BMessage* FindMsgMsg(
	BMessage* archive, const char* name, BMessage* msg = NULL, int32 index = 0);
...
\*------------------------------------------------------------------------------*/
IMPEXPBMMAILKIT inline bool
IS_PART_OF_UTF8_MULTICHAR(unsigned char c)
{
	return c & 0x80;
}
IMPEXPBMMAILKIT inline bool
IS_UTF8_STARTCHAR(unsigned char c)
{
	return (c & 0xc0) == 0xc0;
}
IMPEXPBMMAILKIT inline bool
IS_WITHIN_UTF8_MULTICHAR(unsigned char c)
{
	return (c & 0xc0) == 0x80;
}
