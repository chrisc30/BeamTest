...
\*------------------------------------------------------------------------------*/
BmMailFolder*
BmMailFolderList::AddSpecialFlag(const node_ref& pnref, const node_ref& nref)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderList::RemoveSpecialFlag(const node_ref& pnref, const node_ref& nref)
{
#ifdef BM_REF_DEBUGGING
	BM_ASSERT(ModelLocker().IsLocked());
#endif
...
\*------------------------------------------------------------------------------*/
BmMailFolder*
BmMailFolderList::AddMailFolder(entry_ref& eref, int64 node, BmMailFolder* parent, time_t mtime)
{
...
\*------------------------------------------------------------------------------*/
BmRef<BmMailFolder>
BmMailFolderList ::FindMailFolderBySubPath(const BmString& subPath)
{
	if (!subPath.Length())
		return NULL;
	BmAutolockCheckGlobal lock(mModelLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME(ModelNameNC() << ":FindMailFolderBySubPath(): Unable to get lock");
	...
	while (remainder.Length() && currFolder) {
		...
		bool found = false;
		for (iter = currFolder->begin(); !found && iter != currFolder->end(); ++iter) {
			BmMailFolder* subFolder = dynamic_cast<BmMailFolder*>(iter->second.Get());
			if (subFolder) {
...
\*------------------------------------------------------------------------------*/
BmRef<BmMailRef>
BmMailFolderList::FindMailRefByKey(const node_ref& nref)
{
	if (mTopFolder) {
		...
		for (uint32 i = 0; i < collector.folderVect.size(); ++i) {
			BmRef<BmMailRefList> refList = collector.folderVect[i]->MailRefList();
			if (refList)
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderList::InitializeItems()
{
	BDirectory mailDir;
	BEntry entry;
	entry_ref eref;
	...
	{  // scope for autolocker
		BM_LOG3(BM_LogMailTracking, BmString("Top-folder <")
										<< eref.name << "," << nref.node << "> found");
		...
		BM_LOG(BM_LogMailTracking, BmString("End of initFolders (")
...
\*------------------------------------------------------------------------------*/
int
BmMailFolderList::InitializeSubFolders(BmMailFolder* folder, int level)
{
	BDirectory mailDir;
	entry_ref eref;
	dirent* dent;
		...
		while (count-- > 0) {
			...
			if (!(!strcmp(dent->d_name, ".") || !strcmp(dent->d_name, ".."))) {
				...
				if (S_ISDIR(st.st_mode)) {
					...
					BM_LOG3(BM_LogMailTracking, BmString("Mail-folder <")
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderList::InstantiateItems(BMessage* archive)
{
	status_t err;
	BM_LOG(BM_LogMailTracking, BmString("Starting to read folder-cache"));
	BMessage msg;
	if ((err = archive->FindMessage(BmListModelItem::MSG_CHILDREN, &msg)) != B_OK)
	...
	{  // scope for autolock
		...
		BM_LOG3(BM_LogMailTracking, BmString("Top-folder <") << mTopFolder->EntryRef().name << ","
															 << mTopFolder->Key() << "> read");
		...
		BM_LOG(BM_LogMailTracking, BmString("End of reading folder-cache (")
									   << folderCount << " folders found)");
		mInitCheck = B_OK;
	}
}
...
\*------------------------------------------------------------------------------*/
int
BmMailFolderList::InstantiateSubFolders(BmMailFolder* folder, BMessage* archive, int level)
{
	status_t err;
	...
	for (int i = 0; i < numChildren; ++i) {
		BMessage msg;
		if ((err = archive->FindMessage(BmMailFolder::MSG_CHILDREN, i, &msg)) != B_OK)
			BM_THROW_RUNTIME(BmString("Could not find mailfolder-child nr. ")
							 << i + 1 << " \n\nError:" << strerror(err));
		BmMailFolder* newFolder = new BmMailFolder(&msg, this, folder);
		if (!newFolder->Exists()) {
			BM_LOG(BM_LogMailTracking, BmString("Mail-folder <")
										   << newFolder->EntryRef().name << "," << newFolder->Key()
										   << "> doesn't exist anymore - skipped!");
			continue;
		}
		AddItemToList(newFolder, folder);
		folderCount++;
		BM_LOG3(BM_LogMailTracking, BmString("Mail-folder <") << newFolder->EntryRef().name << ","
															  << newFolder->Key() << "> read");
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderList::QueryForSpecialMails()
{
	...
	BmAutolockCheckGlobal lock(mModelLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME(ModelNameNC() << ":QueryForSpecialMails(): Unable to get lock");
	...
	if ((err = mSpecialMailQuery.SetPredicate("(MAIL:status = 'New') || (MAIL:status = 'Pending')"))
		!= B_OK)
		BM_THROW_RUNTIME(BmString("SetPredicate(): ") << strerror(err));
	if ((err = mSpecialMailQuery.SetTarget(BMessenger(TheMailMonitor))) != B_OK)
		BM_THROW_RUNTIME(BmString("QueryForSpecialMails(): could not set query target.\n\nError:")
						 << strerror(err));
	...
	for (iter = foldersWithSpecialMail.begin(); iter != end; ++iter) {
		if (*iter)
			TellModelItemUpdated(
				*iter, BmMailFolder::UPD_SPECIAL_COUNT | BmMailFolder::UPD_HAVE_SPECIAL_STATUS);
	}
	BM_LOG(BM_LogMailTracking, BmString("End of special-mail-query (")
								   << specialCount << " special mails found)");
}
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderList::RemoveController(BmController* controller)
{
...
\*------------------------------------------------------------------------------*/
const BmString
BmMailFolderList::SettingsFileName()
{
