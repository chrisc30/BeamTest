...
private:
	...
	void EntryCreated(BmMailFolder* parent, node_ref& nref, entry_ref& eref, struct stat& st);
	void EntryRemoved(BmMailFolder* parent, node_ref& nref);
	void EntryMoved(BmMailFolder* parent, node_ref& nref, entry_ref& eref, struct stat& st,
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitorWorker::Run()
{
	mShouldRun = true;
	// start new thread for worker:
	BmString tname("MailMonitorWorker");
	mThreadId
		= spawn_thread(BmMailMonitorWorker::ThreadEntry, tname.String(), B_NORMAL_PRIORITY, this);
	if (mThreadId < 0)
...
\*------------------------------------------------------------------------------*/
int32
BmMailMonitorWorker::ThreadEntry(void* data)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitorWorker::MessageLoop()
{
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitorWorker::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			case B_NODE_MONITOR:
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitorWorker::AddMessage(BMessage* msg)
{
	if (mLocker.Lock()) {
...
\*------------------------------------------------------------------------------*/
bool
BmMailMonitorWorker::IsIdle(uint32 msecs)
{
	bool res = false;
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitorWorker::HandleMailMonitorMsg(BMessage* msg)
{
			...
			case B_ENTRY_MOVED:
			{
				...
				if (opcode != B_ENTRY_REMOVED) {
					...
					if ((err = aNode.SetTo(&eref)) != B_OK) {
						BM_LOG(
							BM_LogMailTracking, BmString("Couldn't create node for parent-node <")
				...
				if (opcode == B_ENTRY_CREATED) {
					...
					if ((err = msg->FindInt64("from directory", &erefFrom.directory)) != B_OK)
						BM_THROW_RUNTIME("Field 'directory' not found in msg !?!");
					if ((err = msg->FindInt32("device", &erefFrom.device)) != B_OK)
						BM_THROW_RUNTIME("Field 'device' not found in msg !?!");
					if ((err = msg->FindString("name", &name)) != B_OK)
						BM_THROW_RUNTIME("Field 'name' not found in msg !?!");
					...
					parent = dynamic_cast<BmMailFolder*>(
						TheMailFolderList->FindItemByKey(BM_REFKEY(pnref)).Get());
					EntryMoved(parent.Get(), nref, eref, st, oldParent.Get(), erefFrom);
				}
				break;
			}
			case B_STAT_CHANGED:
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitorWorker::EntryCreated(
	BmMailFolder* parent, node_ref& nref, entry_ref& eref, struct stat& st)
{
	if (!parent)
		throw BM_runtime_error(
			BmString("Folder with inode <") << eref.directory << "> is unknown.");
	if (S_ISDIR(st.st_mode)) {
		// a new mail-folder has been created, we add
		// it to our list:
		BM_LOG2(BM_LogMailTracking, BmString("New mail folder <")
										<< eref.name << "," << nref.node << "> detected.");
		TheMailFolderList->AddMailFolder(eref, nref.node, parent, st.st_mtime);
	} else {
		// a new mail has been created, we add it to the
		// parent folder:
		BM_LOG2(BM_LogMailTracking, BmString("New mail <")
										<< eref.name << "," << nref.node << "> detected.");
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitorWorker::EntryRemoved(BmMailFolder* parent, node_ref& nref)
{
	BmRef<BmMailFolder> folder;
	...
	if (folder) {
		// a folder has been deleted, we remove it from our list:
		BM_LOG2(
			BM_LogMailTracking, BmString("Removal of mail folder <") << nref.node << "> detected.");
		BmAutolockCheckGlobal lock(TheMailFolderList->ModelLocker());
		if (!lock.IsLocked())
			BM_THROW_RUNTIME("MailMonitor::EntryRemoved(): Unable to get lock");
		// adjust special-mail-count accordingly...
		int32 specialMailCount
			= (int32)(folder->SpecialMailCount() + folder->SpecialMailCountForSubfolders());
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitorWorker::EntryMoved(BmMailFolder* parent, node_ref& nref, entry_ref& eref,
	struct stat& st, BmMailFolder* oldParent, entry_ref& erefFrom)
{
	...
	if (last_parent == parent && last_oldParent == oldParent && last_eref == eref
		&& last_erefFrom == erefFrom) {
		BM_LOG2(BM_LogMailTracking, BmString("Second move-event of mail/folder <")
										<< eref.name << "," << nref.node << "> dropped.");
		return;
	}
	last_parent = parent;
	...
	if (S_ISDIR(st.st_mode)) {
		...
		BmRef<BmMailFolder> folder;
		folder
			= dynamic_cast<BmMailFolder*>(TheMailFolderList->FindItemByKey(BM_REFKEY(nref)).Get());
		if (erefFrom.directory == eref.directory) {
			// rename only, we take the short path:
			BM_LOG2(BM_LogMailTracking, BmString("Rename of mail folder <")
											<< eref.name << "," << nref.node << "> detected.");
			...
			if (!folder) {
				...
				BM_LOG2(BM_LogMailTracking,
					BmString("Move of mail folder <")
						<< eref.name << "," << nref.node << "> detected.\nFrom: "
						<< (oldParent ? oldParent->Key() : BmString("<outside>"))
						<< " to: " << (parent ? parent->Key() : BmString("<outside>")));
			} else {
		...
		BM_LOG2(BM_LogMailTracking, BmString("Move of mail <")
										<< eref.name << "," << nref.node << "> detected.");
		if (oldParent)
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitorWorker::EntryChanged(node_ref& nref)
{
	BM_LOG2(
		BM_LogMailTracking, BmString("Change of item with node <") << nref.node << "> detected...");
	...
	if (pos != mCachedRefToFolderMap.end()) {
		...
		BmRef<BmListModelItem> folderItem = TheMailFolderList->FindItemByKey(pos->second.folderKey);
		BmRef<BmMailFolder> folder = dynamic_cast<BmMailFolder*>(folderItem.Get());
		if (folder)
		...
		if (ref) {
			BM_LOG2(BM_LogMailTracking, "...corresponding ref was found in loaded ref-lists.");
			ref->ResyncFromDisk();
		} else {
			BM_LOG2(BM_LogMailTracking, "...corresponding ref wasn't found in loaded ref-lists.");
		}
	}
}
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitorWorker::CacheRefToFolder(node_ref& nref, const BmString& fKey)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitorWorker::HandleQueryUpdateMsg(BMessage* msg)
{
	int32 opcode = msg->FindInt32("opcode");
	status_t err;
	node_ref pnref;
		...
		switch (opcode) {
			...
			case B_ENTRY_REMOVED:
			{
				...
				{  // scope for lock
					BmAutolockCheckGlobal lock(TheMailFolderList->ModelLocker());
					if (!lock.IsLocked())
						BM_THROW_RUNTIME("HandleMailMonitorMsg(): Unable to get lock");
					if (opcode == B_ENTRY_CREATED)
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitor::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		case B_NODE_MONITOR:
		case B_QUERY_UPDATE:
...
\*------------------------------------------------------------------------------*/
void
BmMailMonitor::CacheRefToFolder(node_ref& nref, const BmString& fKey)
{
...
\*------------------------------------------------------------------------------*/
bool
BmMailMonitor::IsIdle(uint32 msecs)
{
