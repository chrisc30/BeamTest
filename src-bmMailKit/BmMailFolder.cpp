...
\*------------------------------------------------------------------------------*/
bool
BmMailFolder::IsSystemFolderSubPath(const BmString& subPath)
{
	return (subPath == DRAFT_FOLDER_NAME || subPath == IN_FOLDER_NAME || subPath == OUT_FOLDER_NAME
			|| subPath == QUARANTINE_FOLDER_NAME || subPath == SPAM_FOLDER_NAME);
}
...
BmMailFolder::BmMailFolder(
	BmMailFolderList* model, entry_ref& eref, ino_t node, BmMailFolder* parent, time_t& modified)
	  ...
	  mMailCount(-1)
{
	try {
		...
		if (version > 3)
			mRefListStateInfoConnectedToParent = FindMsgBool(archive, MSG_STATEINFO_CONNECTED);
		StartNodeMonitor();
	} catch (BM_error& e) {
		BM_SHOWERR(e.what());
	}
...
\*------------------------------------------------------------------------------*/
status_t
BmMailFolder::Archive(BMessage* archive, bool deep) const
{
	status_t ret = inherited::Archive(archive, deep) || archive->AddRef(MSG_ENTRYREF, &mEntryRef)
				   ...
				   || archive->AddBool(MSG_STATEINFO_CONNECTED, mRefListStateInfoConnectedToParent);
	if (deep && ret == B_OK) {
		BmModelItemMap::const_iterator pos;
		for (pos = begin(); pos != end(); ++pos) {
			if (ret == B_OK) {
				BMessage msg;
				ret = pos->second->Archive(&msg, deep) || archive->AddMessage(MSG_CHILDREN, &msg);
			}
		}
	}
	return ret;
...
\*------------------------------------------------------------------------------*/
bool
BmMailFolder::CheckIfModifiedSinceLastTime()
{
...
\*------------------------------------------------------------------------------*/
bool
BmMailFolder::CheckIfModifiedSince(time_t when, time_t* storeNewModTime)
{
	status_t err;
	BDirectory mailDir;
	...
	if ((err = mailDir.GetModificationTime(&mtime)) != B_OK)
		BM_THROW_RUNTIME(BmString("Could not get mtime \nfor mail folder <")
						 << Name() << "> \n\nError:" << strerror(err));
	BM_LOG3(BM_LogMailTracking, BmString("checking if ")
									<< Name() << ": (" << mtime << " > " << when << ")");
	if (mtime > when) {
...
\*------------------------------------------------------------------------------*/
void
BmMailFolder::AddSpecialFlagForMailRef(const BmString& key)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailFolder::RemoveSpecialFlagForMailRef(const BmString& key)
{
	int32 oldCount = (int32)mSpecialMailRefSet.size();
...
\*------------------------------------------------------------------------------*/
void
BmMailFolder::BumpMailCount(int32 offset)
{
	if (mMailCount < 0)
...
\*------------------------------------------------------------------------------*/
void
BmMailFolder::MailCount(int32 count)
{
	if (mMailCount != count) {
		mMailCount = count;
		TellModelItemUpdated(UPD_TOTAL_COUNT);
...
\*------------------------------------------------------------------------------*/
void
BmMailFolder::BumpSpecialMailCountForSubfolders(int32 offset)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailFolder::UpdateName(const entry_ref& eref)
{
	...
	TheMailFolderList->AdjustForeignKeys(
		path.Length() ? path + "/" + oldName : oldName, path.Length() ? path + "/" + mName : mName);
}
...
\*------------------------------------------------------------------------------*/
void
BmMailFolder::AddMailRef(entry_ref& eref, struct stat& st)
{
	node_ref nref;
	nref.node = st.st_ino;
...
\*------------------------------------------------------------------------------*/
void
BmMailFolder::RemoveMailRef(const node_ref& nref)
{
	BmString key(BM_REFKEY(nref));
...
\*------------------------------------------------------------------------------*/
void
BmMailFolder::UpdateMailRef(const node_ref& nref)
{
	BmString key(BM_REFKEY(nref));
...
\*------------------------------------------------------------------------------*/
void
BmMailFolder::CreateSubFolder(BmString name)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailFolder::Rename(BmString newName)
{
