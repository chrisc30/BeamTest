...
extern "C" {
const char* FilterKinds[] = {FILTER_SIEVE, FILTER_SIEVE_SCRIPT, NULL};
}
...
\*------------------------------------------------------------------------------*/
status_t
BmSieveFilter::Archive(BMessage* archive, bool) const
{
...
BmSieveFilter::CompileScript()
{
	...
	BM_LOG(BM_LogFilter, BmString("Sieve-Addon: compiling SIEVE-script of filter ") << Name());
...
\*------------------------------------------------------------------------------*/
void
BmSieveFilter::Content(const BmString& s)
{
	mContent = s;
	if (mCompiledScript) {
		sieve_script_free(&mCompiledScript);
...
\*------------------------------------------------------------------------------*/
void
BmSieveFilter::RegisterCallbacks(sieve_interp_t* interp)
{
...
\*------------------------------------------------------------------------------*/
int
BmSieveFilter::sieve_parse_error(int lineno, const char* msg, void*, void* script_context)
{
	if (script_context) {
...
\*------------------------------------------------------------------------------*/
BmString
BmSieveFilter::ErrorString() const
{
	return LastErr() + "\n\n"
		   << "Error: " << sieve_strerror(LastErrVal()) << "\n\n"
		   << LastSieveErr();
...
\*------------------------------------------------------------------------------*/
bool
BmSieveFilter::SanityCheck(BmString& complaint, BmString& fieldName)
{
	if (!CompileScript()) {
...
\*------------------------------------------------------------------------------*/
int
BmSieveFilter::sieve_redirect(void* /*action_context*/, void* /*interp_context*/,
	void* /*script_context*/, void* /*message_context*/, const char** /*errmsg*/)
{
	return SIEVE_OK;
}
...
\*------------------------------------------------------------------------------*/
void
BmSieveFilter::SetMailFlags(sieve_imapflags_t* flags, BmMsgContext* msgContext)
{
	if (msgContext && flags) {
...
\*------------------------------------------------------------------------------*/
int
BmSieveFilter::sieve_keep(void* action_context, void*, void*, void* message_context, const char**)
{
	...
	sieve_keep_context* keepContext = static_cast<sieve_keep_context*>(action_context);
...
\*------------------------------------------------------------------------------*/
int
BmSieveFilter::sieve_fileinto(
	void* action_context, void* script_context, void*, void* message_context, const char**)
{
	BmMsgContext* msgContext = static_cast<BmMsgContext*>(message_context);
	sieve_fileinto_context* fileintoContext = static_cast<sieve_fileinto_context*>(action_context);
	BmSieveFilter* filter = static_cast<BmSieveFilter*>(script_context);
	if (msgContext && filter && fileintoContext) {
		...
		if (msgContext->mail->Outbound() && filter->AskBeforeFileInto()) {
			BmMenuAlert* alert = new BmMenuAlert(300, 100, "File into folder",
				"Please select the folder this mail\n should be filed into:",
				new BmMenuControl("Target folder:",
					new BmMenuControllerBase(fileintoContext->mailbox, NULL,
						new BMessage(BM_MENUITEM_SELECTED), &BmGuiRosterBase::RebuildFolderMenu)),
				"Cancel", "OK");
...
\*------------------------------------------------------------------------------*/
int
BmSieveFilter::sieve_reject(
	void* action_context, void* script_context, void*, void* message_context, const char**)
{
	BmMsgContext* msgContext = static_cast<BmMsgContext*>(message_context);
	sieve_reject_context* rejectContext = static_cast<sieve_reject_context*>(action_context);
	BmSieveFilter* filter = static_cast<BmSieveFilter*>(script_context);
...
\*------------------------------------------------------------------------------*/
int
BmSieveFilter::sieve_notify(void* action_context, void*, void*, void* message_context, const char**)
{
	BmMsgContext* msgContext = static_cast<BmMsgContext*>(message_context);
	sieve_notify_context* notifyContext = static_cast<sieve_notify_context*>(action_context);
	if (msgContext && notifyContext) {
		BM_LOG3(BM_LogFilter, BmString("Sieve-Addon: sieve_notify called with "
									   "method ")
								  << notifyContext->method);
...
\*------------------------------------------------------------------------------*/
int
BmSieveFilter::sieve_get_size(void* message_context, int* sizePtr)
{
	BmMsgContext* msgContext = static_cast<BmMsgContext*>(message_context);
	if (msgContext && sizePtr)
		*sizePtr = msgContext->mail->RawText().Length();
	BM_LOG3(BM_LogFilter, BmString("Sieve-Addon: sieve_get_size called, answer = ")
...
\*------------------------------------------------------------------------------*/
int
BmSieveFilter::sieve_get_header(
	void* message_context, const char* header, const char*** contentsPtr)
{
...
\*------------------------------------------------------------------------------*/
int
BmSieveFilter::sieve_execute_error(
	const char* msg, void*, void* script_context, void* message_context)
{
...
static const char* mailParts[] = {"Bcc", "Cc", "From", "Reply-To", "Subject", "To",
	BM_MP_SEPARATOR.String(), "List-Id", "Mailing-List", BM_MP_SEPARATOR.String(), "Status",
	"Account", BM_MP_SEPARATOR.String(), BM_MAILPART_OTHER,
	//	BM_MAILPART_BODY,
	NULL};
...
static const char* addrParts[]
	= {BM_ADDRPART_COMPLETE, BM_ADDRPART_ADDRESS, BM_ADDRPART_DOMAIN, BM_ADDRPART_LOCALPART, NULL};
const char* BM_LONGEST_OP = "num-value is greater than or equal to";
static const char* operators[][3] = {{"is", "", "is"}, {"is not", "not", "is"},
	{"contains", "", "contains"}, {"does not contain", "not", "contains"},
	{"starts with", "", "matches"}, {"does not start with", "not", "matches"},
	{"ends with", "", "matches"}, {"does not end with", "not", "matches"},
	{"matches regex", "", "regex"}, {"does not match regex", "not", "regex"},
...
static const char* choices[] = {"all of", "any of", NULL};
...
\*------------------------------------------------------------------------------*/
status_t
BmGraphicalSieveFilter::Archive(BMessage* archive, bool) const
{
	status_t ret = (archive->AddInt16(MSG_VERSION, nArchiveVersion)
					...
					|| archive->AddString(MSG_SET_SPAM_TOFU_VALUE, mActionSetSpamTofuValue.String())
					|| archive->AddBool(MSG_STOP_PROCESSING, mStopProcessing)
					|| archive->AddBool(MSG_SET_LIST_ID, mActionSetListId)
					|| archive->AddString(MSG_SET_LIST_ID_VALUE, mActionSetListIdValue.String()));
...
\*------------------------------------------------------------------------------*/
void
BmGraphicalSieveFilter::SetupFromMailData(
...
\*------------------------------------------------------------------------------*/
bool
BmGraphicalSieveFilter::IsAddrField(const BmString& addrField)
{
...
\*------------------------------------------------------------------------------*/
bool
BmGraphicalSieveFilter::SanityCheck(BmString& complaint, BmString& fieldName)
{
	if (mActionFileInto && !mActionFileIntoValue.Length()) {
		...
		complaint
			= "Please select the identity that matching mails should "
			  "be associated with";
		fieldName = "setIdentityValue";
		return false;
	} else if (mActionSetSpamTofu && !mActionSetSpamTofuValue.Length()) {
		complaint = "Please select what matching mails should be learned as (SPAM or TOFU)";
	...
	for (int i = 0; i < mMatchCount; ++i) {
		...
		if (!mMatchMailPart[i].Length()) {
			...
			complaint
				= "Please enter the name of the header-field that is to "
				  "be filtered";
			...
			complaint
				= "Please select the value that is to be used for the "
				  "comparison";
	...
	if (!validMatchCount && mActionDiscard) {
		complaint
			= "You have created a filter that will throw away each and "
...
\*------------------------------------------------------------------------------*/
bool
BmGraphicalSieveFilter::BuildScriptFromStrings()
{
	...
	if (mActionSetIdentity || mActionSetStatus || mActionSetSpamTofu || mStopProcessing
		|| mActionSetListId)
	...
	for (int i = 0; i < mMatchCount; ++i) {
		...
		bool needsAddressTest
			= IsAddrField(mMatchMailPart[i]) && mMatchAddrPart[i] != BM_ADDRPART_COMPLETE;
		...
		line << ":" << op << " " << "\"" << (otherField ? mMatchFieldName[i] : mMatchMailPart[i])
			 << "\" ";
		// add list of values:
		line << "[";
	...
	if (mActionSetIdentity)
		script << "   notify :method \"" << BmNotifySetIdentity << "\" :options \""
			   << mActionSetIdentityValue << "\";\n";
	if (mActionSetStatus)
		script << "   notify :method \"" << BmNotifySetStatus << "\" :options \""
			   << mActionSetStatusValue << "\";\n";
	if (mActionSetSpamTofu)
		script << "   notify :method \"" << BmNotifySetSpamTofu << "\" :options \""
			   << mActionSetSpamTofuValue << "\";\n";
	if (mStopProcessing)
		script << "   notify :method \"" << BmNotifyStopProcessing << "\";\n";
	if (mActionSetListId)
		script << "   notify :method \"" << BmNotifySetListId << "\" :options \""
			   << mActionSetListIdValue << "\";\n";
	if (mActionDiscard)
		script << "   discard;\n";
	if (matchPart.Length()) {
		script << "}";
...
\*------------------------------------------------------------------------------*/
void
BmGraphicalSieveFilter::ForeignKeyChanged(
...
public:
	BmFilterGroup()
		: BView(BRect(0, 0, 99, 99), "filterGroup", B_FOLLOW_ALL, B_WILL_DRAW)
	{
	}
...
BRect
BmFilterGroup::layout(BRect rect)
{
...
public:
	BmFilterScrollView(MView* target, border_style border = B_FANCY_BORDER)
		: BScrollView("scroller", dynamic_cast<BView*>(target), B_FOLLOW_NONE,
			  B_WILL_DRAW | B_FRAME_EVENTS, false, true, border),
		  kid(target)
	{
...
minimax
BmFilterScrollView::layoutprefs()
{
	kid->layoutprefs();
	return mpm = ct_mpm;
}
...
BRect
BmFilterScrollView::layout(BRect rect)
{
	  ...
	  mCurrFilterAddon(NULL)
{
	...
	VGroup* vgroup = new VGroup(
		new MBorder(M_LABELED_BORDER, 5, (char*)"Selection criteria",
			new VGroup(
				new HGroup(mAnyAllControl = new BmMenuControl("Match if ", new BPopUpMenu("")),
					new MStringView((char*)" the following is true"),
					new Space(minimax(1, 1, 1e5, 1)),
					mAddButton = new MButton("Add line", new BMessage(BM_ADD_FILTER_LINE), this,
						minimax(-1, -1, -1, -1)),
					mRemoveButton = new MButton("Remove marked lines",
						new BMessage(BM_REMOVE_FILTER_LINE), this, minimax(-1, -1, -1, -1)),
					0),
				mFilterScrollView = new BmFilterScrollView(mFilterGroup, B_FANCY_BORDER), 0)),
		new MBorder(M_LABELED_BORDER, 5, (char*)"Filter actions",
			mActionGroup = new VGroup(
				new HGroup(
					new VGroup(new HGroup(mFileIntoControl = new BmCheckControl(
											  "File into", new BMessage(BM_FILEINTO_CHANGED), this),
								   mFileIntoValueControl = new BmMenuControl(
									   NULL, new BmMenuControllerBase("", this,
												 new BMessage(BM_FILEINTO_SELECTED),
												 &BmGuiRosterBase::RebuildFolderMenu)),
								   mFileIntoAskControl = new BmCheckControl("Ask",
									   new BMessage(BM_FILEINTO_ASK_CHANGED), this),
								   0),
						new HGroup(mSetStatusControl = new BmCheckControl(
									   "Set status", new BMessage(BM_SET_STATUS_CHANGED), this),
							mSetStatusValueControl = new BmMenuControl(NULL,
								new BmMenuControllerBase("", this,
									new BMessage(BM_SET_STATUS_SELECTED),
									&BmGuiRosterBase::RebuildStatusMenu, BM_MC_LABEL_FROM_MARKED)),
							0),
						new HGroup(mSetIdentityControl = new BmCheckControl(
									   "Set identity", new BMessage(BM_SET_IDENTITY_CHANGED), this),
							mSetIdentityValueControl
							= new BmMenuControl(NULL, new BmMenuControllerBase("", this,
														  new BMessage(BM_SET_IDENTITY_SELECTED),
														  &BmGuiRosterBase::RebuildIdentityMenu,
														  BM_MC_LABEL_FROM_MARKED)),
							0),
						new HGroup(mSetListIdControl = new BmCheckControl(
									   "Set List-Id", new BMessage(BM_SET_LIST_ID_CHANGED), this),
							mSetListIdValueControl = new BmTextControl((const char*)NULL), 0),
						0),
					new Space(minimax(20, 0, 20, 1e5)),
					new VGroup(minimax(-1, -1, 1e5, 1e5, 0.7f),
						new HGroup(mSetSpamTofuControl = new BmCheckControl(
									   "Learn as", new BMessage(BM_SET_SPAM_TOFU_CHANGED), this),
							mSetSpamTofuValueControl = new BmMenuControl(
								NULL, spamTofuMenu = new BmMenuControllerBase("", this,
										  new BMessage(BM_SET_SPAM_TOFU_SELECTED), NULL,
										  BM_MC_LABEL_FROM_MARKED)),
							0),
						new HGroup(mDiscardControl = new BmCheckControl(
									   "Move to Trash", new BMessage(BM_DISCARD_CHANGED), this),
							new Space(), 0),
						new HGroup(mStopProcessingControl
								   = new BmCheckControl("Stop processing if filter matches",
									   new BMessage(BM_STOP_PROCESSING_CHANGED), this),
							new Space(), 0),
						rightSpace = new Space(), 0),
					0),
				0)),
		0);
	...
	mFileIntoControl->ct_mpm.maxi.x = mFileIntoControl->ct_mpm.mini.x
		...
		= mSetListIdControl->ct_mpm.maxi.x = mSetListIdControl->ct_mpm.mini.x = maxWidth + 5;

	rightSpace->ct_mpm.maxi.y = rightSpace->ct_mpm.mini.y = mSetListIdValueControl->ct_mpm.mini.y;
	...
	for (int i = 0; i < BM_MAX_MATCH_COUNT; ++i) {
		mFilterLine[i] = new HGroup(mMailPartControl[i] = new BmMenuControl(
										NULL, new BPopUpMenu(""), 0, 0, BM_MAILPART_OTHER),
			new VGroup(mFieldSpecLayer[i] = new LayeredGroup(new Space(),
						   mAddrPartControl[i] = new BmMenuControl(
							   NULL, new BPopUpMenu(""), 0, 0, BM_ADDRPART_LOCALPART),
						   mFieldNameControl[i] = new BmTextControl((const char*)NULL), NULL),
				0),
			mOperatorControl[i] = new BmMenuControl(NULL, new BPopUpMenu(""), 0, 0, BM_LONGEST_OP),
			mValueControl[i] = new BmMultiLineTextControl(NULL, false, 1, 0, true),
			new VGroup(mMarkControl[i] = new BmCheckControl(NULL, NULL, this), new Space(), 0), 0);
...
\*------------------------------------------------------------------------------*/
void
BmSieveFilterPrefs::Initialize()
{
	TheBubbleHelper->SetHelp(mAnyAllControl,
	...
	for (int i = 0; i < BM_MAX_MATCH_COUNT; ++i) {
		TheBubbleHelper->SetHelp(mMailPartControl[i],
			"Please select the part of the mail (a header-field)\n"
			"that shall be matched.");
		TheBubbleHelper->SetHelp(mAddrPartControl[i],
			...
			"                i.e. 'esmith'");
		TheBubbleHelper->SetHelp(mFieldNameControl[i],
			"If you have chosen to specify the fieldname yourself,\n"
			"you can do so in this textfield.");
		TheBubbleHelper->SetHelp(
			mOperatorControl[i], "Here you can select what kind of match shall be done.");
		TheBubbleHelper->SetHelp(mValueControl[i],
		...
		TheBubbleHelper->SetHelp(mMarkControl[i], "Check to mark this line.");
	}
	TheBubbleHelper->SetHelp(
		mFileIntoControl, "Check this to move matching mails into a specific folder.");
	TheBubbleHelper->SetHelp(
		mFileIntoValueControl, "Here you can select the folder to file mails into.");
	TheBubbleHelper->SetHelp(mFileIntoAskControl,
		"Check this if you want to be asked for acknowledgement\n"
		"before any outbound mail matching the filter is filed.");
	TheBubbleHelper->SetHelp(
		mSetStatusControl, "Check this to modify the status of matching mails.");
	TheBubbleHelper->SetHelp(mSetStatusValueControl, "Please select the new status here.");
	TheBubbleHelper->SetHelp(mSetIdentityControl,
		"Check this if you want to associate matching mails\n"
		"with a given identity (e.g. useful for mailing-lists).");
	TheBubbleHelper->SetHelp(
		mSetIdentityValueControl, "Please select the associated identity here.");
	TheBubbleHelper->SetHelp(
		mSetSpamTofuControl, "Check this to learn matching mails as SPAM or TOFU.");
	TheBubbleHelper->SetHelp(
		mSetSpamTofuValueControl, "Please select the new classification here.");
	TheBubbleHelper->SetHelp(mSetListIdControl,
		"Check this if you want to associate matching mails\n"
		"with a specific mailing list.");
	TheBubbleHelper->SetHelp(mSetIdentityValueControl, "Please enter the List-Id here.");
	TheBubbleHelper->SetHelp(
		mDiscardControl, "Check this to directly move matching mails to trash.");
	...
	BMenuItem* item = new BMenuItem("SPAM", new BMessage(BM_SET_SPAM_TOFU_SELECTED));
	item->SetTarget(this);
...
\*------------------------------------------------------------------------------*/
void
BmSieveFilterPrefs::AddFilterLine()
{
...
\*------------------------------------------------------------------------------*/
void
BmSieveFilterPrefs::RemoveMarkedFilterLines()
{
	int32 currIdx = 0;
	for (int32 i = 0; i < mVisibleLines; ++i) {
		if (!mMarkControl[i]->Value()) {
			if (i != currIdx) {
				...
				mMailPartControl[currIdx]->MarkItem(mMailPartControl[i]->MenuItem()->Label());
				mCurrFilterAddon->mMatchMailPart[currIdx] = mCurrFilterAddon->mMatchMailPart[i];
				mFieldNameControl[currIdx]->SetText(mFieldNameControl[i]->Text());
				mCurrFilterAddon->mMatchFieldName[currIdx] = mCurrFilterAddon->mMatchFieldName[i];
				mOperatorControl[currIdx]->MarkItem(mOperatorControl[i]->MenuItem()->Label());
				mOperatorControl[currIdx]->MenuItem()->SetLabel(
					mOperatorControl[i]->MenuItem()->Label());
				mCurrFilterAddon->mMatchOperator[currIdx] = mCurrFilterAddon->mMatchOperator[i];
				mValueControl[currIdx]->SetText(mValueControl[i]->Text());
				mCurrFilterAddon->mMatchValue[currIdx] = mCurrFilterAddon->mMatchValue[i];
			}
			currIdx++;
		} else
...
\*------------------------------------------------------------------------------*/
void
BmSieveFilterPrefs::AdjustScrollView()
{
...
\*------------------------------------------------------------------------------*/
void
BmSieveFilterPrefs::AdjustSizeOfValueControl(BmMultiLineTextControl* control)
{
	...
	control->ct_mpm.mini.y = control->ct_mpm.maxi.y = control->mpm.mini.y = control->mpm.maxi.y
		= height;
...
\*------------------------------------------------------------------------------*/
void
BmSieveFilterPrefs::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		...
		case BM_TEXTFIELD_MODIFIED:
		{
			if (mCurrFilterAddon) {
				...
				if (source == mSetListIdValueControl) {
					mCurrFilterAddon->mActionSetListIdValue = mSetListIdValueControl->Text();
					UpdateState();
					PropagateChange();
				} else {
...
\*------------------------------------------------------------------------------*/
void
BmSieveFilterPrefs::ShowFilter(BmFilterAddon* addon)
{
	...
	if (mCurrFilterAddon) {
		...
		mFileIntoValueControl->MarkItem(mCurrFilterAddon->mActionFileIntoValue.String());
		mFileIntoAskControl->SetValueSilently(mCurrFilterAddon->mActionFileIntoAsk);
		...
		mSetSpamTofuControl->SetValueSilently(mCurrFilterAddon->mActionSetSpamTofu);
		mSetSpamTofuValueControl->MarkItem(mCurrFilterAddon->mActionSetSpamTofuValue.String());
		mSetListIdControl->SetValueSilently(mCurrFilterAddon->mActionSetListId);
		mSetListIdValueControl->SetTextSilently(mCurrFilterAddon->mActionSetListIdValue.String());
...
\*------------------------------------------------------------------------------*/
void
BmSieveFilterPrefs::UpdateState()
{
	if (mCurrFilterAddon) {
	  ...
	  mCurrFilterAddon(NULL)
{
	...
	AddChild(dynamic_cast<BView*>(
		new HGroup(new VGroup(mContentControl = new BmMultiLineTextControl("", false, 10), 0),
			new Space(minimax(5, 0, 5, 1e5)),
			new VGroup(mTestButton = new MButton("Check the SIEVE-script",
						   new BMessage(BM_TEST_FILTER), this, minimax(-1, -1, -1, -1)),
				new Space(), 0),
			new Space(minimax(5, 0, 5, 1e5)), 0)));
}
...
\*------------------------------------------------------------------------------*/
void
BmSieveScriptFilterPrefs::Initialize()
{
	TheBubbleHelper->SetHelp(
		mContentControl, "Here you can enter the content of this filter (a SIEVE script).");
...
\*------------------------------------------------------------------------------*/
const char*
BmSieveScriptFilterPrefs::Kind() const
{
...
\*------------------------------------------------------------------------------*/
void
BmSieveScriptFilterPrefs::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		case BM_MULTILINE_TEXTFIELD_MODIFIED:
		{
		...
		case BM_TEST_FILTER:
		{
			if (mCurrFilterAddon) {
				if (!mCurrFilterAddon->CompileScript()) {
					BAlert* alert
						= new BAlert("Filter test", mCurrFilterAddon->ErrorString().String(), "OK",
...
\*------------------------------------------------------------------------------*/
void
BmSieveScriptFilterPrefs::ShowFilter(BmFilterAddon* addon)
{
...
InstantiateFilterPrefs(float minx, float miny, float maxx, float maxy, const BmString& kind)
{
	if (!kind.ICompare(FILTER_SIEVE))
		return new BmSieveFilterPrefs(minimax(int(minx), int(miny), int(maxx), int(maxy)));
	else if (!kind.ICompare(FILTER_SIEVE_SCRIPT))
		return new BmSieveScriptFilterPrefs(minimax(int(minx), int(miny), int(maxx), int(maxy)));
	else
		return NULL;
}
