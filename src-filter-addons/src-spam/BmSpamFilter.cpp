...
extern "C" BmFilterAddon* InstantiateFilter(
	const BmString& name, const BMessage* archive, const BmString& kind);

extern "C" BmFilterAddonPrefsView* InstantiateFilterPrefs(
	float minx, float miny, float maxx, float maxy, const BmString& kind);

extern "C" {
const char* FilterKinds[] = {FILTER_SPAM, NULL};
...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmSpamFilter::OsbfClassifier::SpamRelevantMailtextSelector::HtmlRemover ::HtmlRemover(
	BmMemIBuf* input, uint32 blockSize)
...
\*------------------------------------------------------------------------------*/
void
BmSpamFilter::OsbfClassifier::SpamRelevantMailtextSelector::HtmlRemover ::Filter(
	const char* srcBuf, uint32& srcLen, char* destBuf, uint32& destLen)
{
	BM_LOG3(BM_LogMailParse, BmString("starting to remove html of ") << srcLen << " bytes");
...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmSpamFilter::OsbfClassifier::SpamRelevantMailtextSelector ::SpamRelevantMailtextSelector(
	BmMail* mail)
	: mMail(mail),
	  mDeHtmlBuf(4096)
{
...
\*------------------------------------------------------------------------------*/
void
BmSpamFilter::OsbfClassifier::SpamRelevantMailtextSelector ::operator()(BmStringIBuf& inBuf)
{
	...
	if ((from.IFindFirst("Mailer-Daemon") >= 0 || from.IFindFirst("Postmaster") >= 0)
		...
		if (origMailColl.mOrigMailBodyPart)
			...
			if (textBody) {
				int32 inlineMailPos = textBody->DecodedData().IFindFirst("Return-Path:");
				if (inlineMailPos < 0)
...
\*------------------------------------------------------------------------------*/
void
BmSpamFilter::OsbfClassifier::SpamRelevantMailtextSelector ::AddSpamRelevantBodyParts(
	BmStringIBuf& inBuf)
{
	if (mMail->Body()->empty())
		return;
	BAutolock lock(mMail->Body()->ModelLocker());
	BmRef<BmBodyPart> body = dynamic_cast<BmBodyPart*>(mMail->Body()->begin()->second.Get());
...
\*------------------------------------------------------------------------------*/
BmBodyPart*
BmSpamFilter::OsbfClassifier::SpamRelevantMailtextSelector ::FindBodyPartWithHighestSpamRelevance(
	BmBodyPart* parent)
{
...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmSpamFilter::OsbfClassifier ::FeatureFilter::FeatureFilter(BmMemIBuf* input, uint32 blockSize)
...
\*------------------------------------------------------------------------------*/
void
BmSpamFilter::OsbfClassifier ::FeatureFilter::Filter(
	const char* srcBuf, uint32& srcLen, char* destBuf, uint32& destLen)
{
	BM_LOG3(BM_LogFilter, BmString("starting to filter features of ") << srcLen << " bytes");
	const char* src = srcBuf;
	const char* srcEnd = srcBuf + srcLen;
	char* dest = destBuf;
	char* destEnd = destBuf + destLen;
...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmSpamFilter::OsbfClassifier ::FeatureLearner::FeatureLearner(
	FeatureBucket* hash, Header* header, bool revert)
	: mHash(hash),
	  mHeader(header),
	  mRevert(revert),
	  mStatus(B_OK)
...
\*------------------------------------------------------------------------------*/
status_t
BmSpamFilter::OsbfClassifier ::FeatureLearner::operator()(char* buf, uint32 bufLen)
{
	...
	for (uint32 j = 1; j < WindowLen; j++) {
		...
		if (mHash[hindex].GetValue() == 0)
			BM_LOG3(BM_LogFilter, BmString("New feature at ") << hindex);
		else
			BM_LOG3(BM_LogFilter, BmString("Old feature at ") << hindex);
		...
		if (!mHash[hindex].IsLocked()) {
			if (sense > 0 && mHash[hindex].GetValue() + sense >= FeatureBucketValueMax - 1)
...
\*------------------------------------------------------------------------------*/
void
BmSpamFilter::OsbfClassifier ::FeatureLearner::Finalize()
{
...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmSpamFilter::OsbfClassifier ::FeatureClassifier::FeatureClassifier(
	FeatureBucket* spamHash, Header* spamHeader, FeatureBucket* tofuHash, Header* tofuHeader)
	: mStatus(B_OK),
	  mTotalLearnings(0),
	  mTotalFeatures(0)
{
...
\*------------------------------------------------------------------------------*/
status_t
BmSpamFilter::OsbfClassifier ::FeatureClassifier::operator()(char* buf, uint32 bufLen)
{
	...
	for (j = 1; j < WindowLen; j++) {
		...
		if (!ApplyVoodoo)
			...
			confidence_factor
				= pow((diff_hits * diff_hits + hits_max_p * hits_min_p - K1 / sum_hits)
						  / (sum_hits * sum_hits),
					  K2)
				  / (1.0 + K3 / (sum_hits * FeatureWeight[j]));

		BM_LOG3(BM_LogFilter, BmString("CF:")
								  << confidence_factor << " max_hits:" << hits_max_p
								  << " min_hits:" << hits_min_p << " weight:" << FeatureWeight[j]);
		...
		for (k = 0; k < MaxHash; k++) {
			...
			mPltc[k] = (double)mLearnings[k] / (double)mTotalLearnings
					   * (0.5
						   + confidence_factor *
								 //								   ((double) mUniqueFeatures[k] /
								 //(double)mTotalFeatures) *
								 ((double)mHits[k] / (double)mLearnings[k] - 0.5));

			BM_LOG3(BM_LogFilter,
				BmString("CF:") << confidence_factor << " totalhits[k]:" << mTotalHits[k]
								<< " missedfeatures[k]:" << mMissedFeatures[k]
								<< " uniquefeatures[k]:" << mUniqueFeatures[k] << " totalfeatures:"
								<< mTotalFeatures << " weight:" << FeatureWeight[j]);
...
\*------------------------------------------------------------------------------*/
void
BmSpamFilter::OsbfClassifier ::FeatureClassifier::Finalize()
{
	uint32 k;
	...
	for (k = 0; k < MaxHash; k++)
		BM_LOG2(BM_LogFilter, BmString("Probability of match for file ")
								  << mHashName[k] << " = " << mPtc[k]);
	...
	BM_LOG2(BM_LogFilter, BmString("Best match to file ")
							  << mHashName[bestseen] << " prob:" << mPtc[bestseen]
							  << " pR:" << mOverallPr);
}
...
unsigned char BmSpamFilter::OsbfClassifier ::FileVersion[4] = {'C', 'F', 'C', '1'};
...
//
const int32 BmSpamFilter::OsbfClassifier ::HashCoeff[20]
	= {1, 7, 3, 13, 5, 29, 11, 51, 23, 101, 47, 203, 97, 407, 197, 817, 397, 1637, 797, 3277};

float BmSpamFilter::OsbfClassifier ::FeatureWeight[6] = {0, 3125, 277, 27, 4, 1};

/* max feature value */
const uint32 BmSpamFilter::OsbfClassifier ::WindowLen = 5;

/* max feature value */
const uint32 BmSpamFilter::OsbfClassifier ::FeatureBucketValueMax = 65535;

/* max number of features */
const uint32 BmSpamFilter::OsbfClassifier ::DefaultFileLength = 94321;

/* shall we try to do small cleanups automatically, if the hash-chains
   get too long? */
const bool BmSpamFilter::OsbfClassifier ::DoMicrogroom = true;

/* max chain len - microgrooming is triggered after this, if enabled */
const unsigned long BmSpamFilter::OsbfClassifier ::MicrogroomChainLength = 29;

/* maximum number of buckets groom-zeroed */
const unsigned long BmSpamFilter::OsbfClassifier ::MicrogroomStopAfter = 128;

/* minimum ratio between max and min P(F|C) */
const unsigned long BmSpamFilter::OsbfClassifier ::MinPmaxPminRatio = 9;

const uint32 BmSpamFilter::OsbfClassifier ::ValueMask = 0x0000FFFFLU;
const uint32 BmSpamFilter::OsbfClassifier ::LockedMask = 0x80000000LU;
...
\*------------------------------------------------------------------------------*/
void
BmSpamFilter::OsbfClassifier::Initialize()
{
	if (!mSpamHash) {
		BmString spamFilename = BmString(BeamRoster->SettingsPath()) << "/Spam.data";
		ReadDataFile(spamFilename, mSpamHeader, mSpamHash);
	}
	if (!mTofuHash) {
		BmString tofuFilename = BmString(BeamRoster->SettingsPath()) << "/Tofu.data";
...
\*------------------------------------------------------------------------------*/
void
BmSpamFilter::OsbfClassifier::Store()
{
...
{
	if (msgContext->mail->IsMarkedAsSpam() && !msgContext->GetBool("ForceLearning"))
...
\*------------------------------------------------------------------------------*/
bool
BmSpamFilter::OsbfClassifier::LearnAsTofu(BmMsgContext* msgContext)
{
	if (msgContext->mail->IsMarkedAsTofu() && !msgContext->GetBool("ForceLearning"))
		return false;  // learning once is enough
...
\*------------------------------------------------------------------------------*/
bool
BmSpamFilter::OsbfClassifier::Learn(BmMsgContext* msgContext, bool learnAsSpam, bool revert)
{
...
\*------------------------------------------------------------------------------*/
bool
BmSpamFilter::OsbfClassifier::Classify(BmMsgContext* msgContext)
{
	// set lock to serialize OSBF-calls:
	BAutolock lock(&mLock);
	if (!lock.IsLocked()) {
...
\*------------------------------------------------------------------------------*/
bool
BmSpamFilter::OsbfClassifier ::DoActualClassification(BmMsgContext* msgContext, double& overallPr)
{
	BmStringIBuf text;
	SpamRelevantMailtextSelector selector(msgContext->mail);
	selector(text);
	FeatureFilter filter(&text);
	BmMemBufConsumer consumer(4096);
	FeatureClassifier classifier(mSpamHash, &mSpamHeader, mTofuHash, &mTofuHeader);
...
\*------------------------------------------------------------------------------*/
bool
BmSpamFilter::OsbfClassifier::Reset(BmMsgContext* msgContext)
{
...
\*------------------------------------------------------------------------------*/
bool
BmSpamFilter::OsbfClassifier::Reload(BmMsgContext* msgContext)
{
...
\*------------------------------------------------------------------------------*/
bool
BmSpamFilter::OsbfClassifier::ResetStatistics(BmMsgContext* msgContext)
{
...
\*------------------------------------------------------------------------------*/
bool
BmSpamFilter::OsbfClassifier::GetStatistics(BmMsgContext* msgContext)
{
	...
	msgContext->SetInt32("SpamChainsAverageLength", numChains > 0 ? totChain / numChains : 0);
	...
	msgContext->SetInt32("TofuChainsAverageLength", numChains > 0 ? totChain / numChains : 0);
...
//
void
BmSpamFilter::OsbfClassifier::Microgroom(FeatureBucket* hash, Header* header, uint32 hindex)
{
	uint32 i, j;
	uint32 packstart;
	int32 packlen;
	...
	if (min_value == FeatureBucketValueMax) { /* no unlocked bucket avaiable so groom any */
		groom_any = true;
	...
	while (zeroed_countdown == max_zeroed_buckets) {
		...
		// microgroom_stop_after, max_distance);
		i = packstart;
		while (hash[i].InChain() && zeroed_countdown > 0) {
			// check if it's a candidate
			if (hash[i].GetValue() == min_value && (!hash[i].IsLocked() || groom_any)) {
	...
	BM_LOG3(BM_LogFilter, BmString("Leaving microgroom: ")
							  << MicrogroomStopAfter - zeroed_countdown << " buckets with value "
							  << hash[i].GetValue() << " zeroed at distance " << max_distance - 1);
...
void
BmSpamFilter::OsbfClassifier::PackData(
	Header* header, FeatureBucket* hash, uint32 packstart, uint32 packlen)
{
...
\*------------------------------------------------------------------------------*/
void
BmSpamFilter::OsbfClassifier::PackDataSeg(
	Header* header, FeatureBucket* hash, uint32 packstart, uint32 packlen)
{
	uint32 ifrom, ito;
	uint32 thash, tkey;
...
\*------------------------------------------------------------------------------*/
const BmString&
BmSpamFilter::OsbfClassifier::ErrorString() const
{
	return mLastErr;
}
...
\*------------------------------------------------------------------------------*/
status_t
BmSpamFilter::OsbfClassifier::CreateDataFile(const BmString& filename)
{
	status_t err;
	BFile file;
	BM_LOG(BM_LogFilter, BmString("Spam-Addon: trying to create datafile ") << filename);
	...
	if (err == B_OK) {
		...
		if ((err = (status_t)file.Write(feature, sz)) != sz)
			return err < 0 ? err : B_IO_ERROR;
		free(feature);
		BM_LOG(BM_LogFilter, BmString("Spam-Addon: ok, done creating datafile ") << filename);
	}
	if (err < B_OK)
		BM_LOGERR(
			BmString("Couldn't create spam/tofu datafile ") << filename << " -> " << strerror(err));
...
\*------------------------------------------------------------------------------*/
status_t
BmSpamFilter::OsbfClassifier::ReadDataFile(
	const BmString& filename, Header& header, FeatureBucket*& hash)
{
	BFile file;
	// try to open data-file...
	status_t err;
	BM_LOG(BM_LogFilter, BmString("Spam-Addon: trying to read datafile ") << filename);
	...
	if ((err = (status_t)file.Read(&header, sizeof(header))) < (int32)sizeof(header))
		return err < 0 ? err : B_IO_ERROR;
	BM_LOG2(BM_LogFilter, BmString("Spam-Addon: file has room for ")
							  << header.buckets << " features, number of learnings is "
							  << header.learnings);
	// check version
	...
	BM_LOG(BM_LogFilter, BmString("Spam-Addon: ok, done reading datafile ") << filename);
	return B_OK;
}
...
\*------------------------------------------------------------------------------*/
status_t
BmSpamFilter::OsbfClassifier::WriteDataFile(
	const BmString& filename, Header& header, FeatureBucket*& hash)
{
	BFile file;
	// try to open data-file...
	status_t err;
	BM_LOG(BM_LogFilter, BmString("Spam-Addon: trying to write datafile ") << filename);
	...
	BM_LOG(BM_LogFilter, BmString("Spam-Addon: ok, done writing datafile ") << filename);
	return B_OK;
}
	...
	if ((!jobSpecifier.ICompare("Classify") || !jobSpecifier.Length())
		...
		if (result) {
			...
			if (isSpam) {
				...
				BM_LOG(BM_LogFilter, BmString("Spam-Addon: mail is considered SPAM ")
										 << "(with a confidence of " << (int)(ratioSpam * 100)
										 << "%).");
				...
				if (D.mProtectKnownAddrs && BeamGuiRoster) {
					...
					isFromKnownAddress = BeamGuiRoster->IsEmailKnown(fromAddr.AddrSpec());
				}
				if (isFromKnownAddress) {
					...
					if (D.mActionFileUnsure && ratioSpam < D.mUnsureThreshold / 100.0) {
						msgContext->SetString("FolderName", BmMailFolder::QUARANTINE_FOLDER_NAME);
					} else {
...
\*------------------------------------------------------------------------------*/
bool
BmSpamFilter::SanityCheck(BmString& complaint, BmString& fieldName)
{
	return true;
}
...
\*------------------------------------------------------------------------------*/
BmString
BmSpamFilter::ErrorString() const
{
...
/*------------------------------------------------------------------------------*\
	InstantiateFilter()
		-
\*------------------------------------------------------------------------------*/
extern "C" BmFilterAddon*
InstantiateFilter(const BmString& name, const BMessage* archive, const BmString& kind)
{
	  ...
	  mCurrFilterAddon(NULL)
{
	mThresholdControl = new MSlider(
		"Auto-learning threshold:", 0, 40, 1, new BMessage(BM_THRESHOLD_CHANGED), this);
	mProtectMyTofuControl = new MSlider(
		"Tofu (genuine mail) protection:", 0, 25, 1, new BMessage(BM_PROTECTMYTOFU_CHANGED), this);
	mUnsureThresholdControl = new MSlider(
		"Quarantine threshold:", 50, 90, 1, new BMessage(BM_UNSURE_THRESHOLD_CHANGED), this);
	mShowStatisticsButton = new MButton("Show statistics" B_UTF8_ELLIPSIS,
		new BMessage(BM_SHOW_STATISTICS), this, minimax(-1, -1, -1, -1));
	mResetStatisticsButton = new MButton("Reset statistics" B_UTF8_ELLIPSIS,
		new BMessage(BM_RESET_STATISTICS), this, minimax(-1, -1, -1, -1));
	mTrainButton = new MButton("Train filter" B_UTF8_ELLIPSIS, new BMessage(BM_TRAIN_FILTER), this,
		minimax(-1, -1, -1, -1));
	mSpamThresholdBar = new BStatusBar(BRect(), "SpamBar", "Resulting Spam threshold: ", "");
	mTofuThresholdBar = new BStatusBar(BRect(), "TofuBar", "Resulting Tofu threshold: ", "");
	mProtectKnownAddrsControl = new BmCheckControl("Accept all mails from known addresses as Tofu",
		new BMessage(BM_PROTECTKNOWN_CHANGED), this);
	mStopProcessingControl = new BmCheckControl("Stop processing a message considered Spam",
		new BMessage(BM_STOP_PROCESSING_CHANGED), this);
	HGroup* hgroup = new HGroup(
		new VGroup(new MBorder(M_LABELED_BORDER, 5, (char*)"Settings",
					   new VGroup(mThresholdControl, new Space(minimax(0, 5, 1e5, 5)),
						   mProtectMyTofuControl, new Space(minimax(0, 10, 1e5, 10)),
						   mUnsureThresholdControl, 0)),
			new HGroup(mShowStatisticsButton, new Space(minimax(5, 0, 5, 1e5)),
				mResetStatisticsButton, new Space(minimax(0, 0, 1e5, 1e5)), mTrainButton, 0),
			new Space(minimax(0, 0, 1e5, 1e5)), 0),
		new VGroup(new MBorder(M_LABELED_BORDER, 5, (char*)"Effective Vvlues",
					   new VGroup(new MBViewWrapper(mSpamThresholdBar, true, false, false),
						   new MBViewWrapper(mTofuThresholdBar, true, false, false), 0)),
			new MBorder(M_LABELED_BORDER, 5, (char*)"Filter actions",
				new VGroup(mProtectKnownAddrsControl, new Space(minimax(0, 5, 1e5, 5)),
					mStopProcessingControl, 0)),
			new Space(minimax(0, 0, 1e5, 1e5)), 0),
		0);
...
\*------------------------------------------------------------------------------*/
void
BmSpamFilterPrefs::Initialize()
{
	TheBubbleHelper->SetHelp(mThresholdControl,
	...
	TheBubbleHelper->SetHelp(mProtectMyTofuControl,
	...
	TheBubbleHelper->SetHelp(mProtectKnownAddrsControl,
	...
	TheBubbleHelper->SetHelp(mStopProcessingControl,
	...
	TheBubbleHelper->SetHelp(mUnsureThresholdControl,
	...
	TheBubbleHelper->SetHelp(mTrainButton,
...
\*------------------------------------------------------------------------------*/
void
BmSpamFilterPrefs::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		case BM_SHOW_STATISTICS:
		{
			...
			double errorsFP
				= spamClassifications ? 100.0 * spamMistakes / (float)spamClassifications : 0;
			...
			stats << buf;
			sprintf(buf,
				" features: %6" B_PRId32 " (%3" B_PRId32 "%% used) | features: %6" B_PRId32
				" (%3" B_PRId32 "%% used)\n",
				spamBuckets, 100 * spamBucketsUsed / spamBuckets, tofuBuckets,
				100 * tofuBucketsUsed / tofuBuckets);
			stats << buf;
			sprintf(buf,
			...
			if (spamClassifications || tofuClassifications) {
				sprintf(buf, " correctness(FP):     %6.2f  | correctness (all):    %6.2f\n",
					100.0 - errorsFP, 100.0 - errorsAll);
				stats << buf;
			}
			TextEntryAlert* alert = new TextEntryAlert("", "SPAM-filter statistics:", "", "OK",
				NULL, false, 500.0, 12, B_WIDTH_AS_USUAL, true);
		...
		case BM_RESET_STATISTICS:
		{
			...
			if (msg->FindInt32("which", &buttonPressed) != B_OK) {
				...
				BmString s(
					"This will reset all statistical information (how many\n"
					"mails have been learned/classified) from the datafiles\n"
					"(the SPAM-/TOFU-data itself is not changed at all).\n\n");
				BAlert* alert = new BAlert("Reset statistics", s.String(), "Reset", "Cancel", NULL,
					B_WIDTH_AS_USUAL, B_WARNING_ALERT);
				alert->SetShortcut(1, B_ESCAPE);
				alert->Go(new BInvoker(new BMessage(BM_RESET_STATISTICS), BMessenger(this)));
			} else {
		...
		case BM_TRAIN_FILTER:
		{
			if (nSpamometerRunning) {
				BAlert* alert
					= new BAlert("Train SPAM-filter", "A training session is already running!",
			...
			if (msg->FindInt32("which", &buttonPressed) != B_OK) {
				// first step, ask user about it:
				BmString s(
					"Beam will search for (up to 1000) SPAM/TOFU mails\n"
				...
				BAlert* alert = new BAlert("Train SPAM-filter", s.String(), "Train filter",
					"Cancel", NULL, B_WIDTH_AS_USUAL, B_WARNING_ALERT);
				alert->SetShortcut(1, B_ESCAPE);
				alert->Go(new BInvoker(new BMessage(BM_TRAIN_FILTER), BMessenger(this)));
			} else {
				// second step, do it if user said ok:
				if (buttonPressed == 0) {
					thread_id t_id = spawn_thread(
						StartSpamometer, "SpamOMeter", B_NORMAL_PRIORITY, mCurrFilterAddon);
					if (t_id < 0)
...
\*------------------------------------------------------------------------------*/
int32
BmSpamFilterPrefs::StartSpamometer(void* data)
{
	BmSpamFilter* filterAddon = static_cast<BmSpamFilter*>(data);
	if (filterAddon) {
		BmString spamometer = BeamRoster->AppPath();
		spamometer << "/tools/SpamOMeter --dehtml --keep-atags"
				   << " --rst=" << filterAddon->D.mSpamThreshold
				   << " --rtt=" << filterAddon->D.mTofuThreshold << " --do-training";
		...
		BAlert* alert = new BAlert("Train SPAM-filter", s.String(), b.String(), NULL, NULL,
			B_WIDTH_AS_USUAL, B_WARNING_ALERT);
		alert->SetShortcut(0, B_ESCAPE);
		alert->Go();
	}
...
\*------------------------------------------------------------------------------*/
void
BmSpamFilterPrefs::ShowFilter(BmFilterAddon* addon)
{
	...
	if (mCurrFilterAddon) {
		...
		mProtectKnownAddrsControl->SetValueSilently(mCurrFilterAddon->D.mProtectKnownAddrs);
		mStopProcessingControl->SetValueSilently(mCurrFilterAddon->D.mStopProcessing);
...
\*------------------------------------------------------------------------------*/
bool
BmSpamFilterPrefs::UpdateState(bool force)
{
	if (mCurrFilterAddon) {
		...
		int32 tofuThr = thresholdVal + protectMyTofuVal / 2 + protectMyTofuVal % 2;
...
/*------------------------------------------------------------------------------*\
	InstantiateFilterPrefs()
		-
\*------------------------------------------------------------------------------*/
extern "C" BmFilterAddonPrefsView*
InstantiateFilterPrefs(float minx, float miny, float maxx, float maxy, const BmString& kind)
{
	if (!kind.ICompare(FILTER_SPAM))
		return new BmSpamFilterPrefs(minimax(int(minx), int(miny), int(maxx), int(maxy)));
	else
		return NULL;
}
