...
class BmSpamFilter : public BmFilterAddon {
	...
	public:
		...
		void JobSpecs(const BMessage* jobSpecs) { mJobSpecs = jobSpecs; }
	...
	private:
		...
		typedef struct {
			...
			bool HashCompare(uint32 h, uint32 k) const { return hash == h && key == k; }
	...
	public:
		static void Microgroom(FeatureBucket* hash, Header* header, uint32 hindex);

	private:
		static void PackData(Header* header, FeatureBucket* hash, uint32 packstart, uint32 packlen);
		static void PackDataSeg(
		...
		struct FeatureClassifier : public BmMemBufConsumer::Functor {
			FeatureClassifier(FeatureBucket* spamHash, Header* spamHeader, FeatureBucket* tofuHash,
				Header* tofuHeader);
			~FeatureClassifier();
		...
		status_t ReadDataFile(const BmString& filename, Header& header, FeatureBucket*& hash);
		status_t WriteDataFile(const BmString& filename, Header& header, FeatureBucket*& hash);
