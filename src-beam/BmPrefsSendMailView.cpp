...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmSendAccItem::BmSendAccItem(ColumnListView* lv, BmListModelItem* _item)
...
\*------------------------------------------------------------------------------*/
void
BmSendAccItem::UpdateView(BmUpdFlags flags, bool redraw, uint32 updColBitmap)
{
	BmSendAcc* acc = ModelItem();
	if (flags & UPD_ALL) {
		const char* cols[] = {acc->Key().String(), acc->SMTPServer().String(),
			acc->EncryptionType().String(), acc->AuthMethod().String(), acc->Username().String(),
			acc->PwdStoredOnDisk() ? "*****" : "", acc->DomainToAnnounce().String(),
			acc->PortNrString().String(), NULL};
		SetTextCols(0, cols);
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmSendAccView::CreateListViewItem(BmListModelItem* item, BMessage*)
{
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmSendAccView::AddModelItem(BmListModelItem* item)
{
...
\*------------------------------------------------------------------------------*/
void
BmSendAccView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			default:
	  ...
	  mClientCertPanel(NULL)
{
	MView* view = new VGroup(
		new BetterScrollView(minimax(400, 60, 1E5, 1E5), mAccListView = new BmSendAccView(400, 80),
			BM_SV_H_SCROLLBAR | BM_SV_V_SCROLLBAR | BM_SV_CORNER | BM_SV_CAPTION, "99 accounts"),
		new HGroup(mAddButton = new BetterButton("Add account", new BMessage(BM_ADD_ACCOUNT), this),
			mRemoveButton
			...
			new VGroup(
				new MBorder(M_LABELED_BORDER, 10, (char*)"Account info",
					new VGroup(mAccountControl = new BmTextControl("Account name:", false, 0, 25),
						...
						new HGroup(mServerControl = new BmTextControl("SMTP Server / Port:"),
						...
						mPopControl = new BmMenuControl(
							"POP3 account:", new BmMenuController("POP3 account:", this,
						...
						new Space(minimax(0, 5, 0, 5)),
						mDomainControl = new BmTextControl("Domain to announce:"), 0)),
				new MBorder(M_LABELED_BORDER, 10, (char*)"Encryption & security",
					new VGroup(
						...
						0)),
				0),
			new VGroup(new MBorder(M_LABELED_BORDER, 10, (char*)"Options",
						   new VGroup(mStorePwdControl
									  = new BmCheckControl("Store password on disk (UNSAFE!)",
										  new BMessage(BM_PWD_STORED_CHANGED), this),
							   new Space(minimax(250, 0, 250, 0)), 0)),
				new Space(), 0),
			0),
		0);
	...
	BmDividable::DivideSame(mAccountControl, mDomainControl, mLoginControl, mClientCertControl,
		mServerControl, mEncryptionControl, mAuthControl, mPopControl, mAcceptedCertControl, NULL);
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSendMailView::Initialize()
{
	inherited::Initialize();

	TheBubbleHelper->SetHelp(
		mAccListView, "This listview shows every SMTP account you have defined.");
	...
	TheBubbleHelper->SetHelp(mDomainControl,
	...
	TheBubbleHelper->SetHelp(mPopControl,
	...
	if (TheNetEndpointRoster->SupportsEncryptionType(BmSmtpAccount::ENCR_TLS)) {
		AddItemToMenu(mEncryptionControl->Menu(),
			new BMenuItem(BmSmtpAccount::ENCR_AUTO, new BMessage(BM_ENCRYPTION_SELECTED)), this);
		AddItemToMenu(mEncryptionControl->Menu(),
			new BMenuItem(BmSmtpAccount::ENCR_STARTTLS, new BMessage(BM_ENCRYPTION_SELECTED)),
			this);
		AddItemToMenu(mEncryptionControl->Menu(),
			new BMenuItem(BmSmtpAccount::ENCR_TLS, new BMessage(BM_ENCRYPTION_SELECTED)), this);
	}
	if (TheNetEndpointRoster->SupportsEncryptionType(BmSmtpAccount::ENCR_SSL)) {
		AddItemToMenu(mEncryptionControl->Menu(),
			new BMenuItem(BmSmtpAccount::ENCR_SSL, new BMessage(BM_ENCRYPTION_SELECTED)), this);
	}
...
\*------------------------------------------------------------------------------*/
bool
BmPrefsSendMailView::SanityCheck()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSendMailView::WriteStateInfo()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSendMailView::UndoChanges()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSendMailView::EncryptionSelected()
{
	if (!mCurrAcc)
		return;
	...
	if (encryptionType == BmSmtpAccount::ENCR_TLS || encryptionType == BmSmtpAccount::ENCR_SSL) {
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSendMailView::AuthTypeSelected()
{
	if (!mCurrAcc)
		return;
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSendMailView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			...
			case BM_ADD_ACCOUNT:
			{
				...
				TheSmtpAccountList->AddItemToList(
					new BmSmtpAccount(key.String(), TheSmtpAccountList.Get()));
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSendMailView::ShowAccount(int32 selection)
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSendMailView::UpdateState()
{
	...
	if (!accSelected) {
		...
		mLoginControl->SetEnabled(
			mCurrAcc->AuthMethod().Length() && !mCurrAcc->NeedsAuthViaPopServer());
		bool pwdEnabled = mCurrAcc->PwdStoredOnDisk() && mCurrAcc->AuthMethod().Length()
