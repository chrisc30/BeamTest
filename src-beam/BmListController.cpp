...
BmListViewItem::BmListViewItem(
	ColumnListView* lv, BmListModelItem* modelItem, bool, BMessage* archive)
...
\*------------------------------------------------------------------------------*/
status_t
BmListViewItem::Archive(BMessage* archive, bool) const
{
...
\*------------------------------------------------------------------------------*/
void
BmListViewItem::UpdateView(BmUpdFlags flags, bool redraw, uint32 updColBitmap)
{
...
\*------------------------------------------------------------------------------*/
void
BmListViewItem::SetTextCols(int16 firstTextCol, const char** content)
{
...
\*------------------------------------------------------------------------------*/
void
BmViewItemManager::Add(const BmListModelItem* modelItem, BmListViewItem* viewItem)
{
	if (!viewItem)
		return;
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmViewItemManager::Remove(const BmListModelItem* modelItem)
{
...
BmViewItemManager::FindViewItemFor(const BmListModelItem* modelItem) const
{
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("BmViewItemManager::FindViewItemFor(): Unable to get lock");
...
\*------------------------------------------------------------------------------*/
bool
BmViewItemManager::ApplyFilter(
	BmViewItemFilter* filter, ContinueCallback& continueCallback, BmListViewController* listView)
{
	...
	{  // scope for lock
		BAutolock lock(mLocker);
		if (!lock.IsLocked())
			BM_THROW_RUNTIME("BmViewItemManager::ApplyFilter(): Unable to get lock");
	...
	*/
	...
	if (!listLock.IsLocked())
		BM_THROW_RUNTIME("BmViewItemManager::ApplyFilter(): Unable to get looper lock!");
	BAutolock lock(mLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME("BmViewItemManager::ApplyFilter(): Unable to get lock");
...
BmListViewController::BmListViewController(
	BRect rect, const char* Name, list_view_type Type, bool hierarchical, bool showLabelView)
	: inherited(rect, Name, B_WILL_DRAW | B_FRAME_EVENTS | B_NAVIGABLE, Type, hierarchical,
		  showLabelView),
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::MouseDown(BPoint point)
{
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::MouseUp(BPoint point)
{
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::MouseMoved(BPoint point, uint32 transit, const BMessage* msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::HighlightItemAt(const BPoint& point)
{
	int32 index;
...
\*------------------------------------------------------------------------------*/
BBitmap*
BmListViewController::CreateDragImage(const vector<int>& cols, int32 max)
{
	int32 currIdx;
	...
	for (selCount = 0; (currIdx = CurrentSelection(selCount)) >= 0; ++selCount)
		;
	BM_LOG2(BM_LogGui, BmString("CreateDragImage() - found ") << selCount << " selections");
	const float v = 5;
	const float h = 5;
	...
	for (int32 i = 0; (currIdx = CurrentSelection(i)) >= 0; ++i) {
		...
		if (i == max - 1 && selCount > max) {
			...
			BmString indicator = BmString("(...and ")
								 << selCount - max
								 << (selCount - max == 1 ? " more item)" : " more items)");
			dummyView->DrawString(
				indicator.String(), BPoint(h, v + float(i) * lineHeight + baselineOffset));
			...
			for (uint32 c = 0; c < cols.size(); ++c) {
				...
				if ((col->Type() & CLV_COLTYPE_MASK) == CLV_COLTYPE_BITMAP) {
					const BmBitmapHandle* icon = item->GetColumnContentBitmap(cols[c]);
					if (icon && icon->bitmap) {
					...
					if (text)
						dummyView->DrawString(
							text, BPoint(h + hOffs, v + float(i) * lineHeight + baselineOffset));
				}
				hOffs += col->Width();
			}
		}
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::HandleDrop(BMessage*)
{
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::MessageReceived(BMessage* msg)
{
	try {
...
BmListViewController::AddAllModelItems()
{
	...
	if (!lock.IsLocked())
		BM_THROW_RUNTIME(model->ModelNameNC() << ": Unable to get lock");
	BList* tempList = NULL;
	if (!Hierarchical())
	...
	for (iter = model->begin(); iter != end; ++iter) {
		...
		if (Hierarchical()) {
			...
			if (viewItem) {
				...
				BM_LOG2(BM_LogMailTracking, BmString("ListView <")
												<< ModelName() << "> added view-item "
												<< viewItem->Key());
			} else
				BM_LOG(BM_LogMailTracking, BmString("ListView <")
											   << ModelName()
		...
		if (count % 100 == 0) {
			...
			BmString caption = BmString()
							   << count << " " << ItemNameForCaption() << (count > 1 ? "s" : "");
	...
	UpdateCaption();
	BM_LOG2(
		BM_LogModelController, BmString(ControllerName()) << ": finished with sorting added items");
}
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmListViewController::AddModelItem(BmListModelItem* item)
{
	BmListModel* model = dynamic_cast<BmListModel*>(DataModel().Get());
	BM_ASSERT(model);
	BmAutolockCheckGlobal lock(model->ModelLocker());
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmListViewController::doAddModelItem(BmListViewItem* parent, BmListModelItem* item, bool redraw)
{
	std::auto_ptr<BMessage> archive(Hierarchical() ? GetArchiveForItemKey(item->Key()) : NULL);
	...
	if (newItem) {
		...
		BM_LOG2(BM_LogMailTracking, BmString("ListView <")
										<< ModelName() << "> added view-item " << newItem->Key());
	}
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::RemoveModelItem(BmListModelItem* item)
{
	BM_LOG2(
		BM_LogModelController, BmString(ControllerName()) << ": removing one item from listview");
	if (item) {
		BmListModel* model = dynamic_cast<BmListModel*>(DataModel().Get());
		BM_ASSERT(model);
		BmAutolockCheckGlobal lock(model->ModelLocker());
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::doRemoveModelItem(BmListModelItem* item)
{
	BmListViewItem* viewItem = mViewItemManager.Remove(item);
	if (viewItem) {
		RemoveItem(viewItem);
		BM_LOG2(BM_LogMailTracking,
			BmString("ListView <") << ModelName() << "> removed view-item " << viewItem->Key());
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmListViewController::UpdateModelItem(BmListModelItem* item, BmUpdFlags updFlags)
{
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::UpdateModelState(BMessage*)
{
}
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::UpdateCaption(const char* text)
{
	if (fScrollView) {
		if (text) {
			...
			if (!numItems)
				...
				caption = BmString("")
						  << numItems << " " << ItemNameForCaption() << (numItems > 1 ? "s" : "");
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::ItemInvoked(int32)
{
}
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::ShowOrHideColumn(BMessage* msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::PopulateLabelViewMenu(BMenu* menu)
{
	...
	for (int32 i = 0; i < numCols; ++i) {
		...
		if (flags & (CLV_EXPANDER | CLV_LOCK_WITH_RIGHT | CLV_MERGE_WITH_RIGHT | CLV_NOT_MOVABLE))
			continue;
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::ShowLabelViewMenu(BPoint point)
{
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::ExpansionChanged(CLVListItem* _item, bool)
{
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::UpdateItem(BmListViewItem* item, BmUpdFlags flags)
{
	if (!item)
		return;
	if (LockLooper()) {
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::AttachModel(BmDataModel* model)
{
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::DetachModel()
{
	WriteStateInfo();
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::MakeEmpty()
{
	if (LockLooper()) {
...
\*------------------------------------------------------------------------------*/
bool
BmListViewController::ApplyViewItemFilter(
	BmViewItemFilter* filter, BmViewItemManager::ContinueCallback& continueCallback)
{
...
\*------------------------------------------------------------------------------*/
bool
BmListViewController::ApplyModelItemFilter(BmListModelItemFilter* filter)
{
	bool result = false;
	if (LockLooper()) {
		BmListModel* model = dynamic_cast<BmListModel*>(DataModel().Get());
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::StartJob(BmJobModel* model, bool startInNewThread, int32 jobSpecifier)
{
	if (fScrollView)
		fScrollView->SetBusy();
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::JobIsDone(bool completed)
{
	if (completed) {
		AddAllModelItems();
	} else {
...
\*------------------------------------------------------------------------------*/
void
BmListViewController::WriteStateInfo()
{
	status_t err;
	BFile stateInfoFile;
	BMessage archive;
	...
	try {
		...
		if ((err = stateInfoFile.SetTo(BeamRoster->StateInfoFolder(), stateInfoFilename.String(),
...
\*------------------------------------------------------------------------------*/
status_t
BmListViewController::Archive(BMessage* archive, bool deep) const
{
...
\*------------------------------------------------------------------------------*/
BMessage*
BmListViewController::GetArchiveForItemKey(const BmString& key, BMessage* msg)
{
...
\*------------------------------------------------------------------------------*/
BmString
BmListViewController::StateInfoFilename(bool forRead)
{
...
BmListViewController::ReadStateInfo()
{
	...
	if (mUseStateCache
		&& (err = stateInfoFile.SetTo(
				BeamRoster->StateInfoFolder(), stateInfoFilename.String(), B_READ_ONLY))
			   != B_OK) {
		...
		BEntry entry(BeamRoster->StateInfoFolder(), oldStateInfoFilename.String());
