...
BmMailFolderItem::BmMailFolderItem(
	ColumnListView* lv, BmListModelItem* _item, bool, BMessage* archive)
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderItem::UpdateView(BmUpdFlags flags, bool redraw, uint32 updColBitmap)
{
...
\*------------------------------------------------------------------------------*/
int
BmMailFolderItem::CompareItems(
	const CLVListItem* a_Item1, const CLVListItem* a_Item2, int32 KeyColumn, int32 col_flags)
{
	if (ThePrefs->GetBool("InOutAlwaysAtTop", false)) {
		...
		if (folder1->Name().ICompare(BmMailFolder::IN_FOLDER_NAME) == 0 && folder1FollowsTop)
			return -1 * rev;
		if (folder2->Name().ICompare(BmMailFolder::IN_FOLDER_NAME) == 0 && folder2FollowsTop)
			return 1 * rev;
		if (folder1->Name().ICompare(BmMailFolder::OUT_FOLDER_NAME) == 0 && folder1FollowsTop)
			return -1 * rev;
		if (folder2->Name().ICompare(BmMailFolder::OUT_FOLDER_NAME) == 0 && folder2FollowsTop)
			return 1 * rev;
		if (folder1->Name().ICompare(BmMailFolder::DRAFT_FOLDER_NAME) == 0 && folder1FollowsTop)
			return -1 * rev;
		if (folder2->Name().ICompare(BmMailFolder::DRAFT_FOLDER_NAME) == 0 && folder2FollowsTop)
			return 1 * rev;
		...
		if (folder2->Name().ICompare(BmMailFolder::QUARANTINE_FOLDER_NAME) == 0
			&& folder2FollowsTop)
			return 1 * rev;
		if (folder1->Name().ICompare(BmMailFolder::SPAM_FOLDER_NAME) == 0 && folder1FollowsTop)
			return -1 * rev;
		if (folder2->Name().ICompare(BmMailFolder::SPAM_FOLDER_NAME) == 0 && folder2FollowsTop)
			return 1 * rev;
	}
...
\*------------------------------------------------------------------------------*/
BmBitmapHandle*
BmMailFolderItem::GetExpanderBitmap(bool expanded)
{
	BmMailFolder* folder(ModelItem());
...
\*------------------------------------------------------------------------------*/
BmMailFolderView*
BmMailFolderView::CreateInstance(int32 width, int32 height)
{
	if (theInstance)
		return theInstance;
	else
	  ...
	  mHaveSelectedFolder(false)
{
	AddColumn(new CLVColumn(NULL, 14.0,
		CLV_EXPANDER | CLV_LOCK_AT_BEGINNING | CLV_NOT_MOVABLE | CLV_COLTYPE_BITMAP, 14.0));
	AddColumn(new CLVColumn(NULL, 18.0,
		CLV_LOCK_AT_BEGINNING | CLV_NOT_MOVABLE | CLV_NOT_RESIZABLE | CLV_PUSH_PASS
			| CLV_MERGE_WITH_RIGHT | CLV_COLTYPE_BITMAP,
		18.0));
	AddColumn(new CLVColumn(
		"Folders", 100.0, CLV_SORT_KEYABLE | CLV_LOCK_AT_BEGINNING | CLV_NOT_MOVABLE, 50.0));
...
\*------------------------------------------------------------------------------*/
status_t
BmMailFolderView::Archive(BMessage* archive, bool deep) const
{
...
\*------------------------------------------------------------------------------*/
status_t
BmMailFolderView::Unarchive(const BMessage* archive, bool deep)
{
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmMailFolderView::CreateListViewItem(BmListModelItem* item, BMessage* archive)
{
...
\*------------------------------------------------------------------------------*/
bool
BmMailFolderView::AcceptsDropOf(const BMessage* msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderView::HandleDrop(BMessage* msg)
{
	static int jobNum = 1;
	type_code tc;
	int32 refCount;
	if (msg && mCurrHighlightItem && (msg->what == B_SIMPLE_DATA)
		...
		BmRef<BmMailFolder> folder(dynamic_cast<BmMailFolder*>(mCurrHighlightItem->ModelItem()));
		if (folder) {
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderView::MouseDown(BPoint point)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderView::JobIsDone(bool completed)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderView::KeyDown(const char* bytes, int32 numBytes)
{
	if (numBytes == 1) {
		switch (bytes[0]) {
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderView::MessageReceived(BMessage* msg)
{
	try {
			...
			case BMM_NEW_MAILFOLDER:
			{
				...
				if (msg->FindInt32("which", &buttonPressed) != B_OK) {
					...
					TextEntryAlert* alert = new TextEntryAlert(
						"New Mail-Folder", "Enter name of new folder:", "", "Cancel", "OK");
			...
			case BMM_RENAME_MAILFOLDER:
			{
				...
				if (msg->FindInt32("which", &buttonPressed) != B_OK) {
					...
					TextEntryAlert* alert = new TextEntryAlert("Rename Mail-Folder",
						"Enter new name for folder:", folder->Name().String(), "Cancel", "OK");
					alert->SetShortcut(0, B_ESCAPE);
				...
				if (msg->FindInt32("which", &buttonPressed) != B_OK) {
					...
					BAlert* alert = new BAlert("Trash mail folder",
						(BmString("Are you sure about trashing the folder <")
							<< folder->Name() << ">?")
							.String(),
						"Move to Trash", "Cancel", NULL, B_WIDTH_AS_USUAL, B_WARNING_ALERT);
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderView::SelectionChanged(void)
{
	if (!mPartnerMailRefView)
		return;
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderView::SendNoticesIfNeeded(bool haveSelectedFolder)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderView::ItemInvoked(int32 index)
{
...
\*------------------------------------------------------------------------------*/
BmRef<BmMailFolder>
BmMailFolderView::CurrentFolder(void) const
{
...
\*------------------------------------------------------------------------------*/
void
BmMailFolderView::ShowMenu(BPoint point)
{
	BmRef<BmMailFolder> folder = CurrentFolder();
	...
	BMenuItem* item;
	if (folder) {
		item = new BMenuItem("New folder" B_UTF8_ELLIPSIS, new BMessage(BMM_NEW_MAILFOLDER));
		item->SetTarget(this);
		theMenu->AddItem(item);

		item = new BMenuItem("Rename folder" B_UTF8_ELLIPSIS, new BMessage(BMM_RENAME_MAILFOLDER));
		item->SetTarget(this);
		theMenu->AddItem(item);

		item = new BMenuItem("Delete folder" B_UTF8_ELLIPSIS, new BMessage(BMM_DELETE_MAILFOLDER));
		item->SetTarget(this);
		theMenu->AddItem(item);

		theMenu->AddSeparatorItem();

		item = new BMenuItem("Connect listview-layout to parent", new BMessage(BMM_CONNECT_LAYOUT));
		folder = CurrentFolder();
		...
		item = new BMenuItem("Recreate cache", new BMessage(BMM_RECACHE_MAILFOLDER));
		item->SetTarget(this);
		theMenu->AddItem(item);
	}
