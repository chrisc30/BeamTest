...
\*------------------------------------------------------------------------------*/
BmBitmapHandle*
BmResources::IconByName(const BmString name)
{
		...
		if (mt.InitCheck() == B_OK) {
			...
			if (mt.GetIcon(bitmap, B_MINI_ICON) != B_OK) {
				...
				if (mt.GetPreferredApp(preferredAppName) != B_OK
					...
					|| preferredApp.GetIconForType(name.String(), bitmap, B_MINI_ICON) != B_OK) {
...
\*------------------------------------------------------------------------------*/
void
BmResources::AddFontSubmenuTo(BMenu* menu, BHandler* target, BFont* selectedFont)
{
	if (!menu)
		return;
...
\*------------------------------------------------------------------------------*/
float
BmResources::FontBaselineOffset(const BFont* font)
{
	font_height fh;
	if (!font)
...
\*------------------------------------------------------------------------------*/
float
BmResources::FontHeight(const BFont* font)
{
	font_height fh;
	if (!font)
		fh = BePlainFontHeight;
...
\*------------------------------------------------------------------------------*/
float
BmResources::FontLineHeight(const BFont* font)
{
	font_height fh;
	if (!font)
		fh = BePlainFontHeight;
...
\*------------------------------------------------------------------------------*/
BPicture*
BmResources::CreatePictureFor(BBitmap* image, float width, float height, bool transparentBack,
	color_which backgroundCol, BmPicFrameType frameType, float offset)
{
	...
	if (image) {
		...
		view->DrawBitmap(image,
			BPoint((width - imageWidth) / 2.0f + offset, (height - imageHeight) / 2.0f + offset));
