...
BmMailMover::BmMailMover(
	const BmString& name, entry_ref* refs, int32 refCount, BmMailFolder* destFolder)
	: BmJobModel(name),
	  mRefs(refs),
	  mRefCount(refCount),
		...
		for (i = 0; ShouldContinue() && i < mRefCount; ++i) {
			...
			if (err == B_FILE_EXISTS) {
				...
				while ((err = entry.MoveTo(
							&destDir, (BmString(ref->name) << "_" << counter++).String()))
					   == B_FILE_EXISTS)
					;
			}
			if (err != B_OK)
...
\*------------------------------------------------------------------------------*/
void
BmMailMover::UpdateStatus(const float delta, const char* filename, const char* currentCount)
{
