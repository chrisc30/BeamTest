...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmBodyPartItem::BmBodyPartItem(ColumnListView* lv, BmListModelItem* _item)
	: inherited(lv, _item)
{
	...
	BmString sizeString = bodyPart->IsMultiPart() ? BM_DEFAULT_STRING
	...
	// set column-values:
	const char* cols[]
		= {bodyPart->FileName().String(), bodyPart->MimeType().String(), sizeString.String(),
			bodyPart->Description().String(), bodyPart->TransferEncoding().String(),
			bodyPart->IsText() ? bodyPart->SuggestedCharset().String() : "",
			bodyPart->Language().String(), NULL};
...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmBodyPartView::BmBodyPartView(minimax minmax, int32 width, int32 height, bool editable)
	  ...
	  mInUpdate(false)
{
	...
	AddColumn(new CLVColumn("Icon", nColWidths[1], CLV_PUSH_PASS | CLV_COLTYPE_BITMAP, 18.0));
...
\*------------------------------------------------------------------------------*/
void
BmBodyPartView::AddAttachment(const BMessage* msg)
{
	if (!mEditable)
		return;
	entry_ref ref;
...
BmBodyPartView::AddAllModelItems()
{
	...
	for (int i = 0; i < count; ++i) {
		...
		for (int c = nFirstTextCol; c < CountColumns(); ++c) {
			...
			mColWidths[c] = (float)std::max(mColWidths[c],
...
\*------------------------------------------------------------------------------*/
void
BmBodyPartView::KeyDown(const char* bytes, int32 numBytes)
{
...
\*------------------------------------------------------------------------------*/
void
BmBodyPartView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			...
			case BM_BODYPARTVIEW_SRC_CHARSET:
			{
				...
				BmString utf8Text;
				const uint32 blockSize = max_c((int32)128, bodyPart->DecodedLength());
				...
				if (encoder.HadError() || encoder.HadToDiscardChars()) {
					BAlert* alert = new BAlert("Wrong source charset",
			...
			case BM_BODYPARTVIEW_DELETE_ATTACHMENT:
			{
				...
				if (msg->FindInt32("which", &buttonPressed) != B_OK) {
					...
					BmRef<BmDataModel> modelRef(DataModel());
					BmBodyPartList* bodyPartList = dynamic_cast<BmBodyPartList*>(modelRef.Get());
					if (bodyPartList && bodyPartList->Mail() && buttonPressed == 0) {
						int32 index;
						for (int32 i = 0; msg->FindInt32("sel_index", i, &index) == B_OK; ++i) {
							if (index < 0)
								break;
							BmBodyPartItem* bodyPartItem
								= dynamic_cast<BmBodyPartItem*>(FullListItemAt(index));
			...
			case BM_EXECUTE_COPY_TARGET:
			{
				...
				for (int32 i = 0; msg->FindString("name", i, &name) == B_OK; ++i) {
					if (msg->FindString("bpkey", i, &bpkey) != B_OK)
						continue;
					BmRef<BmListModelItem> listItem = bodyPartList->FindItemByKey(bpkey);
					BmBodyPart* bodyPart = dynamic_cast<BmBodyPart*>(listItem.Get());
					if (!bodyPart)
						return;
					if (bodyPart->MimeType().ICompare("text/x-email") == 0
						...
						mail.StoreIntoFile(&dir, name, BM_MAIL_STATUS_NEW, real_time_clock_usecs());
					} else {
...
\*------------------------------------------------------------------------------*/
void
BmBodyPartView::HandleDrop(BMessage* msg)
{
	entry_ref ref;
	if (msg->FindRef("refs", &ref) == B_OK)
		...
		if (find_directory(B_SYSTEM_TEMP_DIRECTORY, &tempPath, true) == B_OK
			...
			{
				BFile file(dropFileName.String(), B_READ_WRITE | B_CREATE_FILE | B_ERASE_FILE);
			}
...
\*------------------------------------------------------------------------------*/
void
BmBodyPartView::ItemInvoked(int32 index)
{
	BmBodyPartItem* bodyPartItem = dynamic_cast<BmBodyPartItem*>(FullListItemAt(index));
	if (bodyPartItem) {
		BmBodyPart* bodyPart(bodyPartItem->ModelItem());
		...
		if (realMT.ICompare(bodyPart->MimeType()) != 0) {
			...
			if (BeamRoster->IsSupportedEmailMimeType(bodyPart->MimeType())
				...
				BAlert* alert = new BAlert(
					"", s.String(), openReal.String(), openDeclared.String(), "Cancel");
		...
		if (beamApp->HandlesMimetype(realMT)) {
			...
			if (BmBodyPart::MimeTypeIsPotentiallyHarmful(realMT)) {
				BmString s(
					"The attachment may contain harmful content, "
			...
			if (doIt) {
				...
				if (res == B_OK && path.InitCheck() == B_OK && path.Path()) {
					...
					if (res != B_OK && res != B_ALREADY_RUNNING) {
						ShowAlert(BmString("Sorry, could not launch application for "
...
\*------------------------------------------------------------------------------*/
bool
BmBodyPartView::InitiateDrag(BPoint, int32 index, bool wasSelected)
{
	if (!wasSelected)
		return false;
...
\*------------------------------------------------------------------------------*/
void
BmBodyPartView::ShowMenu(BPoint point)
{
	...
	if (!mEditable) {
		item = new BMenuItem("Show All MIME-Bodies",
			new BMessage(
				mShowAllParts ? BM_BODYPARTVIEW_SHOWATTACHMENTS : BM_BODYPARTVIEW_SHOWALL));
		item->SetTarget(this);
		...
		item = new BMenuItem(
			"Save attachment as" B_UTF8_ELLIPSIS, new BMessage(BM_BODYPARTVIEW_SAVE_ATTACHMENT));
		item->SetTarget(this);
		theMenu->AddItem(item);
