...
public:
	BmPerson(BmPeopleList* model, const node_ref& nref, const BmString& name, const BmString& nick,
		const BmString& email, const BmString& groups, const entry_ref& eref, bool foreign);
	virtual ~BmPerson();
	...
	//
	static BmString GenerateMailAddr(
		const BmString& name, const BmString& email, const BmString& nick = BM_DEFAULT_STRING);
	...
	inline const BmStringVect& Emails() const { return mEmails; }
...
public:
	...
	// native methods:
	void AddPeopleToMenu(BMenu* menu, const BMessage& templateMsg, const char* addrField);
	...
	//
	status_t CreateNewPerson(const BmString& name, const BmString& email, entry_ref* eref = NULL);

	void GetEmailsFromPeopleFile(const entry_ref& eref, BmStringVect& emails);

	void AddAsKnownAddress(const BmString& addr);
	bool IsAddressKnown(const BmString& addr) const;
	BmKnownAddrSet::const_iterator KnownAddrBegin() { return mKnownAddrSet.begin(); }
	BmKnownAddrSet::const_iterator KnownAddrEnd() { return mKnownAddrSet.end(); }
...
private:
	...
	// native methods:
	BMenu* _CreateSubmenuForPersonMap(const BmPersonMap& personMap, const BMessage& templateMsg,
		const char* addrField, BmString label, BFont* font, bool createAllEntry = false);
