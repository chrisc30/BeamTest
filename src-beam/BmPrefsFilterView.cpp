...
\*------------------------------------------------------------------------------*/
void
BmFilterItem::UpdateView(BmUpdFlags flags, bool redraw, uint32 updColBitmap)
{
	BmFilter* filter(dynamic_cast<BmFilter*>(ModelItem()));
	if (flags & UPD_ALL) {
		const char* cols[] = {filter->Key().String(), filter->IsDisabled() ? "DISABLED" : "", NULL};
		SetTextCols(0, cols);
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmFilterView::CreateListViewItem(BmListModelItem* item, BMessage*)
{
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmFilterView::AddModelItem(BmListModelItem* item)
{
...
\*------------------------------------------------------------------------------*/
void
BmFilterView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			default:
	  ...
	  mCurrAddonView(NULL)
{
	...
	char* k[10] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
	...
	for (iter = FilterAddonMap.begin(); iter != FilterAddonMap.end(); ++iter) {
		...
		if (func) {
			BmFilterAddonPrefsView* pview = (*func)(400, 200, 1E5, 1E5, iter->first);
	...
	MView* view = new UserResizeSplitView(
		new VGroup(minimax(200, 120),
			new HGroup(new BetterScrollView(minimax(200, 60, 1E5, 1E5),
				...
				new VGroup(
					new HGroup(mAddPopup = new MPopup(NULL, new BMessage(BM_ADD_FILTER), this, k[0],
								   k[1], k[2], k[3], k[4], k[5], k[6], k[7], k[8], k[9], NULL),
						mAddToChainControl
						= new BmCheckControl("Add new filter to default chain, too",
							new BMessage(BM_ADD_TO_CHAIN_CHANGED), this),
						0),
					new HGroup(mRemoveButton = new BetterButton("Remove filter",
								   new BMessage(BM_REMOVE_FILTER), this, minimax(-1, -1, -1, -1)),
						new Space(), 0),
					new Space(), 0),
				0),
			0),
		new VGroup(minimax(400, 300), mFilterControl = new BmTextControl("Filter name:"),
			...
			mLayeredAddonGroup = new LayeredGroup(
				...
				new VGroup(new Space(minimax(5, 0, 5, 0)), mInfoLabel = new MStringView(""),
					new Space(), 0),
				// now add up to 10 addon-views (this means that Beam currently
				// only supports 10 different filter-addon types):
				prefsView[0], prefsView[1], prefsView[2], prefsView[3], prefsView[4], prefsView[5],
				prefsView[6], prefsView[7], prefsView[8], prefsView[9], 0),
			0),
		"hsplitter", 180, B_HORIZONTAL, true, true, false, false, false, B_FOLLOW_NONE);
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterView::Initialize()
{
	inherited::Initialize();

	TheBubbleHelper->SetHelp(
		mFilterListView, "This list view shows every filter you have defined.");
	TheBubbleHelper->SetHelp(mFilterControl,
		"Here you can enter a name for this filter.\n"
		"This name is used to identify this filter in Beam.");
	TheBubbleHelper->SetHelp(mAddToChainControl,
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterView::WriteStateInfo()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			...
			case BM_ADD_FILTER:
			{
				...
				if (item) {
					...
					BmString defaultKey = TheFilterList->DefaultNameForFilterKind(filterKind);
					...
					BmFilter* newFilter
						= new BmFilter(key.String(), filterKind, TheFilterList.Get());
					...
					if (mAddToChainControl->Value()) {
						...
						if (chain) {
							BmChainedFilter* chainedFilter = new BmChainedFilter(
								newFilter->Key().String(), chain->ChainedFilters());
			...
			case BMM_CREATE_FILTER:
			{
				...
				BmFilter* newFilter = new BmFilter(key.String(), sieveKind, TheFilterList.Get());
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterView::ShowFilter(int32 selection)
{
				...
				if (mCurrFilter) {
					...
					for (int i = 0; i < mLayeredAddonGroup->CountChildren(); ++i) {
						BmFilterAddonPrefsView* view
							= dynamic_cast<BmFilterAddonPrefsView*>(mLayeredAddonGroup->ChildAt(i));
