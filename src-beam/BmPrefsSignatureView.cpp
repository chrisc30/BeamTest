...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmSignatureItem::BmSignatureItem(ColumnListView* lv, BmListModelItem* _item)
...
\*------------------------------------------------------------------------------*/
void
BmSignatureItem::UpdateView(BmUpdFlags flags, bool redraw, uint32 updColBitmap)
{
	...
	if (flags & UPD_ALL) {
		...
		const char* cols[]
			= {sig->Key().String(), sig->Dynamic() ? "*" : "", beautifiedContent.String(), NULL};
		SetTextCols(0, cols);
...
BmSignatureView::BmSignatureView(int32 width, int32 height)
	: inherited(BRect(0, 0, float(width - 1), float(height - 1)), "Beam_SignatureView",
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmSignatureView::CreateListViewItem(BmListModelItem* item, BMessage*)
{
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmSignatureView::AddModelItem(BmListModelItem* item)
{
...
\*------------------------------------------------------------------------------*/
void
BmSignatureView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			default:
	...
	: inherited("Signatures")
{
	MView* view = new VGroup(
		new BetterScrollView(minimax(400, 60, 1E5, 1E5),
			mSigListView = new BmSignatureView(400, 150),
			BM_SV_H_SCROLLBAR | BM_SV_V_SCROLLBAR | BM_SV_CORNER | BM_SV_CAPTION, "99 signatures"),
		new HGroup(
			mAddButton = new BetterButton("Add signature", new BMessage(BM_ADD_SIGNATURE), this),
			mRemoveButton
			= new BetterButton("Remove signature", new BMessage(BM_REMOVE_SIGNATURE), this),
			0),
		new Space(minimax(0, 10, 0, 10)),
		new MBorder(M_LABELED_BORDER, 10, (char*)"Signature info",
			new VGroup(mSignatureControl = new BmTextControl("Signature name:"),
				new Space(minimax(0, 5, 0, 5)),
				mContentControl = new BmMultiLineTextControl("Content:", false, 4, 0, true),
				new Space(minimax(0, 5, 0, 5)),
				new HGroup(mDynamicControl = new BmCheckControl(
							   "Content is dynamic (execute the given command via shell"
							   ", fetch stdout as signature)",
							   new BMessage(BM_DYNAMIC_CHANGED), this),
					new Space(),
					mTestButton = new BetterButton("Test this signature",
						new BMessage(BM_TEST_SIGNATURE), this, minimax(-1, -1, -1, -1)),
					0),
				new HGroup(mCharsetControl = new BmMenuControl("Charset:",
							   new BmMenuController("", this, new BMessage(BM_CHARSET_SELECTED),
								   &BmGuiRosterBase::RebuildCharsetMenu, BM_MC_LABEL_FROM_MARKED)),
					new Space(), 0),
				0)),
		new Space(minimax(0, 0, 1E5, 1E5, 0.1f)), 0);
	mGroupView->AddChild(dynamic_cast<BView*>(view));

	BmDividable::DivideSame(mSignatureControl, mContentControl, mCharsetControl, NULL);
}
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSignatureView::Initialize()
{
	inherited::Initialize();

	TheBubbleHelper->SetHelp(
		mSigListView, "This list view shows every signature you have defined.");
	...
	TheBubbleHelper->SetHelp(mCharsetControl,
	...
	BeamGuiRoster->AddCharsetMenu(mCharsetControl->Menu(), this, BM_CHARSET_SELECTED);
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSignatureView::Activated()
{
	inherited::Activated();
}
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSignatureView::WriteStateInfo()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSignatureView::UndoChanges()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSignatureView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			...
			case BM_ADD_SIGNATURE:
			{
				...
				TheSignatureList->AddItemToList(
					new BmSignature(key.String(), TheSignatureList.Get()));
...
\*------------------------------------------------------------------------------*/
void
BmPrefsSignatureView::ShowSignature(int32 selection)
{
