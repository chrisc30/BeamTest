	...
	: inherited("Logging messages")
{
	MView* view = new VGroup(new Space(minimax(0, 10, 0, 10)),
		new HGroup(
			new MBorder(M_LABELED_BORDER, 10, (char*)"Levels of logging",
				new VGroup(mLogPopControl = new BmMenuControl(
							   "Receiving mail:", new BPopUpMenu(""), 1.0, 0, BmPrefs::LOG_LVL_3),
					mLogSmtpControl = new BmMenuControl("Sending mail:", new BPopUpMenu("")),
			...
			new MBorder(M_LABELED_BORDER, 10, (char*)"Levels of debug logging",
				new VGroup(mLogMailTrackingControl = new BmMenuControl(
							   "Tracking mails:", new BPopUpMenu(""), 1.0, 0, BmPrefs::LOG_LVL_3),
					...
					= new BmMenuControl("Info transport:", new BPopUpMenu("")),
					mLogRefCountControl = new BmMenuControl("Memory:", new BPopUpMenu("")), 0)),
			new Space(), 0),
		new Space(minimax(0, 10, 0, 10)),
		new HGroup(new MBorder(M_LABELED_BORDER, 10, (char*)"Logfile sizes",
					   new VGroup(mMaxLogfileSizeControl
								  = new BmTextControl("Maximum size of logfile (KB):", false, 8),
						   mMinLogfileSizeControl
						   = new BmTextControl("Minimum size of logfile (KB):", false, 8),
						   0)),
			new Space(), 0),
		new Space(), 0);
	...
	BmDividable::DivideSame(
		mLogPopControl, mLogSmtpControl, mLogAppControl, mLogFilterControl, NULL);

	BmDividable::DivideSame(mLogMailParseControl, mLogMailTrackingControl, mLogRefCountControl,
		mLogJobWinControl, mLogGuiControl, mLogModelControllerControl, NULL);

	BmDividable::DivideSame(mMaxLogfileSizeControl, mMinLogfileSizeControl, NULL);
}
...
\*------------------------------------------------------------------------------*/
void
BmPrefsLoggingView::Initialize()
{
	inherited::Initialize();

	TheBubbleHelper->SetHelp(mLogPopControl,
	...
	TheBubbleHelper->SetHelp(mLogMailParseControl,
	...
	TheBubbleHelper->SetHelp(mLogJobWinControl,
		"This is for debugging only!\n"
		"If there seems to be a problem related to\n"
		"the status-window, increasing this setting\n"
		"may help to find out what is going wrong.");
	TheBubbleHelper->SetHelp(mLogModelControllerControl,
		"This is for debugging only!\n"
	...
	TheBubbleHelper->SetHelp(mLogRefCountControl,
		"This is for debugging only!\n"
	...
	// add log-levels:
	const char* logModes[]
		= {BmPrefs::LOG_LVL_0, BmPrefs::LOG_LVL_1, BmPrefs::LOG_LVL_2, BmPrefs::LOG_LVL_3, NULL};
	for (int32 i = 0; logModes[i]; ++i) {
		AddItemToMenu(mLogPopControl->Menu(),
			new BMenuItem(logModes[i], new BMessage(BM_LOG_POP_SELECTED)), this);
		AddItemToMenu(mLogSmtpControl->Menu(),
			new BMenuItem(logModes[i], new BMessage(BM_LOG_SMTP_SELECTED)), this);
		AddItemToMenu(mLogAppControl->Menu(),
			new BMenuItem(logModes[i], new BMessage(BM_LOG_APP_SELECTED)), this);
		AddItemToMenu(mLogFilterControl->Menu(),
			new BMenuItem(logModes[i], new BMessage(BM_LOG_FILTER_SELECTED)), this);
		AddItemToMenu(mLogMailParseControl->Menu(),
			new BMenuItem(logModes[i], new BMessage(BM_LOG_MAILPARSE_SELECTED)), this);
		AddItemToMenu(mLogMailTrackingControl->Menu(),
			new BMenuItem(logModes[i], new BMessage(BM_LOG_MAILTRACKING_SELECTED)), this);
		AddItemToMenu(mLogGuiControl->Menu(),
			new BMenuItem(logModes[i], new BMessage(BM_LOG_GUI_SELECTED)), this);
		AddItemToMenu(mLogJobWinControl->Menu(),
			new BMenuItem(logModes[i], new BMessage(BM_LOG_JOBWIN_SELECTED)), this);
		AddItemToMenu(mLogModelControllerControl->Menu(),
			new BMenuItem(logModes[i], new BMessage(BM_LOG_MODELCONTROLLER_SELECTED)), this);
		AddItemToMenu(mLogRefCountControl->Menu(),
			new BMenuItem(logModes[i], new BMessage(BM_LOG_REFCOUNT_SELECTED)), this);
	}
...
\*------------------------------------------------------------------------------*/
void
BmPrefsLoggingView::UndoChanges()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsLoggingView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
