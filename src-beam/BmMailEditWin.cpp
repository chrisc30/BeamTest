...
\*------------------------------------------------------------------------------*/
BmString
SelectEmailForPerson(const BmStringVect& emails)
{
	if (emails.empty())
...
static void
NoteOutboundAddresses(
	const BmAddressList& toList, const BmAddressList& ccList, const BmAddressList& bccList)
{
...
BMessageFilter*
CreateShiftTabMsgFilter(BControl* stControl, uint32 cmd)
{
...
BMessageFilter*
CreatePeopleDropMsgFilter(uint32 cmd)
{
...
\*------------------------------------------------------------------------------*/
filter_result
BmShiftTabMsgFilter::Filter(BMessage* msg, BHandler**)
{
...
\*------------------------------------------------------------------------------*/
filter_result
BmPeopleDropMsgFilter::Filter(BMessage* msg, BHandler** handler)
{
	...
	if (msg && msg->what == B_SIMPLE_DATA && cntrl) {
		...
		for (int32 i = 0; msg->FindRef("refs", i, &eref) == B_OK; ++i) {
			if (CheckMimeType(&eref, "application/x-person")) {
				...
				win->AddAddressToTextControl(dynamic_cast<BmTextControl*>(cntrl), email);
				res = B_SKIP_MESSAGE;
			}
		}
	}
...
\*------------------------------------------------------------------------------*/
BmMailEditWin*
BmMailEditWin::CreateInstance(BmMailRef* mailRef)
{
...
\*------------------------------------------------------------------------------*/
BmMailEditWin*
BmMailEditWin::CreateInstance(BmMail* mail)
{
...
\*------------------------------------------------------------------------------*/
status_t
BmMailEditWin::ArchiveState(BMessage* archive) const
{
...
\*------------------------------------------------------------------------------*/
status_t
BmMailEditWin::UnarchiveState(BMessage* archive)
{
...
BmMailEditWin::CreateGUI()
{
	...
	mOuterGroup = new VGroup(minimax(200, 300, 1E5, 1E5), CreateMenu(),
		mToolbar = new BmToolbar(new HGroup(minimax(-1, -1, 1E5, -1),
			mSendButton = new BmToolbarButton(
				"Send", width, height, new BMessage(BMM_SEND_NOW), this, "Send mail now", true),
			mSaveButton = new BmToolbarButton("Save", width, height, new BMessage(BMM_SAVE), this,
				"Save mail as draft (for later use)"),
			mNewButton = new BmToolbarButton("New", width, height, new BMessage(BMM_NEW_MAIL), this,
				"Compose a new mail message"),
			mAttachButton = new BmToolbarButton("Attach" B_UTF8_ELLIPSIS, width, height,
				new BMessage(BMM_ATTACH), this, "Attach a file to this mail", false, "Attachment"),
			new BmToolbarSpace(), 0)),
		new Space(minimax(-1, 4, -1, 4)),
		new HGroup(new Space(minimax(20, -1, 20, -1)),
			mFromControl = new BmTextControl(
				"From:", new BmMenuController("From:", this, new BMessage(BM_FROM_SET),
							 &BmGuiRosterBase::RebuildIdentityMenu, BM_MC_RADIO_MODE)),
			mCharsetControl = new BmMenuControl("Charset:",
				new BmMenuController("", this, new BMessage(BM_CHARSET_SELECTED),
					&BmGuiRosterBase::RebuildCharsetMenu, BM_MC_LABEL_FROM_MARKED),
				0.5),
			0),
		new HGroup(mShowDetails1Button = new MPictureButton(minimax(16, 16, 16, 16),
			...
			mToControl
			= new BmTextControl("To:", new BmMenuController("To:", this, new BMessage(BM_TO_ADDED),
										   &BmGuiRosterBase::RebuildPeopleMenu)),
			0),
		mDetails1Group
		= new VGroup(new HGroup(mShowDetails2Button = new MPictureButton(minimax(16, 16, 16, 16),
									TheResources->CreatePictureFor(rightArrow, 16, 16),
									TheResources->CreatePictureFor(downArrow, 16, 16),
									new BMessage(BM_SHOWDETAILS2), this, B_TWO_STATE_BUTTON),
						 new Space(minimax(4, -1, 4, -1)),
						 mCcControl = new BmTextControl(
							 "Cc:", new BmMenuController("Cc:", this, new BMessage(BM_CC_ADDED),
										&BmGuiRosterBase::RebuildPeopleMenu)),
						 mReplyToControl = new BmTextControl("Reply to:", false), 0),
			mDetails2Group = new HGroup(new Space(minimax(20, -1, 20, -1)),
				mBccControl = new BmTextControl(
					"Bcc:", new BmMenuController("Bcc:", this, new BMessage(BM_BCC_ADDED),
								&BmGuiRosterBase::RebuildPeopleMenu)),
				mSenderControl = new BmTextControl("Sender:", false), 0),
			0),
		mSubjectGroup = new HGroup(mShowDetails3Button = new MPictureButton(minimax(16, 16, 16, 16),
									   TheResources->CreatePictureFor(rightArrow, 16, 16),
									   TheResources->CreatePictureFor(downArrow, 16, 16),
									   new BMessage(BM_SHOWDETAILS3), this, B_TWO_STATE_BUTTON),
			new Space(minimax(4, -1, 4, -1)),
			mSubjectControl = new BmTextControl("Subject:", false), 0),
		mDetails3Group = new VGroup(
			new HGroup(new Space(minimax(20, -1, 20, -1)),
				mSignatureControl = new BmMenuControl("Signature:",
					new BmMenuController("Signature:", this, new BMessage(BM_SIGNATURE_SELECTED),
						&BmGuiRosterBase::RebuildSignatureMenu,
						BM_MC_ADD_NONE_ITEM | BM_MC_LABEL_FROM_MARKED)),
				new Space(minimax(20, -1, 20, -1)),
				mSmtpControl = new BmMenuControl("SMTP-Server:",
					new BmMenuController("", this, new BMessage(BM_SMTP_SELECTED),
						&BmGuiRosterBase::RebuildSmtpAccountMenu, BM_MC_LABEL_FROM_MARKED),
					0.5),
				0),
			new HGroup(new Space(minimax(20, -1, 20, -1)),
				mFileIntoControl = new BmMenuControl("Target Folder:",
					new BmMenuControllerBase("out", this, new BMessage(BM_FILEINTO_SELECTED),
						&BmGuiRosterBase::RebuildFolderMenu),
					2.0),
				new Space(minimax(20, -1, 20, -1)),
				mEditHeaderControl = new BmCheckControl("Edit headers", 1, false), 0),
			0),
		mSeparator = new Space(minimax(-1, 4, -1, 4)),
		new BmMailViewContainer(minimax(200, 200, 1E5, 1E5),
	...
	BmDividable::DivideSame(mToControl, mSubjectControl, mFromControl, mCcControl, mBccControl,
		mReplyToControl, mSenderControl, mSignatureControl, NULL);

	BmDividable::DivideSame(mSmtpControl, mCharsetControl, NULL);
	mCharsetControl->ct_mpm = mSmtpControl->ct_mpm;

	mShowDetails1Button->SetFlags(mShowDetails1Button->Flags() & ~B_NAVIGABLE);
	mShowDetails2Button->SetFlags(mShowDetails2Button->Flags() & ~B_NAVIGABLE);
	mShowDetails3Button->SetFlags(mShowDetails3Button->Flags() & ~B_NAVIGABLE);
...
\*------------------------------------------------------------------------------*/
void
BmMailEditWin::MessageReceived(BMessage* msg)
{
	int32 newVal;
	try {
		switch (msg->what) {
			...
			case BMM_SEND_NOW:
			{
				...
				if (!subject.Length()) {
					...
					if (msg->FindInt32("which", &buttonPressed) != B_OK) {
						...
						BAlert* alert = new BAlert("Empty subject",
							"The mail does not have a subject. Do you want to "
							"send it anyway?",
							"Send anyway", "Edit subject", NULL, B_WIDTH_AS_USUAL, B_WARNING_ALERT);
						alert->SetShortcut(1, B_ESCAPE);
						alert->Go(new BInvoker(new BMessage(msg->what), BMessenger(this)));
						break;
					} else {
			...
			case BM_TO_ADDED:
			{
				AddAddressToTextControl(mToControl, msg->FindString(BmListModel::MSG_ITEMKEY));
				break;
			}
			case BM_CC_ADDED:
			{
				AddAddressToTextControl(mCcControl, msg->FindString(BmListModel::MSG_ITEMKEY));
				break;
			}
			case BM_BCC_ADDED:
			{
				AddAddressToTextControl(mBccControl, msg->FindString(BmListModel::MSG_ITEMKEY));
				break;
			}
			case BM_TO_REMOVE:
			{
				RemoveAddressFromTextControl(mToControl, msg->FindString(MSG_ADDRESS));
				break;
			}
			case BM_CC_REMOVE:
			{
				RemoveAddressFromTextControl(mCcControl, msg->FindString(MSG_ADDRESS));
				break;
			}
			case BM_BCC_REMOVE:
			{
				RemoveAddressFromTextControl(mBccControl, msg->FindString(MSG_ADDRESS));
				break;
			}
			case BM_TO_CLEAR:
			{
			...
			case BMM_ATTACH:
			{
				...
				if (msg->FindRef("refs", 0, &attachRef) != B_OK) {
					...
					if (!mAttachPanel) {
						mAttachPanel = new BFilePanel(
							B_OPEN_PANEL, new BMessenger(this), NULL, B_FILE_NODE, true, msg);
					}
					mAttachPanel->Show();
				} else {
			...
			case BM_FILEINTO_SELECTED:
			{
				...
				if (item) {
					...
					BmString path;
					while (currMenu && currItem && currItem != mFileIntoControl->MenuItem()) {
						if (!path.Length())
...
\*------------------------------------------------------------------------------*/
void
BmMailEditWin::AddAddressToTextControl(BmTextControl* cntrl, const BmString& email)
{
	if (cntrl) {
...
\*------------------------------------------------------------------------------*/
void
BmMailEditWin::RemoveAddressFromTextControl(BmTextControl* cntrl, const BmString& email)
{
	if (cntrl) {
...
\*------------------------------------------------------------------------------*/
BmMailEditWin*
FindMailEditWinFor(const BmString& key)
{
...
\*------------------------------------------------------------------------------*/
void
AddMailEditWin(const BmString& key, BmMailEditWin* win)
{
	nEditWinMap[key] = win;
}
...
\*------------------------------------------------------------------------------*/
void
RemoveMailEditWin(BmMailEditWin* win)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailEditWin::RebuildPeopleMenu(BmMenuControllerBase* peopleMenu)
{
	BmMailEditWin* editWin = dynamic_cast<BmMailEditWin*>(peopleMenu->MsgTarget());
	if (!editWin)
		return;

	BMessage* msgTempl = peopleMenu->MsgTemplate();
	// add all adresses to menu and a menu-entry for clearing the field:
	ThePeopleList->AddPeopleToMenu(peopleMenu, *msgTempl, BmListModel::MSG_ITEMKEY);
	...
	if (currMail) {
		...
		for (iter = addrList.begin(); iter != addrList.end(); ++iter) {
			...
			removeMenu->AddItem(new BMenuItem((*iter).AddrString().String(), removeMsg));
		}
		if (removeMenu)
...
BmMailEditWin::EditHeaders()
{
	...
	float w = 600, h = 400;
	BRect alertFrame((screen.Width() - w) / 2, (screen.Height() - h) / 2, (screen.Width() + w) / 2,
		(screen.Height() + h) / 2);
	BmString headerStr;
	headerStr.ConvertLinebreaksToLF(&mail->Header()->HeaderString());
	TextEntryAlert* alert = new TextEntryAlert("Edit headers",
		"Please edit the mail headers below:", headerStr.String(), "Cancel", "OK, Send Message",
		false, 80, 20, B_WIDTH_FROM_LABEL, true, &alertFrame);
...
\*------------------------------------------------------------------------------*/
void
BmMailEditWin::SendMail(bool sendNow)
{
	if (mHasBeenSent)
...
\*------------------------------------------------------------------------------*/
void
BmMailEditWin::HandleFromSet(const BmString& from)
{
	BmRef<BmListModelItem> identRef = TheIdentityList->FindItemByKey(from);
...
\*------------------------------------------------------------------------------*/
void
BmMailEditWin::SetFieldsFromMail(BmMail* mail, BmIdentity* identity)
{
	if (mail) {
		...
		if (mail->IsRedirect()) {
			...
			fromAddrSpec = mail->Header()->GetAddressList(BM_FIELD_FROM).FirstAddress().AddrSpec();
		}
		if (!onlyIdentityFields) {
		...
		BmRef<BmIdentity> identRef
			= identity ? identity : TheIdentityList->FindIdentityForAddrSpec(fromAddrSpec);
		if (identRef)
...
\*------------------------------------------------------------------------------*/
void
BmMailEditWin::EditMail(BmMailRef* ref)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailEditWin::EditMail(BmMail* mail)
{
...
\*------------------------------------------------------------------------------*/
bool
BmMailEditWin::CreateMailFromFields(bool hardWrapIfNeeded)
{
	BmRef<BmMail> mail = mMailView->CurrMail();
	if (mail) {
		BmString editedText;
		...
		if (mail->IsRedirect()) {
			...
			NoteOutboundAddresses(mail->Header()->GetAddressList(BM_FIELD_RESENT_TO),
			...
			NoteOutboundAddresses(mail->Header()->GetAddressList(BM_FIELD_TO),
		...
		if (!mail->IsRedirect() && ThePrefs->GetBool("SetMailDateWithEverySave", true)) {
			mail->SetFieldVal(BM_FIELD_DATE, TimeToString(time(NULL), "%a, %d %b %Y %H:%M:%S %z"));
		}
		try {
...
\*------------------------------------------------------------------------------*/
void
BmMailEditWin::SetDetailsButton(int32 nr, int32 newVal)
{
	switch (nr) {
		case 1:
		{
...
\*------------------------------------------------------------------------------*/
bool
BmMailEditWin::SaveMail(bool saveForSend)
{
...
BmMailEditWin::QuitRequested()
{
	...
	if (mModified) {
		if (IsMinimized())
			Minimize(false);
		Activate();
		BAlert* alert = new BAlert("title", "Save mail as draft before closing?", "Cancel",
			"Don't save", "Save", B_WIDTH_AS_USUAL, B_OFFSET_SPACING, B_WARNING_ALERT);
		alert->SetShortcut(0, B_ESCAPE);
