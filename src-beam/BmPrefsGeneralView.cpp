	...
	MView* view = new VGroup(new Space(minimax(0, 10, 0, 10)),
		new HGroup(
			new MBorder(M_LABELED_BORDER, 10, (char*)"General GUI options",
				new VGroup(mWorkspaceControl
						   = new BmMenuControl("Workspace to start in:", new BPopUpMenu("")),
					new Space(minimax(0, 10, 0, 10)),
					mListviewLikeTrackerControl = new BmCheckControl("Simulate Tracker's list view",
						new BMessage(BM_LISTVIEW_LIKE_TRACKER_CHANGED), this,
						ThePrefs->GetBool("ListviewLikeTracker", false)),
					new Space(minimax(0, 10, 0, 10)),
					mShowTooltipsControl
					= new BmCheckControl("Show tooltips for toolbar buttons and prefs",
						new BMessage(BM_SHOW_TOOLTIPS_CHANGED), this,
						ThePrefs->GetBool("ShowTooltips", true)),
					0)),
			new MBorder(M_LABELED_BORDER, 10, (char*)"Toolbar options",
				new VGroup(mToolbarLabelControl
						   = new BmMenuControl("Layout of labels in toolbar:", new BPopUpMenu("")),
					new Space(minimax(0, 10, 0, 10)),
					new HGroup(mIconboxButton = new BetterButton(IconboxButtonLabel().String(),
								   new BMessage(BM_SELECT_ICONBOX), this, minimax(-1, -1, -1, -1)),
						new Space(), 0),
					0)),
			0),
		new HGroup(mMailboxButton = new BetterButton(MailboxButtonLabel().String(),
					   new BMessage(BM_SELECT_MAILBOX), this, minimax(-1, -1, -1, -1)),
			new Space(), 0),
		new Space(minimax(10, 0, 10, 0)),
		new HGroup(new BetterButton("Make Beam preferred app for email" B_UTF8_ELLIPSIS,
					   new BMessage(BM_MAKE_BEAM_STD_APP), this, minimax(-1, -1, -1, -1)),
			new Space(), 0),
		new Space(), 0);
...
\*------------------------------------------------------------------------------*/
BmString
BmPrefsGeneralView::MailboxButtonLabel()
{
...
\*------------------------------------------------------------------------------*/
BmString
BmPrefsGeneralView::IconboxButtonLabel()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsGeneralView::Initialize()
{
	inherited::Initialize();

	TheBubbleHelper->SetHelp(
		mWorkspaceControl, "In this menu you can select the workspace that Beam should live in.");
	TheBubbleHelper->SetHelp(mListviewLikeTrackerControl,
	...
	// add label-layouts:
	const char* layouts[] = {"Left", "Right", "Top", "Bottom", "Hide", NULL};
...
\*------------------------------------------------------------------------------*/
void
BmPrefsGeneralView::WriteStateInfo()
{
}
...
\*------------------------------------------------------------------------------*/
void
BmPrefsGeneralView::UndoChanges()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsGeneralView::SetDefaults()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsGeneralView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			...
			case BM_MAKE_BEAM_STD_APP:
			{
				...
				if (msg->FindInt32("which", &buttonPressed) != B_OK) {
					...
					BAlert* alert = new BAlert("Set Beam as preferred app",
						...
						"OK", "Cancel", NULL, B_WIDTH_AS_USUAL, B_WARNING_ALERT);
					...
					if (buttonPressed == 0) {
						...
						NoticeChange();
						BAlert* alert = new BAlert("Set Beam as preferred app",
							"Done, Beam is now the preferred application for email.", "OK", NULL,
			...
			case BM_SELECT_MAILBOX:
			{
				...
				if (msg->FindRef("refs", 0, &mailboxRef) != B_OK) {
					...
					if (!mMailboxPanel) {
						entry_ref eref;
						status_t err
							= get_ref_for_path(ThePrefs->GetString("MailboxPath").String(), &eref);
						mMailboxPanel = new BFilePanel(B_OPEN_PANEL, new BMessenger(this),
					...
					if (entry.GetPath(&path) == B_OK) {
						...
						BAlert* alert = new BAlert("Mailbox path",
							"Done, Beam will use the new mailbox after a restart", "OK", NULL, NULL,
			...
			case BM_SELECT_ICONBOX:
			{
				...
				if (msg->FindRef("refs", 0, &iconboxRef) != B_OK) {
					...
					if (!mIconboxPanel) {
						...
						mIconboxPanel = new BFilePanel(B_OPEN_PANEL, &msnger,
							err == B_OK ? &eref : NULL, B_DIRECTORY_NODE, false, msg);
					}
					mIconboxPanel->Show();
				} else {
