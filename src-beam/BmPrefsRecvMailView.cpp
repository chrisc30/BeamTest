...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmRecvAccItem::BmRecvAccItem(ColumnListView* lv, BmListModelItem* _item)
...
\*------------------------------------------------------------------------------*/
void
BmRecvAccItem::UpdateView(BmUpdFlags flags, bool redraw, uint32 updColBitmap)
{
	BmRecvAccount* acc = ModelItem();
	if (flags & UPD_ALL) {
		const char* cols[] = {acc->Key().String(), acc->Type(), acc->CheckMail() ? "*" : "",
			acc->DeleteMailFromServer() ? "*" : "", acc->FilterChain().String(),
			acc->Server().String(), acc->PortNrString().String(),
			acc->CheckIntervalString().String(), acc->EncryptionType().String(),
			acc->AuthMethod().String(), acc->Username().String(),
			acc->PwdStoredOnDisk() ? "*****" : "", NULL};
		SetTextCols(0, cols);
...
BmRecvAccView::BmRecvAccView(int32 width, int32 height)
	: inherited(BRect(0, 0, float(width - 1), float(height - 1)), "Beam_RecvAccView",
		  B_SINGLE_SELECTION_LIST, false, true)
{
	...
	AddColumn(new CLVColumn("R", 20.0, flags, 20.0, "(R)emove mails from server?"));
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmRecvAccView::CreateListViewItem(BmListModelItem* item, BMessage*)
{
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmRecvAccView::AddModelItem(BmListModelItem* item)
{
...
\*------------------------------------------------------------------------------*/
void
BmRecvAccView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			default:
	  ...
	  mClientCertPanel(NULL)
{
	MView* view = new VGroup(
		new BetterScrollView(minimax(-1, 100, 1E5, 1E5), mAccListView = new BmRecvAccView(400, 100),
			BM_SV_H_SCROLLBAR | BM_SV_V_SCROLLBAR | BM_SV_CORNER | BM_SV_CAPTION, "99 accounts"),
		new HGroup(mAddPopButton
				   = new BetterButton("Add POP3 account", new BMessage(BM_ADD_POP_ACCOUNT), this),
			mAddImapButton
			= new BetterButton("Add IMAP account", new BMessage(BM_ADD_IMAP_ACCOUNT), this),
			mRemoveButton
			= new BetterButton("Remove account", new BMessage(BM_REMOVE_ACCOUNT), this),
			0),
		new Space(minimax(0, 10, 0, 10)),
		new HGroup(
			new VGroup(
				new MBorder(M_LABELED_BORDER, 10, (char*)"Account info",
					new VGroup(mAccountControl = new BmTextControl("Account name:", false, 0, 25),
						new Space(minimax(0, 5, 0, 5)),
						new HGroup(mServerControl = new BmTextControl("Server / Port:"),
							mPortControl = new BmTextControl("", false, 0, 8), 0),
						new HGroup(
							mAuthControl = new BmMenuControl("Auth method:", new BPopUpMenu("")),
							mCheckAndSuggestButton = new BetterButton("Check and suggest",
								new BMessage(BM_CHECK_AND_SUGGEST), this, minimax(-1, -1, -1, -1)),
							0),
						new HGroup(mLoginControl = new BmTextControl("User / Pwd:"),
							mPwdControl = new BmTextControl("", false, 0, 8), 0),
						new Space(minimax(0, 5, 0, 5)),
						mHomeFolderControl = new BmMenuControl("Home folder:",
							new BmMenuController("", this, new BMessage(BM_HOME_FOLDER_SELECTED),
								&BmGuiRosterBase::RebuildFolderMenu, BM_MC_SKIP_FIRST_LEVEL)),
						mFilterChainControl = new BmMenuControl("Filter chain:",
							new BmMenuController("", this, new BMessage(BM_FILTER_CHAIN_SELECTED),
				...
				new MBorder(M_LABELED_BORDER, 10, (char*)"Encryption & security",
					new VGroup(
						mEncryptionControl = new BmMenuControl("Encryption:", new BPopUpMenu("")),
						new HGroup(mAcceptedCertControl = new BmTextControl("Server certificate:"),
							mClearAcceptedCertButton
							= new BetterButton("Clear", new BMessage(BM_CLEAR_ACCEPTED_CERT), this),
							0),
						new HGroup(mClientCertControl = new BmTextControl("Client certificate:"),
							mSelectClientCertButton = new BetterButton("Select" B_UTF8_ELLIPSIS,
								new BMessage(BM_CLIENT_CERT_SELECTED), this),
							0),
						0)),
				0),
			new VGroup(
				new MBorder(M_LABELED_BORDER, 10, (char*)"Options",
					new VGroup(mAutoCheckIfPppUpControl = new BmCheckControl(
								   "Automatically check for mails only if PPP is up",
								   new BMessage(BM_CHECK_IF_PPP_UP_CHANGED), this,
								   ThePrefs->GetBool("AutoCheckOnlyIfPPPRunning")),
						new Space(minimax(0, 10, 0, 10)),
						mCheckAccountControl = new BmCheckControl(
							"Include in manual check", new BMessage(BM_CHECK_MAIL_CHANGED), this),
						new HGroup(
							mCheckEveryControl = new BmCheckControl("Check automatically every",
								new BMessage(BM_CHECK_EVERY_CHANGED), this),
							mCheckIntervalControl = new BmTextControl("", false, 4),
							mMinutesLabel = new MStringView("minutes"), new Space(), 0),
						new HGroup(mRemoveMailControl
								   = new BmCheckControl("Remove mails from server after",
									   new BMessage(BM_REMOVE_MAIL_CHANGED), this),
							mDeleteMailDelayControl = new BmTextControl("", false, 4),
							mDaysLabel = new MStringView("days"), new Space(), 0),
						new Space(minimax(0, 10, 0, 10)),
						mStorePwdControl = new BmCheckControl("Store password on disk (UNSAFE!)",
							new BMessage(BM_PWD_STORED_CHANGED), this),
						0)),
				new Space(), 0),
			0),
		0);
	...
	BmDividable::DivideSame(mAccountControl, mLoginControl, mClientCertControl, mServerControl,
		mEncryptionControl, mAuthControl, mHomeFolderControl, mFilterChainControl,
		mAcceptedCertControl, NULL);
...
\*------------------------------------------------------------------------------*/
void
BmPrefsRecvMailView::Initialize()
{
	inherited::Initialize();

	TheBubbleHelper->SetHelp(
		mAccListView, "This list view shows every receiving account you have defined.");
	TheBubbleHelper->SetHelp(mAccountControl,
		"Here you can enter a name for this account.\n"
		"This name is used to identify this account in Beam.");
	TheBubbleHelper->SetHelp(mLoginControl,
		"Here you can enter the username which \n"
		"will be used during authentication.");
	TheBubbleHelper->SetHelp(mPwdControl,
		"Here you can enter the password which \n"
		"will be used during authentication.\n"
		"(You can only edit this field if you\n"
		"checked 'Store password on disk').");
	TheBubbleHelper->SetHelp(
		mClientCertControl, TheNetEndpointRoster->SupportsEncryption()
	...
	TheBubbleHelper->SetHelp(
		mSelectClientCertButton, TheNetEndpointRoster->SupportsEncryption()
	...
	TheBubbleHelper->SetHelp(mEncryptionControl, encrHelp.String());
	TheBubbleHelper->SetHelp(mAuthControl,
	...
	TheBubbleHelper->SetHelp(mHomeFolderControl,
	...
	AddItemToMenu(mEncryptionControl->Menu(),
		new BMenuItem(BM_NoItemLabel.String(), new BMessage(BM_ENCRYPTION_SELECTED)), this);
	if (TheNetEndpointRoster->SupportsEncryptionType(BmRecvAccount::ENCR_TLS)) {
		AddItemToMenu(mEncryptionControl->Menu(),
			new BMenuItem(BmRecvAccount::ENCR_AUTO, new BMessage(BM_ENCRYPTION_SELECTED)), this);
		AddItemToMenu(mEncryptionControl->Menu(),
			new BMenuItem(BmRecvAccount::ENCR_STARTTLS, new BMessage(BM_ENCRYPTION_SELECTED)),
			this);
		AddItemToMenu(mEncryptionControl->Menu(),
			new BMenuItem(BmRecvAccount::ENCR_TLS, new BMessage(BM_ENCRYPTION_SELECTED)), this);
	}
	if (TheNetEndpointRoster->SupportsEncryptionType(BmRecvAccount::ENCR_SSL)) {
		AddItemToMenu(mEncryptionControl->Menu(),
			new BMenuItem(BmRecvAccount::ENCR_SSL, new BMessage(BM_ENCRYPTION_SELECTED)), this);
	}
...
\*------------------------------------------------------------------------------*/
void
BmPrefsRecvMailView::WriteStateInfo()
{
...
\*------------------------------------------------------------------------------*/
bool
BmPrefsRecvMailView::SanityCheck()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsRecvMailView::UndoChanges()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsRecvMailView::EncryptionSelected()
{
	if (!mCurrAcc)
		return;
	...
	if (encryptionType == BmRecvAccount::ENCR_TLS || encryptionType == BmRecvAccount::ENCR_SSL) {
...
\*------------------------------------------------------------------------------*/
void
BmPrefsRecvMailView::AuthTypeSelected()
{
	if (!mCurrAcc)
		return;
...
\*------------------------------------------------------------------------------*/
void
BmPrefsRecvMailView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			...
			case BM_TEXTFIELD_MODIFIED:
			{
				if (mCurrAcc) {
					...
					if (source == mAccountControl) {
						...
						mCurrAcc->CheckInterval(int16(MAX(0, atoi(mCheckIntervalControl->Text()))));
			...
			case BM_CHECK_EVERY_CHANGED:
			{
				...
				mMinutesLabel->SetHighColor(tint_color(
					ui_color(B_PANEL_TEXT_COLOR), val ? B_NO_TINT : B_DISABLED_MARK_TINT));
			...
			case BM_REMOVE_MAIL_CHANGED:
			{
				if (mCurrAcc) {
					...
					mDaysLabel->SetHighColor(tint_color(
						ui_color(B_PANEL_TEXT_COLOR), val ? B_NO_TINT : B_DISABLED_MARK_TINT));
			...
			case BM_CHECK_AND_SUGGEST:
			{
				if (mCurrAcc) {
					...
					BmString encryptionType;
					BmPopAccount* popAcc = dynamic_cast<BmPopAccount*>(mCurrAcc.Get());
					if (popAcc) {
						BmRef<BmPopper> popper(new BmPopper(mCurrAcc->Key(), popAcc));
						popper->StartJobInThisThread(BmPopper::BM_CHECK_CAPABILITIES_JOB);
						...
						BmImapAccount* imapAcc = dynamic_cast<BmImapAccount*>(mCurrAcc.Get());
						if (imapAcc) {
							BmRef<BmImap> imap(new BmImap(mCurrAcc->Key(), imapAcc));
							imap->StartJobInThisThread(BmImap::BM_CHECK_CAPABILITIES_JOB);
			...
			case BM_CLIENT_CERT_SELECTED:
			{
				...
				if (msg->FindRef("refs", 0, &certRef) != B_OK) {
					...
					if (!mClientCertPanel) {
						...
						mClientCertPanel = new BFilePanel(B_OPEN_PANEL, new BMessenger(this),
			...
			case BM_ADD_IMAP_ACCOUNT:
			{
				...
				if (msg->what == BM_ADD_POP_ACCOUNT)
					recvAcc = new BmPopAccount(key.String(), TheRecvAccountList.Get());
				else
					recvAcc = new BmImapAccount(key.String(), TheRecvAccountList.Get());
				...
				if (msg->FindInt32("which", &buttonPressed) != B_OK) {
					...
					BAlert* alert = new BAlert("Remove mail account",
						(BmString("Are you sure about removing the account <")
							<< mCurrAcc->Name() << ">?")
							.String(),
						"Remove account and its identities", "Remove account, keep identities",
			...
			case BM_COMPLAIN_ABOUT_FIELD:
			{
				...
				if (msg->FindInt32("which", &buttonPressed) != B_OK) {
					...
					BAlert* alert = new BAlert("Sanity check failed", complaint.String(), "OK",
...
\*------------------------------------------------------------------------------*/
void
BmPrefsRecvMailView::ShowAccount(int32 selection)
{
	...
	if (selection == -1) {
		...
		mMinutesLabel->SetHighColor(tint_color(ui_color(B_PANEL_TEXT_COLOR), B_DISABLED_MARK_TINT));
		mDaysLabel->SetHighColor(tint_color(ui_color(B_PANEL_TEXT_COLOR), B_DISABLED_MARK_TINT));
		...
		if (accItem) {
			if (mCurrAcc != accItem->ModelItem()) {
				...
				if (mCurrAcc) {
					...
					mServerControl->SetTextSilently(mCurrAcc->Server().String());
					mEncryptionControl->MarkItem((mCurrAcc->EncryptionType().Length()
					...
					for (; authTypeIter != authTypes.end(); ++authTypeIter) {
						AddItemToMenu(mAuthControl->Menu(),
							new BMenuItem(authTypeIter->String(), new BMessage(BM_AUTH_SELECTED)),
							this);
					}
					mAuthControl->MarkItem(mCurrAcc->AuthMethod().String());
					mHomeFolderControl->MarkItem(mCurrAcc->HomeFolder().String());
					...
					mMinutesLabel->SetHighColor(tint_color(ui_color(B_PANEL_TEXT_COLOR),
						mCurrAcc->CheckInterval() > 0 ? B_NO_TINT : B_DISABLED_MARK_TINT));
					mDeleteMailDelayControl->SetEnabled(mCurrAcc->DeleteMailFromServer());
					mDaysLabel->SetHighColor(tint_color(ui_color(B_PANEL_TEXT_COLOR),
						mCurrAcc->DeleteMailFromServer() ? B_NO_TINT : B_DISABLED_MARK_TINT));
					mClientCertControl->SetTextSilently(mCurrAcc->ClientCertificate().String());
					mAcceptedCertControl->SetTextSilently(mCurrAcc->AcceptedCertID().String());
				}
			}
		} else
			mCurrAcc = NULL;
