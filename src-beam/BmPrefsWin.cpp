...
BmPrefsWin::BmPrefsWin()
	: inherited("PrefsWin", BRect(50, 50, 849, 449), "Beam Preferences", B_FLOATING_WINDOW_LOOK,
	  ...
	  mChanged(false)
{
	MView* mOuterGroup = new VGroup(minimax(-1, 550),
		new MBorder(M_RAISED_BORDER, 5, NULL,
			new HGroup(minimax(-1, 200),
				mVertSplitter
				= new UserResizeSplitView(new VGroup(minimax(100, 200, 200, 1E5),
											  new BetterScrollView(minimax(100, 200, 200, 1E5),
												  CreatePrefsListView(120, 200), BM_SV_V_SCROLLBAR),
											  new Space(minimax(0, 2, 0, 2)), 0),
					mPrefsViewContainer = new BmPrefsViewContainer(
						new LayeredGroup(new BmPrefsView(NULL), new BmPrefsGeneralView(),
							new BmPrefsShortcutsView(), new BmPrefsLoggingView(),
							new BmPrefsMailConstrView(), new BmPrefsSendMailView(),
							new BmPrefsMailReadView(), new BmPrefsRecvMailView(),
							new BmPrefsIdentityView(), new BmPrefsSignatureView(),
							new BmPrefsFilterView(), new BmPrefsFilterChainView(), 0)),
					"vsplitter", 150, B_VERTICAL, true, true, false, false, false, B_FOLLOW_NONE),
				0)),
		new Space(minimax(0, 10, 0, 10)),
		new HGroup(new Space(),
			mDefaultsButton = new BetterButton(
				"Defaults", new BMessage(BM_SET_DEFAULTS), this, minimax(-1, -1, -1, -1)),
			new Space(minimax(40, 0, 40, 0)),
			mRevertButton = new BetterButton(
				"Revert", new BMessage(BM_REVERT_CHANGES), this, minimax(-1, -1, -1, -1)),
			new Space(minimax(20, 0, 20, 0)),
			mSaveButton = new BetterButton(
				"Save", new BMessage(BM_SAVE_CHANGES), this, minimax(-1, -1, -1, -1)),
			new Space(minimax(20, 0, 20, 0)), 0),
		new Space(minimax(0, 10, 0, 10)), 0);
...
\*------------------------------------------------------------------------------*/
status_t
BmPrefsWin::ArchiveState(BMessage* archive) const
{
...
\*------------------------------------------------------------------------------*/
status_t
BmPrefsWin::UnarchiveState(BMessage* archive)
{
	float vDividerPos;
	status_t ret
		= inherited::UnarchiveState(archive) || archive->FindFloat(MSG_VSPLITTER, &vDividerPos);
	if (ret == B_OK) {
...
\*------------------------------------------------------------------------------*/
ColumnListView*
BmPrefsWin::CreatePrefsListView(int32 width, int32 height)
{
	mPrefsListView = new ColumnListView(BRect(0, 0, float(width - 1), float(height - 1)), NULL,
		B_WILL_DRAW | B_FRAME_EVENTS | B_NAVIGABLE, B_SINGLE_SELECTION_LIST, true, true);
	...
	mPrefsListView->SetMinItemHeight(MAX(TheResources->FontLineHeight(),
		float(ThePrefs->GetInt("ListviewHierarchicalMinItemHeight", 16))));
	mPrefsListView->AddColumn(new CLVColumn(NULL, 10.0,
		CLV_EXPANDER | CLV_LOCK_AT_BEGINNING | CLV_NOT_MOVABLE | CLV_COLTYPE_BITMAP, 10.0));
	mPrefsListView->AddColumn(new CLVColumn(
		"   Category", 300.0, CLV_NOT_MOVABLE | CLV_NOT_RESIZABLE | CLV_TELL_ITEMS_WIDTH, 300.0));
...
\*------------------------------------------------------------------------------*/
void
BmPrefsWin::SendMsgToSubView(const BmString& subViewName, BMessage* msg)
{
	int32 index = 0;
...
\*------------------------------------------------------------------------------*/
void
BmPrefsWin::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			case BM_SELECTION_CHANGED:
			{
				...
				int32 fullIndex
					= mPrefsListView->FullListIndexOf((CLVListItem*)mPrefsListView->ItemAt(index));
...
BmPrefsWin::QuitRequested()
{
	...
	if (mChanged) {
		...
		BAlert* alert = new BAlert("title",
			"The preferences have changed. Would you like to save the changes before closing?",
...
\*------------------------------------------------------------------------------*/
void
BmPrefsWin::PrefsListSelectionChanged(int32)
{
}
