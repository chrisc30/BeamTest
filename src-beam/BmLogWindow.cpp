...
\*------------------------------------------------------------------------------*/
BmLogWindow*
BmLogWindow::CreateAndStartInstanceFor(
	const char* logfileName, bool showUponNews, bool clingToMainWin)
{
	...
	title << logfileName;
	BmLogWindow* win = new BmLogWindow(frame, title, logfileName, showUponNews, clingToMainWin);
	if (showUponNews) {
...
/*------------------------------------------------------------------------------*\
	( )
		-
\*------------------------------------------------------------------------------*/
BmLogWindow::BmLogWindow(const BRect& frame, const BmString& title, const char* logfileName,
	bool showUponNews, bool clingToMainWin)
	: inherited(title.String(), frame, title.String(), B_FLOATING_WINDOW_LOOK, B_NORMAL_WINDOW_FEEL,
	  ...
	  mClingToMainWin(clingToMainWin)
{
	...
	AddChild(dynamic_cast<BView*>(new MScrollView(mLogView, false, true)));
}
...
\*------------------------------------------------------------------------------*/
void
BmLogWindow::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		case B_COPY:
		case B_CUT:
