	...
	MView* view = new VGroup(new Space(minimax(0, 10, 0, 10)),
		new HGroup(
			new MBorder(M_LABELED_BORDER, 10, (char*)"Mail editing",
				new VGroup(mMaxLineLenControl = new BmTextControl("Set right margin to (chars):"),
					mHardWrapControl = new BmCheckControl("Hard-wrap mail text at right margin",
						new BMessage(BM_HARD_WRAP_CHANGED), this,
						ThePrefs->GetBool("HardWrapMailText")),
					new Space(minimax(0, 10, 0, 10)),
					mQuotingStringControl = new BmTextControl("Quoting string:"),
					mQuoteFormattingControl
					= new BmMenuControl("Quote formatting:", new BPopUpMenu("")),
					new Space(minimax(0, 10, 0, 10)),
			...
			new VGroup(
				new MBorder(M_LABELED_BORDER, 10, (char*)"Character sets",
					new VGroup(
						...
						= new BmMenuControl("Frequently used charsets:", new BPopUpMenu("")),
						new Space(), 0)),
				new MBorder(M_LABELED_BORDER, 10, (char*)"People options",
					new VGroup(mLookInPeopleFolderControl
							   = new BmCheckControl("Look only in people folder",
								   new BMessage(BM_LOOK_IN_PEOPLE_CHANGED), this,
								   ThePrefs->GetBool("LookForPeopleOnlyInPeopleFolder", true)),
						new Space(minimax(0, 5, 0, 5)),
						mAddNameToPeopleControl
						= new BmCheckControl("Add people's names to mail address",
							new BMessage(BM_ADD_PEOPLE_NAME_CHANGED), this,
							ThePrefs->GetBool("AddPeopleNameToMailAddr", true)),
						new Space(minimax(0, 5, 0, 5)),
						new HGroup(mPeopleFolderButton
								   = new BetterButton(PeopleFolderButtonLabel().String(),
									   new BMessage(BM_SELECT_PEOPLE_FOLDER), this,
									   minimax(-1, -1, -1, -1)),
							new Space(), 0),
						0)),
				0),
		...
		new HGroup(
			new MBorder(M_LABELED_BORDER, 10, (char*)"Replying",
				new VGroup(mReplyIntroStrControl = new BmTextControl("Intro:"),
					mReplySubjectStrControl = new BmTextControl("Subject:"),
					mReplyIntroStrPrivateControl = new BmTextControl("Personal phrase:"), 0)),
			new MBorder(M_LABELED_BORDER, 10, (char*)"Forwarding",
				new VGroup(mForwardIntroStrControl = new BmTextControl("Intro:", false, 0, 23),
					mForwardSubjectStrControl = new BmTextControl("Subject:"), new Space(), 0)),
			0),
		new Space(minimax(0, 5, 0, 5)), new Space(), 0);
	mGroupView->AddChild(dynamic_cast<BView*>(view));

	BmDividable::DivideSame(mMaxLineLenControl, mQuotingStringControl, mQuoteFormattingControl,
		mDefaultCharsetControl, mUsedCharsetsControl, mForwardIntroStrControl,
		mForwardSubjectStrControl, mReplyIntroStrControl, mReplyIntroStrPrivateControl,
		mReplySubjectStrControl, mUndoModeControl, NULL);
...
\*------------------------------------------------------------------------------*/
BmString
BmPrefsMailConstrView::PeopleFolderButtonLabel()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsMailConstrView::Initialize()
{
	...
	TheBubbleHelper->SetHelp(mMaxLineLenControl,
	...
	TheBubbleHelper->SetHelp(mForwardIntroStrControl,
	...
	TheBubbleHelper->SetHelp(mForwardSubjectStrControl,
	...
	TheBubbleHelper->SetHelp(mReplyIntroStrControl,
		"Here you can enter a string that will \n"
		...
		"	%s  -  expands to the subject.\n"
		"	%t  -  expands to the time.");
	TheBubbleHelper->SetHelp(mReplyIntroStrPrivateControl,
	...
	TheBubbleHelper->SetHelp(mReplySubjectStrControl,
		"Here you can influence the subject-string \n"
		"Beam generates for a reply.\n"
		"The following macros are supported:\n"
		"	%s  -  expands to the original mail's subject.");
	TheBubbleHelper->SetHelp(mDefaultCharsetControl,
		"Here you can select the charset-encoding Beam should use by default.");
	TheBubbleHelper->SetHelp(mUsedCharsetsControl,
	...
	TheBubbleHelper->SetHelp(mHardWrapControl,
	...
	TheBubbleHelper->SetHelp(mLookInPeopleFolderControl,
		...
		"folder, too.");
	TheBubbleHelper->SetHelp(mAddNameToPeopleControl,
	...
	TheBubbleHelper->SetHelp(
		mPeopleFolderButton, "Click this button in order to\nselect a different people folder.");
	TheBubbleHelper->SetHelp(mUndoModeControl,
	...
	AddItemToMenu(mQuoteFormattingControl->Menu(),
		new BMenuItem(
			BmMailFactory::BM_QUOTE_AUTO_WRAP, new BMessage(BM_QUOTE_FORMATTING_SELECTED)),
		this);
	AddItemToMenu(mQuoteFormattingControl->Menu(),
		new BMenuItem(
			BmMailFactory::BM_QUOTE_PUSH_MARGIN, new BMessage(BM_QUOTE_FORMATTING_SELECTED)),
		this);
	AddItemToMenu(mQuoteFormattingControl->Menu(),
		new BMenuItem(BmMailFactory::BM_QUOTE_SIMPLE, new BMessage(BM_QUOTE_FORMATTING_SELECTED)),
		this);

	// add undo-modes:
	AddItemToMenu(mUndoModeControl->Menu(),
		new BMenuItem("Words", new BMessage(BM_UNDO_MODE_SELECTED)), this);
	AddItemToMenu(mUndoModeControl->Menu(),
		new BMenuItem("Paragraphs", new BMessage(BM_UNDO_MODE_SELECTED)), this);
	AddItemToMenu(
		mUndoModeControl->Menu(), new BMenuItem("None", new BMessage(BM_UNDO_MODE_SELECTED)), this);
...
\*------------------------------------------------------------------------------*/
void
BmPrefsMailConstrView::SetupUsedCharsetsMenu()
{
	BmString charset;
...
\*------------------------------------------------------------------------------*/
void
BmPrefsMailConstrView::SetupUsedCharsetsPrefs()
{
	BmString stdSets;
	BMenuItem* item;
	BMenu* menu = mUsedCharsetsControl->Menu();
...
\*------------------------------------------------------------------------------*/
void
BmPrefsMailConstrView::Update()
{
	mHardWrapControl->SetValueSilently(ThePrefs->GetBool("HardWrapMailText"));
	mQuoteFormattingControl->MarkItem(ThePrefs->GetString("QuoteFormatting").String());
	mDefaultCharsetControl->ClearMark();
	mUndoModeControl->MarkItem(ThePrefs->GetString("UndoMode", "Words").String());
...
\*------------------------------------------------------------------------------*/
void
BmPrefsMailConstrView::SaveData()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsMailConstrView::UndoChanges()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsMailConstrView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			case BM_TEXTFIELD_MODIFIED:
			{
				...
				if (source == mMaxLineLenControl)
					...
					ThePrefs->SetString(
						"ReplyIntroDefaultNick", mReplyIntroStrPrivateControl->Text());
			...
			{
				ThePrefs->SetBool(
					"LookForPeopleOnlyInPeopleFolder", mLookInPeopleFolderControl->Value());
				NoticeChange();
				break;
			}
			case BM_ADD_PEOPLE_NAME_CHANGED:
			{
				ThePrefs->SetBool("AddPeopleNameToMailAddr", mAddNameToPeopleControl->Value());
				NoticeChange();
				break;
			}
			...
			case BM_SELECT_PEOPLE_FOLDER:
			{
				...
				if (msg->FindRef("refs", 0, &folderRef) != B_OK) {
					...
					if (!mPeoplePanel) {
						mPeoplePanel = new BFilePanel(
							B_OPEN_PANEL, new BMessenger(this), NULL, B_DIRECTORY_NODE, false, msg);
					}
					mPeoplePanel->Show();
				} else {
