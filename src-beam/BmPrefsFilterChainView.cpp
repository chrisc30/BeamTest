...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmFilterChainItem::BmFilterChainItem(ColumnListView* lv, BmListModelItem* _item)
...
\*------------------------------------------------------------------------------*/
void
BmFilterChainItem::UpdateView(BmUpdFlags flags, bool redraw, uint32 updColBitmap)
{
	BmFilterChain* chain = dynamic_cast<BmFilterChain*>(ModelItem());
	if (chain && flags & UPD_ALL) {
		const char* cols[] = {chain->Key().String(), NULL};
		SetTextCols(0, cols);
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmFilterChainView::CreateListViewItem(BmListModelItem* item, BMessage*)
{
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmFilterChainView::AddModelItem(BmListModelItem* item)
{
...
\*------------------------------------------------------------------------------*/
void
BmFilterChainView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			default:
...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmChainedFilterItem::BmChainedFilterItem(ColumnListView* lv, BmListModelItem* _item)
	: inherited(lv, _item, false)
{
}
...
\*------------------------------------------------------------------------------*/
void
BmChainedFilterItem::UpdateView(BmUpdFlags flags, bool redraw, uint32 updColBitmap)
{
	BmChainedFilter* filter(dynamic_cast<BmChainedFilter*>(ModelItem()));
	if (flags & UPD_ALL) {
		BmString pos;
		pos << filter->Position();
		const char* cols[] = {pos.String(), filter->Key().String(), NULL};
		SetTextCols(0, cols);
		if (redraw)
			updColBitmap = 0xFFFFFFFF;
	}
...
BmChainedFilterView::BmChainedFilterView(int32 width, int32 height)
	: inherited(BRect(0, 0, float(width - 1), float(height - 1)), "Beam_ChainedFilterView",
		  B_SINGLE_SELECTION_LIST, false, true)
{
	int32 flags = 0;
	SetViewColor(B_TRANSPARENT_COLOR);
	...
	AddColumn(new CLVColumn("Pos", 20.0,
		flags | CLV_SORT_KEYABLE | CLV_RIGHT_JUSTIFIED | CLV_COLDATA_NUMBER | CLV_HIDDEN, 20.0));
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmChainedFilterView::CreateListViewItem(BmListModelItem* item, BMessage*)
{
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmChainedFilterView::AddModelItem(BmListModelItem* item)
{
	BmListViewItem* viewItem = inherited::AddModelItem(item);
	if (viewItem) {
		Select(IndexOf(viewItem));
...
\*------------------------------------------------------------------------------*/
void
BmChainedFilterView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			default:
...
\*------------------------------------------------------------------------------*/
bool
BmChainedFilterView::InitiateDrag(BPoint, int32 index, bool wasSelected)
{
	if (!wasSelected)
		return false;
	BMessage dragMsg(BM_CHAINED_FILTER_DRAG);
	BmChainedFilterItem* filterItem = dynamic_cast<BmChainedFilterItem*>(ItemAt(index));
	BmChainedFilter* filter = dynamic_cast<BmChainedFilter*>(filterItem->ModelItem());
	if (filter) {
...
\*------------------------------------------------------------------------------*/
bool
BmChainedFilterView::AcceptsDropOf(const BMessage* msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmChainedFilterView::HandleDrop(BMessage* msg)
{
	if (msg && msg->what == BM_CHAINED_FILTER_DRAG && mCurrHighlightItem) {
		...
		int32 newPos
			= highlightedFilter->Position() + (mCurrHighlightItem->HighlightBottom() ? 1 : -1);
		BmChainedFilterList* chainedFilters = dynamic_cast<BmChainedFilterList*>(DataModel().Get());
		if (chainedFilters) {
			BmAutolockCheckGlobal lock(chainedFilters->ModelLocker());
			if (!lock.IsLocked())
				BM_THROW_RUNTIME(chainedFilters->ModelNameNC() << ": Unable to get lock");
			BmModelItemMap::const_iterator iter;
			for (iter = chainedFilters->begin(); iter != chainedFilters->end(); ++iter) {
	...
	: inherited("Filter chains")
{
	...
	MView* view = new UserResizeSplitView(
		new HGroup(minimax(400, 100, 1E5, 1E5),
			new BetterScrollView(minimax(200, 100, 1E5, 1E5),
				mFilterChainListView = new BmFilterChainView(200, 150),
				BM_SV_H_SCROLLBAR | BM_SV_V_SCROLLBAR | BM_SV_CORNER | BM_SV_CAPTION,
				"99 filter chains"),
			new Space(minimax(5, 0, 5, 1E5)),
			new VGroup(mAddButton = new BetterButton("Add chain", new BMessage(BM_ADD_CHAIN), this),
				mRemoveButton
				= new BetterButton("Remove chain", new BMessage(BM_REMOVE_CHAIN), this),
				new Space(), 0),
			new Space(), 0),
		new HGroup(minimax(400, 250),
			borderl = new MBorder(M_LABELED_BORDER, 10, (char*)"Filter chain info",
				new VGroup(new HGroup(mChainControl = new BmTextControl("Filter chain name:"), 0),
					new BetterScrollView(minimax(200, 120, 1E5, 1E5),
			...
			new VGroup(new Space(), mAddFilterButton = new MPlayBW(this),
				mRemoveFilterButton = new MPlayFW(this), mEmptyChainButton = new MFFWD(this),
				new Space(), 0),
			new Space(minimax(10, 0, 10, 1E5)),
			borderr = new MBorder(M_LABELED_BORDER, 10, (char*)"Available filters",
				new BetterScrollView(minimax(200, 120, 1E5, 1E5),
					...
					BM_SV_H_SCROLLBAR | BM_SV_V_SCROLLBAR | BM_SV_CORNER)),
			0),
		"hsplitter", 150, B_HORIZONTAL, true, true, false, false, false, B_FOLLOW_NONE);

	float buttonWidth = StringWidth("Remove chain") + 20;
	mAddButton->ct_mpm.mini.x = mAddButton->ct_mpm.maxi.x = mRemoveButton->ct_mpm.mini.x
		= mRemoveButton->ct_mpm.maxi.x = buttonWidth;
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterChainView::Initialize()
{
	inherited::Initialize();

	TheBubbleHelper->SetHelp(mChainControl,
	...
	mChainedFilterListView->SetInvocationMessage(new BMessage(BM_CHAINED_ITEM_INVOKED));
	mChainedFilterListView->SetTarget(this);
	mChainedFilterListView->StartWatching(this, BmChainedFilterView::BM_NTFY_ORDER_MODIFIED);
	...
	mAvailableFilterListView->SetInvocationMessage(new BMessage(BM_AVAILABLE_ITEM_INVOKED));
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterChainView::Activated()
{
	inherited::Activated();
}
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterChainView::WriteStateInfo()
{
...
\*------------------------------------------------------------------------------*/
bool
BmPrefsFilterChainView::SanityCheck()
{
	return true;
}
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterChainView::SaveData()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterChainView::UndoChanges()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterChainView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			case BM_SELECTION_CHANGED:
			{
				...
				BmFilterChainItem* filterChainItem
					= (selection != -1) ? dynamic_cast<BmFilterChainItem*>(
											  mFilterChainListView->ItemAt(selection))
										: NULL;
				BmFilterChain* chain
					= filterChainItem ? dynamic_cast<BmFilterChain*>(filterChainItem->ModelItem())
									  : NULL;
				if (mCurrFilterChain != chain) {
					mCurrFilterChain = chain;
					if (mCurrFilterChain)
						mChainedFilterListView->StartJob(mCurrFilterChain->ChainedFilters(), false);
					else
			...
			case BM_CHAINED_SELECTION_CHANGED:
			{
				...
				BmChainedFilterItem* chainedFilterItem
					= (selection != -1) ? dynamic_cast<BmChainedFilterItem*>(
											  mChainedFilterListView->ItemAt(selection))
										: NULL;
				mCurrChainedFilter = chainedFilterItem ? dynamic_cast<BmChainedFilter*>(
			...
			case BM_AVAILABLE_SELECTION_CHANGED:
			{
				...
				BmFilterItem* availableFilterItem
					= (selection != -1)
						  ...
						  : NULL;
				mCurrAvailableFilter = availableFilterItem ? dynamic_cast<BmFilter*>(
																 availableFilterItem->ModelItem())
														   : NULL;
				UpdateState();
				break;
			}
			...
			case BM_ADD_CHAIN:
			{
				...
				TheFilterChainList->AddItemToList(
					new BmFilterChain(key.String(), TheFilterChainList.Get()));
			...
			case BM_EMPTY_CHAIN:
			{
				BmListModel* chain = dynamic_cast<BmListModel*>(mCurrFilterChain->ChainedFilters());
				if (chain) {
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterChainView::ChainFilter()
{
	if (!mCurrAvailableFilter || !mCurrFilterChain)
		return;
	BmChainedFilter* chainedFilter = new BmChainedFilter(
		mCurrAvailableFilter->Key().String(), mCurrFilterChain->ChainedFilters());
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterChainView::UnchainFilter()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsFilterChainView::UpdateState()
{
	...
	mChainControl->SetEnabled(haveChain && mCurrFilterChain->Key() != BM_DefaultItemLabel
	...
	if (haveAvailable && haveChain) {
		if (mCurrFilterChain->ChainedFilters()->FindItemByKey(mCurrAvailableFilter->Key())) {
			alreadyIn = true;
		}
	}
