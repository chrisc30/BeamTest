...
public:
	//
	BmListViewItem(ColumnListView* lv, BmListModelItem* item, bool hierarchical = false,
		BMessage* archive = NULL);
	virtual ~BmListViewItem();

	// native methods:
	void SetTextCols(int16 firstTextCol, const char** content);
	virtual void UpdateView(BmUpdFlags flags, bool redraw = true, uint32 updColBitmap = 0);
	...
	// setters
	inline void ShouldBeHidden(bool shouldBeHidden) { mShouldBeHidden = shouldBeHidden; }
...
public:
	...
	bool ApplyFilter(
		BmViewItemFilter* filter, ContinueCallback& callback, BmListViewController* listView);
...
public:
	// c'tors and d'tor:
	BmListViewController(BRect rect, const char* Name = NULL,
	...
	bool ApplyViewItemFilter(
		BmViewItemFilter* filter, BmViewItemManager::ContinueCallback& callback);
...
protected:
	...
	//
	virtual BmListViewItem* CreateListViewItem(BmListModelItem* item, BMessage* archive = NULL) = 0;
	//
...
private:
	BmListViewItem* doAddModelItem(BmListViewItem* parent, BmListModelItem* item, bool redraw);
