...
BmMailRefFilterControl::BmMailRefFilterControl()
	: inherited("Active time span:",
		  new BmMenuController(TIME_SPAN_NONE, this, new BMessage(BM_MAILREF_FILTER_CHANGED),
			  &BmGuiRosterBase::RebuildMailRefFilterMenu, BM_MC_LABEL_FROM_MARKED),
...
\*------------------------------------------------------------------------------*/
void
BmMailRefFilterControl::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			case BM_MAILREF_FILTER_CHANGED:
			{
				if (mPartnerMailRefView) {
					...
					BmMailRefFilter* filter
						= label != TIME_SPAN_NONE ? new BmMailRefFilter(label, numberOfDays) : NULL;
