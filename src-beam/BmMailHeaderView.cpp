...
public:
	// c'tors and d'tor:
	BmMailHeaderFieldView(const BmString& title, const BmAddressList* addrList, BFont* font,
		float fixedWidth, float titleWidth = -1);
	BmMailHeaderFieldView(const BmString& title, const BmString& value, BFont* font,
		float fixedWidth, float titleWidth = -1);
	~BmMailHeaderFieldView();
...
private:
	void SetTo(const BmString& title, const BmString& value, const BmAddressList* addrList,
...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmMailHeaderView::BmMailHeaderFieldView::BmAddrMenuView ::BmAddrMenuView(
	BRect frame, const BmAddressList* addrList)
...
\*------------------------------------------------------------------------------*/
void
BmMailHeaderView::BmMailHeaderFieldView::BmAddrMenuView ::Draw(BRect updateRect)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailHeaderView::BmMailHeaderFieldView::BmAddrMenuView ::MouseDown(BPoint point)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailHeaderView::BmMailHeaderFieldView::BmAddrMenuView ::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			case B_COPY:
			{
				...
				if (be_clipboard->Lock()) {
					...
					if ((clipMsg = be_clipboard->Data()) != NULL) {
						clipMsg->AddData(
							"text/plain", B_MIME_TYPE, addrStr.String(), addrStr.Length());
						be_clipboard->Commit();
					}
					be_clipboard->Unlock();
				}
...
\*------------------------------------------------------------------------------*/
void
BmMailHeaderView::BmMailHeaderFieldView::BmAddrMenuView ::ShowMenu(BPoint point)
{
...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmMailHeaderView::BmMailHeaderFieldView ::BmMailHeaderFieldView(const BmString& title,
	const BmAddressList* addrList, BFont* font, float fixedWidth, float titleWidth)
	: inherited(BRect(0, 0, 0, 0), "MailHeaderFieldView", B_FOLLOW_NONE, B_WILL_DRAW)
{
...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmMailHeaderView::BmMailHeaderFieldView ::BmMailHeaderFieldView(
	const BmString& title, const BmString& value, BFont* font, float fixedWidth, float titleWidth)
	: inherited(BRect(0, 0, 0, 0), "MailHeaderFieldView", B_FOLLOW_NONE, B_WILL_DRAW)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailHeaderView::BmMailHeaderFieldView ::SetTo(const BmString& title, const BmString& value,
	const BmAddressList* addrList, BFont* font, float fixedWidth, float titleWidth)
{
	SetViewColor(BmWeakenColor(B_PANEL_BACKGROUND_COLOR, 2));
	...
	BRect addrMenuRect(titleRect.right + 1, 0, titleRect.right + 1 + addrMenuWidth - 1, 10);
	mAddrMenuView = new BmAddrMenuView(addrMenuRect, addrList);
	AddChild(mAddrMenuView);

	BRect contentRect(titleRect.right + 1 + addrMenuWidth, 0, fixedWidth, 10);
	font->SetFace(B_REGULAR_FACE);
	mContentView = new BmMultiLineStringView(
		contentRect, "contentView", value.String(), B_FOLLOW_NONE, B_WILL_DRAW);
...
\*------------------------------------------------------------------------------*/
filter_result
BmMailHeaderView::BmMailHeaderFieldView::BmMsgFilter ::Filter(BMessage* msg, BHandler** handler)
{
...
\*------------------------------------------------------------------------------*/
float
BmMailHeaderView::BmMailHeaderFieldView::GetTitleWidth()
{
...
\*------------------------------------------------------------------------------*/
void
BmMailHeaderView::BmMailHeaderFieldView ::SetTitleWidth(float newTitleWidth, float fixedWidth)
{
	...
	mContentView->ResizeTo(
		fixedWidth - newTitleWidth - addrMenuWidth, mContentView->Frame().Height());
}
...
\*------------------------------------------------------------------------------*/
void
BmMailHeaderView::BmMailHeaderFieldView::MouseDown(BPoint point)
{
	inherited::MouseDown(point);
	if (Parent())
		if (Parent()->Parent())
...
\*------------------------------------------------------------------------------*/
void
BmMailHeaderView::BmMailHeaderFieldView::ShowMenu(BPoint point)
{
	...
	item->SetMarked(headerView->DisplayMode() == BmMailHeaderView::SMALL_HEADERS);
	theMenu->AddItem(item);
	item = new BMenuItem("Large header", new BMessage(BM_HEADERVIEW_LARGE));
	item->SetTarget(headerView);
	item->SetMarked(headerView->DisplayMode() == BmMailHeaderView::LARGE_HEADERS);
	theMenu->AddItem(item);
	item = new BMenuItem("Full header (raw)", new BMessage(BM_HEADERVIEW_FULL));
	item->SetTarget(headerView);
	item->SetMarked(headerView->DisplayMode() == BmMailHeaderView::FULL_HEADERS);
	theMenu->AddItem(item);
	theMenu->AddSeparatorItem();
	item = new BMenuItem(
		"Show info from resent-fields if present", new BMessage(BM_HEADERVIEW_SWITCH_RESENT));
	item->SetTarget(headerView);
...
BmMailHeaderView::BmMailHeaderView(BmMailHeader* header)
	: inherited(BRect(0, 0, 0, 0), "MailHeaderView", B_FOLLOW_NONE, B_WILL_DRAW),
	  mMailHeader(NULL),
...
\*------------------------------------------------------------------------------*/
status_t
BmMailHeaderView::Archive(BMessage* archive, bool) const
{
...
\*------------------------------------------------------------------------------*/
status_t
BmMailHeaderView::Unarchive(BMessage* archive, bool)
{
	int16 version;
...
\*------------------------------------------------------------------------------*/
void
BmMailHeaderView::ShowHeader(BmMailHeader* header, bool invalidate)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailHeaderView::RemoveFieldViews()
{
...
\*------------------------------------------------------------------------------*/
float
BmMailHeaderView::AddFieldViews()
{
	...
	if (mDisplayMode != FULL_HEADERS) {
		...
		for (uint32 l = 0; l < fields.size(); ++l) {
			...
			for (uint32 v = 0; v < fieldVals.size(); ++v) {
				if (addrList) {
					fv = new BmMailHeaderFieldView(
						titles[l], addrList, &mFont, FixedWidth(), maxTitleWidth);
				} else {
					fv = new BmMailHeaderFieldView(
						titles[l], fieldVals[v], &mFont, FixedWidth(), maxTitleWidth);
				}
		...
		int numLines
			= rx.exec(mMailHeader->HeaderString(), "\\n", Regexx::newline | Regexx::global);
...
\*------------------------------------------------------------------------------*/
status_t
BmMailHeaderView::UISettingsChanged(const BMessage* changes, uint32 flags)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailHeaderView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			...
			case BM_HEADERVIEW_COPY_HEADER:
			{
				...
				if (be_clipboard->Lock()) {
					...
					if ((clipMsg = be_clipboard->Data()) != NULL) {
						clipMsg->AddData(
							"text/plain", B_MIME_TYPE, hdrStr.String(), hdrStr.Length());
						be_clipboard->Commit();
					}
					be_clipboard->Unlock();
				}
...
\*------------------------------------------------------------------------------*/
void
BmMailHeaderView::Draw(BRect bounds)
{
