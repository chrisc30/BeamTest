...
BmRulerView::BmRulerView(const BFont& font)
	: inherited(BRect(0, 0, 0, 19), "RulerView", B_FOLLOW_NONE, B_WILL_DRAW),
	  mMailViewFont(font),
...
\*------------------------------------------------------------------------------*/
void
BmRulerView::SetMailViewFont(const BFont& font)
{
	mMailViewFont = font;
...
\*------------------------------------------------------------------------------*/
void
BmRulerView::Draw(BRect bounds)
{
	inherited::Draw(bounds);

	BRect r = Bounds();
	int32 maxLineLen = ThePrefs->GetBool("NeverExceed78Chars", false) ? 78 : 0;
	float width = maxLineLen ? 1.0f + float(maxLineLen) * mSingleCharWidth
	...
	SetHighColor(BmWeakenColor(B_CONTROL_HIGHLIGHT_COLOR, mIndicatorGrabbed ? 0 : 2));
	if (ThePrefs->GetBool("HardWrapMailText")) {
		FillTriangle(BPoint(xPos - 5, yPos), BPoint(xPos + 5, yPos), BPoint(xPos, yPos + 10));
	} else {
		StrokeTriangle(BPoint(xPos - 5, yPos), BPoint(xPos + 5, yPos), BPoint(xPos, yPos + 10));
	}
}
...
\*------------------------------------------------------------------------------*/
void
BmRulerView::MouseDown(BPoint point)
{
	...
	if (maxLineLen < 100) {
		BmString s = BmString(
						 "The right margin is currently fixed\n"
						 "to a maximum of ")
					 << maxLineLen << " characters.\n\nPlease check the preferences.";
		TheBubbleHelper->SetHelp(this, s.String());
	} else
		TheBubbleHelper->SetHelp(this, NULL);
	BMessage* msg = Looper()->CurrentMessage();
	int32 buttons;
	if (msg->FindInt32("buttons", &buttons) == B_OK && buttons == B_PRIMARY_MOUSE_BUTTON) {
...
\*------------------------------------------------------------------------------*/
void
BmRulerView::MouseMoved(BPoint point, uint32 transit, const BMessage* msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmRulerView::MouseUp(BPoint point)
{
...
\*------------------------------------------------------------------------------*/
void
BmRulerView::SetIndicatorPixelPos(float pixelPos)
{
	pixelPos -= nXOffset;
...
\*------------------------------------------------------------------------------*/
void
BmRulerView::SetIndicatorPos(int32 newPos)
{
