...
\*------------------------------------------------------------------------------*/
void
BmToolbar::UpdateLayout(bool recalcSizes)
{
	if (LockLooper()) {
		...
		BRect rect = Bounds();
		BmBitmapHandle* toolbarBackground = TheResources->IconByName("Toolbar_Background");
		...
		if (group) {
			...
			for (int32 c = 0; c < count; ++c) {
				BmToolbarButton* tbb = dynamic_cast<BmToolbarButton*>(group->ChildAt(c));
				if (tbb)
...
\*------------------------------------------------------------------------------*/
void
BmToolbar::Draw(BRect updateRect)
{
	BRect bounds = Bounds();
	SetHighColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_LIGHTEN_1_TINT));
	StrokeLine(BPoint(bounds.left, bounds.bottom - 1), BPoint(bounds.left, bounds.top));
	StrokeLine(BPoint(bounds.left + 1, bounds.top), BPoint(bounds.right, bounds.top));
	SetHighColor(tint_color(HighColor(), B_DARKEN_1_TINT));
	StrokeLine(BPoint(bounds.right, bounds.top + 1), BPoint(bounds.right, bounds.bottom - 1));
...
\*------------------------------------------------------------------------------*/
void
BmToolbarSpace::Draw(BRect updateRect)
{
...
/*------------------------------------------------------------------------------*\
	( )
		-
\*------------------------------------------------------------------------------*/
BmToolbarButton::BmToolbarButton(const char* label, float width, float height, BMessage* message,
	BHandler* handler, const char* tipText, bool needsLatch, const char* resourceName)
	: inherited(minimax(int(width), int(height), -1, -1), &BmDummyPicture, &BmDummyPicture, message,
		  handler),
	  mHighlighted(false),
...
\*------------------------------------------------------------------------------*/
void
BmToolbarButton::Draw(BRect updateRect)
{
	...
	if (be_control_look && mHighlighted && IsEnabled()) {
		...
		be_control_look->DrawButtonFrame(this, rect, updateRect, base, base, flags);
	...
	if (mHighlighted && IsEnabled()) {
		// draw higlighting border
		BeginLineArray(4);
		AddLine(rect.LeftBottom(), rect.LeftTop(), ui_color(B_SHINE_COLOR));
		AddLine(rect.LeftTop(), rect.RightTop(), ui_color(B_SHINE_COLOR));
		AddLine(rect.LeftBottom(), rect.RightBottom(), BmWeakenColor(B_SHADOW_COLOR, BeShadowMod));
		AddLine(rect.RightBottom(), rect.RightTop(), BmWeakenColor(B_SHADOW_COLOR, BeShadowMod));
		EndLineArray();
	}
#endif
}
...
\*------------------------------------------------------------------------------*/
void
BmToolbarButton::CreateAllPictures(float width, float height)
{
	ct_mpm = minimax(int(width), int(height), -1, -1);
	BPicture* off_pic = CreatePicture(STATE_OFF, width, height);
	BPicture* on_pic = CreatePicture(STATE_ON, width, height);
	BPicture* dis_pic = CreatePicture(STATE_DISABLED, width, height);
...
\*------------------------------------------------------------------------------*/
void
BmToolbarButton::AddActionVariation(const BmString label, BMessage* msg)
{
...
\*------------------------------------------------------------------------------*/
BPicture*
BmToolbarButton::CreatePicture(int32 mode, float width, float height)
{
	const char* label = mLabel.String();
	BmBitmapHandle* imageHandle = TheResources->IconByName(BmString("Button_") << mResourceName);
	...
	if (showIcons && (labelMode == "Left")) {
		...
		posIcon = BPoint(
			d + DIVW + labelWidth + (mNeedsLatch ? DIVLATCHW : 0), (height - iconHeight) / 2 - 1);
		...
		posLabel = BPoint(
			(width - labelWidth - (mNeedsLatch ? DIVLATCHW : 0)) / 2, d + DIVH + iconHeight);
		posIcon = BPoint((width - iconWidth) / 2 - 1, d);
	} else if (!showIcons && labelMode != "Hide") {
		// Label only
		posLabel = BPoint(
			(width - labelWidth - (mNeedsLatch ? DIVLATCHW : 0)) / 2, (height - labelHeight) / 2);
	} else if (showIcons && labelMode == "Hide") {
		// Icon only
		posIcon = BPoint(
			(width - iconWidth - (mNeedsLatch ? DIVLATCHW : 0)) / 2, (height - iconHeight) / 2);
	}

	if (mNeedsLatch) {
		if (labelMode == "Hide")
			mLatchRect.Set(
				posIcon.x + iconWidth + 2, posIcon.y + iconHeight / 2 - LATCHSZ / 2, width, height);
		else
	...
	if (mode == STATE_ON) {
		...
		view->AddLine(
			rect.LeftBottom(), rect.LeftTop(), BmWeakenColor(B_SHADOW_COLOR, BeShadowMod));
		view->AddLine(rect.LeftTop(), rect.RightTop(), BmWeakenColor(B_SHADOW_COLOR, BeShadowMod));
		view->AddLine(rect.LeftBottom(), rect.RightBottom(), ui_color(B_SHINE_COLOR));
		view->AddLine(rect.RightBottom(), rect.RightTop(), ui_color(B_SHINE_COLOR));
		view->EndLineArray();
	}
#endif
	...
	if (mNeedsLatch) {
		...
		view->FillTriangle(BPoint(x_offs, y_offs), BPoint(x_offs + LATCHSZ * 2, y_offs),
...
\*------------------------------------------------------------------------------*/
void
BmToolbarButton::SetUpdateVariationsFunc(BmUpdateVariationsFunc* updFunc)
{
...
\*------------------------------------------------------------------------------*/
void
BmToolbarButton::MouseDown(BPoint point)
{
	int32 buttons = 0;
...
\*------------------------------------------------------------------------------*/
void
BmToolbarButton ::MouseMoved(BPoint point, uint32 transit, const BMessage* msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmToolbarButton::ShowMenu(BPoint point)
{
	...
	for (uint32 i = 0; i < mVariations.size(); ++i) {
		item = new BMenuItem(mVariations[i].label.String(), new BMessage(*mVariations[i].msg));
...
\*------------------------------------------------------------------------------*/
void
BmToolbarButton::CalcMaxSize(float& width, float& height, const char* label, bool needsLatch)
{
	BFont font(be_plain_font);
	float w, h;
	BmBitmapHandle* imageHandle = TheResources->IconByName(BmString("Button_") << label);
	BBitmap* image = imageHandle ? imageHandle->bitmap : NULL;
	bool showIcons = ThePrefs->GetBool("ShowToolbarIcons", true);
	BmString labelMode = ThePrefs->GetString("ShowToolbarLabel", "Bottom");
	...
	if (showIcons && (labelMode == "Left" || labelMode == "Right")) {
		...
		// Icon + Label: vertical
		w = MAX(
			labelWidth + 2 * DIVLABELW + (needsLatch ? DIVLATCHW : 0), iconWidth + 2 * DIVICONW);
