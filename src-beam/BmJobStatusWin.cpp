...
\*------------------------------------------------------------------------------*/
void
BmJobStatusView::UpdateModelView(BMessage*)
{
}
...
\*------------------------------------------------------------------------------*/
void
BmJobStatusView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			case BM_JOB_DONE:
...
\*------------------------------------------------------------------------------*/
void
BmJobStatusView::StartJob(BmJobModel* model, bool startInNewThread, int32 jobSpecifier)
{
	delete mShowMsgRunner;
	mShowMsgRunner = NULL;
	BMessage timerMsg(BM_TIME_TO_SHOW);
	BM_LOG2(BM_LogModelController, BmString("Controller <")
									   << ControllerName() << "> sets timer-to-show to "
									   << MSecsBeforeShow() << " msecs");
	mShowMsgRunner = new BMessageRunner(BMessenger(this), &timerMsg, MSecsBeforeShow(), 1);
...
\*------------------------------------------------------------------------------*/
void
BmJobStatusView::JobIsDone(bool completed)
{
	BM_LOG2(BM_LogModelController, BmString("Controller <")
									   << ControllerName() << "> has been told that job "
									   << ModelName() << " is done");
	...
	BM_LOG2(BM_LogModelController, BmString("Controller <")
									   << ControllerName() << "> sets timer-to-remove to "
									   << MSecsBeforeRemove() << " msecs");
...
\*------------------------------------------------------------------------------*/
BmMailMoverView*
BmMailMoverView::CreateInstance(const char* name)
{
	  ...
	  mBottomLabel(NULL)
{
	...
	MView* view
		= new VGroup(new MBViewWrapper(mStatBar = new BStatusBar(BRect(), name, "Moving: ", ""),
						 true, false, false),
			new HGroup(mBottomLabel = new MStringView(""), new MStop(this), 0), 0);
...
\*------------------------------------------------------------------------------*/
BmJobModel*
BmMailMoverView::CreateJobModel(BMessage* msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailMoverView::ResetController()
{
...
\*------------------------------------------------------------------------------*/
void
BmMailMoverView::UpdateModelView(BMessage* msg)
{
	...
	if (lock.IsLocked()) {
		...
		throw BM_runtime_error(
			"BmMailMoverView::UpdateModelView(): could not "
			"lock window");
}
...
\*------------------------------------------------------------------------------*/
BmMailFilterView*
BmMailFilterView::CreateInstance(const char* name)
{
	  ...
	  mBottomLabel(NULL)
{
	...
	MView* view
		= new HGroup(new MBViewWrapper(mStatBar = new BStatusBar(BRect(), name, "Filtering: ", ""),
						 true, false, false),
			new MStop(this), 0);
	AddChild(dynamic_cast<BView*>(view));
	mStatBar->SetBarHeight(8.0);
...
\*------------------------------------------------------------------------------*/
BmJobModel*
BmMailFilterView::CreateJobModel(BMessage* msg)
{
	...
	if (filterName.Length()) {
		if (filterName == BmFilterList::LEARN_AS_SPAM_NAME)
			...
			BmRef<BmListModelItem> filterRef = TheFilterList->FindItemByKey(filterName);
...
\*------------------------------------------------------------------------------*/
void
BmMailFilterView::ResetController()
{
...
\*------------------------------------------------------------------------------*/
void
BmMailFilterView::UpdateModelView(BMessage* msg)
{
	...
	if (lock.IsLocked()) {
		...
		throw BM_runtime_error(
			"BmMailFilterView::UpdateModelView(): could not "
			"lock window");
}
...
\*------------------------------------------------------------------------------*/
BmPopperView*
BmPopperView::CreateInstance(const char* name, bool isAutoCheck)
{
	  ...
	  mHaveBeeped(false)
{
	...
	MView* view = new VGroup(
		new MBViewWrapper(mStatBar = new BStatusBar(BRect(), name, name, ""), true, false, false),
		new MBViewWrapper(
...
\*------------------------------------------------------------------------------*/
BmJobModel*
BmPopperView::CreateJobModel(BMessage* msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmPopperView::UpdateModelView(BMessage* msg)
{
	...
	if (lock.IsLocked()) {
		if (domain == "mailbar") {
			...
			if (mMailBar->CurrentValue() == mMailBar->MaxValue() && !mHaveBeeped
		...
		throw BM_runtime_error(
			"BmPopperView::UpdateModelView(): could not "
			"lock window");
}
...
\*------------------------------------------------------------------------------*/
BmImapView*
BmImapView::CreateInstance(const char* name, bool isAutoCheck)
{
...
\*------------------------------------------------------------------------------*/
BmJobModel*
BmImapView::CreateJobModel(BMessage* msg)
{
	BmString accName = FindMsgString(msg, BmJobModel::MSG_JOB_NAME);
	BmRef<BmListModelItem> item = TheRecvAccountList->FindItemByKey(accName);
...
\*------------------------------------------------------------------------------*/
void
BmImapView::UpdateModelView(BMessage* msg)
{
	...
	if (lock.IsLocked()) {
		...
		throw BM_runtime_error(
			"BmImapView::UpdateModelView(): could not "
			"lock window");
}
...
\*------------------------------------------------------------------------------*/
BmSmtpView*
BmSmtpView::CreateInstance(const char* name)
{
	  ...
	  mMailBar(NULL)
{
	...
	MView* view = new VGroup(new MBViewWrapper(mStatBar = new BStatusBar(BRect(), name,
												   (BmString("SMTP: ") << name).String(), ""),
								 true, false, false),
		new MBViewWrapper(
			mMailBar = new BStatusBar(BRect(), name, "Mails: ", ""), true, false, false),
		0);
...
\*------------------------------------------------------------------------------*/
BmJobModel*
BmSmtpView::CreateJobModel(BMessage* msg)
{
	BmString accName = FindMsgString(msg, BmJobModel::MSG_JOB_NAME);
...
\*------------------------------------------------------------------------------*/
void
BmSmtpView::UpdateModelView(BMessage* msg)
{
	...
	if (lock.IsLocked()) {
		...
		throw BM_runtime_error(
			"BmSmtpView::UpdateModelView(): could not "
			"lock window");
}
...
BmJobStatusWin::BmJobStatusWin()
	: BmWindow("JobStatusWindow", BRect(beamApp->ScreenFrame().right - BM_MINSIZE - 4, 20, 0, 0),
		  "Jobs", B_FLOATING_WINDOW_LOOK, B_NORMAL_WINDOW_FEEL,
		  B_ASYNCHRONOUS_CONTROLS | B_NOT_ZOOMABLE | B_NOT_V_RESIZABLE | B_NO_WORKSPACE_ACTIVATION
			  | (mouse_mode() == B_NORMAL_MOUSE ? B_AVOID_FOCUS : 0))
{
	mOuterGroup = new VGroup(new Space(minimax(BM_MINSIZE, 0, 1E5, 1E5, 1)), 0);
...
BmJobStatusWin::QuitRequested()
{
	...
	while (!IsHidden())
		Hide();
	BM_LOG2(BM_LogJobWin, "JobStatusWin has been asked to quit; stopping all jobs");
...
\*------------------------------------------------------------------------------*/
void
BmJobStatusWin::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			case BM_JOBWIN_SMTP:
			...
			case BM_JOBWIN_IMAP:
			{
				if (mActiveJobs.empty() || !ThePrefs->GetBool("QueueNetworkJobs", true))
					AddJob(msg);
				else
					QueueJob(msg);
				break;
...
\*------------------------------------------------------------------------------*/
void
BmJobStatusWin::QueueJob(BMessage* msg)
{
	BM_ASSERT(msg);
...
\*------------------------------------------------------------------------------*/
void
BmJobStatusWin::AddJob(BMessage* msg)
{
	BM_ASSERT(msg);
...
\*------------------------------------------------------------------------------*/
void
BmJobStatusWin::RemoveJob(const char* name)
{
	BM_ASSERT(name);
