...
BmMailRefViewFilterControl::BmMailRefViewFilterControl()
	: inheritedView(
		  mMenuControl = new BmMenuControl("Narrow down:",
			  new BmMenuController(BmMailRefItemFilter::FILTER_SUBJECT_OR_ADDRESS, this,
				  new BMessage(BM_MAILREF_VIEW_FILTER_CHANGED),
				  &BmGuiRosterBase::RebuildMailRefViewFilterMenu, BM_MC_LABEL_FROM_MARKED),
			  1, 1E5, BmMailRefItemFilter::FILTER_SUBJECT_OR_ADDRESS),
		  mTextControl = new BmTextControl(NULL, false, 0, 30), NULL),
...
\*------------------------------------------------------------------------------*/
void
BmMailRefViewFilterControl::AttachedToWindow()
{
	inheritedView::AttachedToWindow();
	mTextControl->SetTarget(this);
	mTextControl->TextView()->AddFilter(
		new BMessageFilter(B_ANY_DELIVERY, B_ANY_SOURCE, B_KEY_DOWN, MessageFilterHook));
...
\*------------------------------------------------------------------------------*/
void
BmMailRefViewFilterControl::MakeFocus(bool focus)
{
	if (focus)
...
\*------------------------------------------------------------------------------*/
void
BmMailRefViewFilterControl::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			case BM_MAILREF_VIEW_FILTER_CHANGED:
			{
				...
				if (content != mLastContent || kind != mLastKind) {
					BmMailRefItemFilter* filter
						= content.Length() > 0 ? new BmMailRefItemFilter(kind, content) : NULL;
			...
			case BM_TEXTFIELD_MODIFIED:
			{
				if (!mMsgRunner) {
					...
					mMsgRunner = new BMessageRunner(ourselvesAsTarget, &changedMsg, 200 * 1000, 1);
				}
				break;
			}
			case BM_JOB_DONE:
...
\*------------------------------------------------------------------------------*/
void
BmMailRefViewFilterControl::KeyDown(const char* bytes, int32 numBytes)
{
	if (numBytes == 1) {
		switch (bytes[0]) {
			case B_ESCAPE:
...
\*------------------------------------------------------------------------------*/
void
BmMailRefViewFilterControl::JobIsDone(bool completed)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailRefViewFilterControl::ClearFilter()
{
...
\*------------------------------------------------------------------------------*/
filter_result
BmMailRefViewFilterControl::MessageFilterHook(
	BMessage* msg, BHandler** handler, BMessageFilter* messageFilter)
{
	if (msg->what == B_KEY_DOWN) {
		BmString bytes = msg->FindString("bytes");
		if (bytes.Length() && (bytes[0] == B_ESCAPE || bytes[0] == B_DOWN_ARROW)) {
