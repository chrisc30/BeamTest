...
\*------------------------------------------------------------------------------*/
void
WordWrap(const BmString& in, BmString& out, int32 maxLineLen, BmString nl)
{
	if (!in.Length()) {
	...
	for (int32 pos = 0; !needBreak; pos += nl.Length(), lastPos = pos) {
		...
		while (lineLen > maxLineLen) {
			...
			if (lastSpcPos == B_ERROR || lastSpcPos < lastPos) {
				...
				isUrl = (rxUrl.exec(BmString(in.String() + lastPos, pos - lastPos),
					"(https?://|ftp://|nntp://|file://|mailto:)", Regexx::nocase));
				if (isUrl) {
...
\*------------------------------------------------------------------------------*/
BmMailView*
BmMailView::CreateInstance(BRect frame, bool outbound)
{
	...
	if ((err = archiveFile.SetTo(
			 BeamRoster->StateInfoFolder(), archiveFilename.String(), B_READ_ONLY))
		== B_OK) {
...
\*------------------------------------------------------------------------------*/
status_t
BmMailView::Archive(BMessage* archive, bool deep) const
{
...
\*------------------------------------------------------------------------------*/
status_t
BmMailView::Unarchive(BMessage* archive, bool deep)
{
	int16 version;
	if (archive->FindInt16(MSG_VERSION, &version) != B_OK)
		version = 1;
	status_t ret
		= archive->FindBool(MSG_RAW, &mShowRaw) || archive->FindInt16(MSG_FONTSIZE, &mFontSize);
...
\*------------------------------------------------------------------------------*/
void
BmMailView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			...
			case BM_MAILVIEW_COPY_URL:
			{
				...
				if (be_clipboard->Lock()) {
					...
					if ((clipMsg = be_clipboard->Data()) != NULL) {
						clipMsg->AddData(
							"text/plain", B_MIME_TYPE, urlStr.String(), urlStr.Length());
						be_clipboard->Commit();
					}
					be_clipboard->Unlock();
				}
...
\*------------------------------------------------------------------------------*/
void
BmMailView::KeyDown(const char* bytes, int32 numBytes)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailView::MouseDown(BPoint point)
{
	BMessage* msg = Looper()->CurrentMessage();
	int32 buttons;
	if (msg->FindInt32("buttons", &buttons) == B_OK) {
		if (buttons == B_PRIMARY_MOUSE_BUTTON || buttons == B_TERTIARY_MOUSE_BUTTON) {
...
\*------------------------------------------------------------------------------*/
void
BmMailView::MouseUp(BPoint point)
{
...
\*------------------------------------------------------------------------------*/
bool
BmMailView::IsOverURL(BPoint point)
{
...
\*------------------------------------------------------------------------------*/
BmString
BmMailView::GetTextForTextrun(BmTextRunIter run)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailView::MouseMoved(BPoint point, uint32 transit, const BMessage* msg)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailView::MakeFocus(bool focused)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailView::FrameResized(float newWidth, float newHeight)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailView::SetSignatureByName(const BmString sigName)
{
	if (!mOutbound)
		return;
	BmString text = Text();
...
\*------------------------------------------------------------------------------*/
void
BmMailView::UpdateFont(const BFont& font)
{
	SetTabWidth(font.StringWidth(BM_SPACES.String(), ThePrefs->GetInt("SpacesPerTab", 4)));
...
\*------------------------------------------------------------------------------*/
bool
BmMailView::AcceptsDrop(const BMessage*)
{
	return IsEditable();
}
...
\*------------------------------------------------------------------------------*/
bool
BmMailView::CanEndLine(int32 offset)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailView::InsertText(const char* text, int32 length, int32 offset, const text_run_array* runs)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailView::GetWrappedText(BmString& out, bool hardWrapIfNeeded)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailView::ShowMail(BmMailRef* ref, bool async)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailView::ShowMail(BmMail* mail, bool async)
{
	if (mCurrMail == mail)
		return;
	try {
...
\*------------------------------------------------------------------------------*/
void
BmMailView::AddParsingError(const BmString& errStr)
{
	if (errStr.Length()) {
...
\*------------------------------------------------------------------------------*/
void
BmMailView::JobIsDone(bool completed)
{
	if (completed && mCurrMail && mCurrMail->Header()) {
		mParsingErrors.Truncate(0);
		ContainerView()->SetErrorText(mParsingErrors);
		BmString displayText;
		BmStringOBuf displayBuf(mShowRaw ? mCurrMail->RawText().Length() : 65536);
		mTextRunMap.clear();
		...
		if (body) {
			...
			if (mShowRaw) {
				...
				BM_LOG2(BM_LogMailParse, BmString("extracting parts to be displayed from "
												  "body-structure"));
				BmAutolockCheckGlobal lock(body->ModelLocker());
				if (!lock.IsLocked())
					BM_THROW_RUNTIME(body->ModelNameNC() << ": Unable to get lock");
			...
			if (!mOutbound && (mHighlightFlags & HIGHLIGHT_URL) > 0) {
				...
				if ((count = rx.exec(displayText,
					...
					for (int i = 0; i < count; ++i) {
						...
						mTextRunMap[start]
							= BmTextRunInfo(ui_color(B_CONTROL_HIGHLIGHT_COLOR), true);
...
\*------------------------------------------------------------------------------*/
void
BmMailView::StartIncrementalSearch()
{
	MakeFocus(true);
...
\*------------------------------------------------------------------------------*/
void
BmMailView::HandleIncrementalSearchKeys(const char* bytes, int32 numBytes)
{
	if (!mScrollView)
		return;
...
\*------------------------------------------------------------------------------*/
void
BmMailView::IncrementalSearch(const BmString& search, bool next)
{
	if (search.Length()) {
...
\*------------------------------------------------------------------------------*/
bool
BmMailView::IsDisplayComplete()
{
	return !IsJobRunning() && !mBodyPartView->IsJobRunning() && !mDisplayInProgress;
}
...
\*------------------------------------------------------------------------------*/
void
BmMailView::SendNoticesIfNeeded(bool haveMail)
{
...
\*------------------------------------------------------------------------------*/
void
BmMailView::DisplayBodyPart(BmStringOBuf& displayBuf, BmBodyPart* bodyPart)
{
		...
		if (bodyPart->ShouldBeShownInline()) {
			...
			if (bodyPart->IsBinary()) {
				...
				if (bodyPart->HadErrorDuringConversion())
					AddParsingError(BmString("The mailtext contains characters that could not ")
...
\*------------------------------------------------------------------------------*/
void
BmMailView::UpdateParsingStatus()
{
...
/*------------------------------------------------------------------------------*\
	TextRunInfoAt()
		-
\*------------------------------------------------------------------------------*/
BmMailView::BmTextRunIter
BmMailView::TextRunInfoAt(int32 pos) const
{
...
BmMailView::WriteStateInfo()
{
	...
	try {
		...
		if ((err = cacheFile.SetTo(BeamRoster->StateInfoFolder(), filename.String(),
...
\*------------------------------------------------------------------------------*/
void
BmMailView::ShowMenu(BPoint point)
{
	...
	if (!mOutbound) {
		item = new BMenuItem("Show all MIME bodies",
			new BMessage(mBodyPartView->ShowAllParts() ? BM_BODYPARTVIEW_SHOWATTACHMENTS
													   : BM_BODYPARTVIEW_SHOWALL));
		item->SetTarget(mBodyPartView);
		item->SetMarked(mBodyPartView->ShowAllParts());
		theMenu->AddItem(item);
		item = new BMenuItem("Show raw message",
			new BMessage(ShowRaw() ? BM_MAILVIEW_SHOWCOOKED : BM_MAILVIEW_SHOWRAW));
		item->SetTarget(this);
		item->SetMarked(ShowRaw());
		theMenu->AddItem(item);

		item = new BMenuItem("Separate inlines",
			new BMessage(ShowInlinesSeparately() ? BM_MAILVIEW_SHOWINLINES_CONCATENATED
	...
	if (!mOutbound) {
		...
		item->SetTarget(this);
		hiMenu->AddItem(item);
		item = new BMenuItem("Signatures", new BMessage(BM_MAILVIEW_HIGHLIGHT_SIG));
		if ((mHighlightFlags & HIGHLIGHT_SIG) > 0)
			item->SetMarked(true);
		item->SetTarget(this);
		hiMenu->AddItem(item);
...
BmMailViewContainer::BmMailViewContainer(minimax minmax, BmMailView* target)
	: inherited(minmax, target,
		  BM_SV_H_SCROLLBAR | BM_SV_V_SCROLLBAR | BM_SV_BUSYVIEW | BM_SV_CAPTION,
...
\*------------------------------------------------------------------------------*/
void
BmMailViewContainer::RedrawScrollbars()
{
	/*
