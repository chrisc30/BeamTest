...
/*------------------------------------------------------------------------------*\
	()
		-
\*------------------------------------------------------------------------------*/
BmRecvIdentItem::BmRecvIdentItem(ColumnListView* lv, BmListModelItem* _item)
...
\*------------------------------------------------------------------------------*/
void
BmRecvIdentItem::UpdateView(BmUpdFlags flags, bool redraw, uint32 updColBitmap)
{
	BmRecvIdent* ident = ModelItem();
	if (flags & UPD_ALL) {
		const char* cols[] = {ident->Key().String(), ident->RealName().String(),
			ident->MailAddr().String(), ident->MailAliases().String(),
			ident->RecvAccount().String(), ident->MarkedAsBitBucket() ? "*" : "",
			ident->SignatureName().String(), ident->SMTPAccount().String(), NULL};
		SetTextCols(0, cols);
		if (redraw)
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmRecvIdentView::CreateListViewItem(BmListModelItem* item, BMessage*)
{
...
\*------------------------------------------------------------------------------*/
BmListViewItem*
BmRecvIdentView::AddModelItem(BmListModelItem* item)
{
...
\*------------------------------------------------------------------------------*/
void
BmRecvIdentView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			default:
	...
	: inherited("Identities")
{
	MView* view = new VGroup(
		new BetterScrollView(minimax(400, 100, 1E5, 1E5),
			mIdentListView = new BmRecvIdentView(400, 100),
			BM_SV_H_SCROLLBAR | BM_SV_V_SCROLLBAR | BM_SV_CORNER | BM_SV_CAPTION, "99 identities"),
		new HGroup(
			mAddButton = new BetterButton("Add identity", new BMessage(BM_ADD_IDENTITY), this),
			mRemoveButton
			= new BetterButton("Remove identity", new BMessage(BM_REMOVE_IDENTITY), this),
			0),
		new Space(minimax(0, 10, 0, 10)),
		new HGroup(
			new MBorder(M_LABELED_BORDER, 10, (char*)"Identity info",
				new VGroup(mIdentityControl = new BmTextControl("Identity name:", false, 0, 25),
					mRealNameControl = new BmTextControl("Real name:"),
					mMailAddrControl = new BmTextControl("Mail address:"),
					new HGroup(mReplyToControl = new BmTextControl("Reply to:"),
						mSpecialHeadersButton = new BetterButton("Special headers" B_UTF8_ELLIPSIS,
							new BMessage(BM_SET_SPECIAL_HEADERS), this, minimax(-1, -1, -1, -1)),
						0),
					mAliasesControl = new BmTextControl("Aliases:"), new Space(minimax(0, 5, 0, 5)),
					mRecvControl = new BmMenuControl("Receiving account:",
						new BmMenuController("Receiving account:", this,
							new BMessage(BM_RECV_SELECTED),
							&BmGuiRosterBase::RebuildRecvAccountMenu, BM_MC_LABEL_FROM_MARKED)),
					new Space(minimax(0, 5, 0, 5)),
					mSmtpControl = new BmMenuControl("SMTP account:",
						new BmMenuController("SMTP account:", this, new BMessage(BM_SMTP_SELECTED),
							&BmGuiRosterBase::RebuildSmtpAccountMenu, BM_MC_LABEL_FROM_MARKED)),
					new Space(minimax(0, 5, 0, 5)),
					mSignatureControl = new BmMenuControl(
						"Signature:", new BmMenuController("Signature:", this,
			...
			new VGroup(new MBorder(M_LABELED_BORDER, 10, (char*)"Options",
						   new VGroup(mIsBucketControl = new BmCheckControl(
										  "Use as fallback identity for the receiving account",
										  new BMessage(BM_IS_BUCKET_CHANGED), this),
							   0)),
				new Space(), 0),
			0),
		0);
	...
	BmDividable::DivideSame(mIdentityControl, mMailAddrControl, mAliasesControl, mRealNameControl,
		mReplyToControl, mSignatureControl, mSmtpControl, mRecvControl, NULL);
}
...
\*------------------------------------------------------------------------------*/
void
BmPrefsIdentityView::Initialize()
{
	inherited::Initialize();

	TheBubbleHelper->SetHelp(
		mIdentListView, "This list view shows every identity you have defined.");
	TheBubbleHelper->SetHelp(mIdentityControl,
		"Here you can enter a name for this identity.\n"
		"This name is used when you select an identity in Beam.");
	TheBubbleHelper->SetHelp(mMailAddrControl,
	...
	TheBubbleHelper->SetHelp(mAliasesControl,
	...
	TheBubbleHelper->SetHelp(
		mRealNameControl, "Please enter your real name here (e.g. 'Bob Meyer').");
	TheBubbleHelper->SetHelp(mReplyToControl,
		...
		"else.");
	TheBubbleHelper->SetHelp(mSpecialHeadersButton,
	...
	TheBubbleHelper->SetHelp(mIsBucketControl,
...
\*------------------------------------------------------------------------------*/
void
BmPrefsIdentityView::WriteStateInfo()
{
...
\*------------------------------------------------------------------------------*/
bool
BmPrefsIdentityView::SanityCheck()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsIdentityView::UndoChanges()
{
...
\*------------------------------------------------------------------------------*/
void
BmPrefsIdentityView::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			...
			case BM_SET_SPECIAL_HEADERS:
			{
				...
				TextEntryAlert* alert = new TextEntryAlert("Edit special headers",
					...
					specialHeaders.String(), "Cancel", "OK", false, 80, 20, B_WIDTH_FROM_LABEL,
					true, &alertFrame);
			...
			case BM_REMOVE_IDENTITY:
			{
				...
				if (msg->FindInt32("which", &buttonPressed) != B_OK) {
					...
					BAlert* alert = new BAlert("Remove mail identity",
						(BmString("Are you sure about removing the "
								  "identity <")
							<< mCurrIdent->Name() << ">?")
							.String(),
...
\*------------------------------------------------------------------------------*/
void
BmPrefsIdentityView::ShowIdentity(int32 selection)
{
		...
		if (identItem) {
			if (mCurrIdent != identItem->ModelItem()) {
				...
				if (mCurrIdent) {
					...
					mReplyToControl->SetTextSilently(mCurrIdent->ReplyTo().String());
