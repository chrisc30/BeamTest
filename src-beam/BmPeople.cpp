...
\*------------------------------------------------------------------------------*/
void
BmPersonInfo::AddEmails(const BmString& name, const BmStringVect& mails)
{
	Regexx rx;
...
BmPerson::BmPerson(BmPeopleList* model, const node_ref& nref, const BmString& name,
	const BmString& nick, const BmString& email, const BmString& groups, const entry_ref& eref,
	bool foreign)
...
\*------------------------------------------------------------------------------*/
bool
BmPerson::AddEmail(const BmString& em)
{
	Regexx rx;
	if (!rx.exec(em, "^\\s*$")) {
...
\*------------------------------------------------------------------------------*/
bool
BmPerson::CreateNewEmail(const BmString& em)
{
	...
	if (AddEmail(em)) {
		...
		if (count > 1)
			attrName << count;
		node.WriteAttr(attrName.String(), B_STRING_TYPE, 0, em.String(), em.Length() + 1);
		return true;
	}
	return false;
}
...
\*------------------------------------------------------------------------------*/
bool
BmPerson::HasEmail(const BmString& em) const
{
	for (uint32 i = 0; i < mEmails.size(); ++i)
...
\*------------------------------------------------------------------------------*/
BmString
BmPerson::GenerateMailAddr(const BmString& name, const BmString& email, const BmString& nick)
{
	BmString addr;
	if (nick.Length() > 0)
		addr << "(" << nick << ") ";
...
\*------------------------------------------------------------------------------*/
void
BmPerson::AddToNickMap(BmPersonMap& nickMap) const
{
...
\*------------------------------------------------------------------------------*/
void
BmPerson::AddToForeignMap(BmPersonMap& foreignMap) const
{
...
\*------------------------------------------------------------------------------*/
void
BmPerson::AddToNoGroupMap(BmPersonMap& noGroupMap) const
{
...
\*------------------------------------------------------------------------------*/
void
BmPerson::AddToAllPeopleMap(BmPersonMap& allPeopleMap) const
{
...
\*------------------------------------------------------------------------------*/
void
BmPerson::AddToGroupMap(BmGroupMap& groupMap) const
{
...
\*------------------------------------------------------------------------------*/
void
BmPeopleList::AddPeopleToMenu(BMenu* menu, const BMessage& templateMsg, const char* addrField)
{
	if (!menu)
		return;
	...
	menu->GetFont(&font);
	BMenu* subMenu
		= _CreateSubmenuForPersonMap(nickMap, templateMsg, addrField, "Nicknames", &font);
	menu->AddItem(subMenu);
	menu->AddSeparatorItem();
	BmGroupMap::const_iterator group;
	for (group = groupMap.begin(); group != groupMap.end(); ++group) {
		subMenu = _CreateSubmenuForPersonMap(group->second.personMap, templateMsg, addrField,
			BmString("Group ") << group->second.name, &font, true);
		menu->AddItem(subMenu);
	}
	subMenu = _CreateSubmenuForPersonMap(
		noGroupMap, templateMsg, addrField, "(Not in any group)", &font);
	menu->AddItem(subMenu);
	menu->AddSeparatorItem();
	if (foreignMap.size() > 0) {
		subMenu = _CreateSubmenuForPersonMap(
			foreignMap, templateMsg, addrField, "(Not in people folder)", &font);
		menu->AddItem(subMenu);
		menu->AddSeparatorItem();
	}
	subMenu = _CreateSubmenuForPersonMap(allPeopleMap, templateMsg, addrField, "All people", &font);
	menu->AddItem(subMenu);
...
\*------------------------------------------------------------------------------*/
BmRef<BmPerson>
BmPeopleList::FindPersonByNodeRef(const node_ref& nref)
{
	BmAutolockCheckGlobal lock(ModelLocker());
	if (!lock.IsLocked())
		BM_THROW_RUNTIME(ModelNameNC() << ": Unable to get lock");
...
\*------------------------------------------------------------------------------*/
BmRef<BmPerson>
BmPeopleList::FindPersonByEmail(const BmString& email)
{
	if (!email.Length())
		return NULL;
	BmAutolockCheckGlobal lock(ModelLocker());
...
\*------------------------------------------------------------------------------*/
BmRef<BmPerson>
BmPeopleList::FindPersonByName(const BmString& name)
{
	if (!name.Length())
		return NULL;
	BmAutolockCheckGlobal lock(ModelLocker());
...
\*------------------------------------------------------------------------------*/
status_t
BmPeopleList::CreateNewPerson(const BmString& name, const BmString& email, entry_ref* eref)
{
	if (!email.Length())
		return B_BAD_VALUE;
	BmString fileName;
	BmString peopleFolder = ThePrefs->GetString("PeopleFolder", "/boot/home/people");
	...
	if (res == B_OK) {
		peopleFile.WriteAttr("META:name", B_STRING_TYPE, 0, name.String(), name.Length() + 1);
		peopleFile.WriteAttr("META:email", B_STRING_TYPE, 0, email.String(), email.Length() + 1);
...
\*------------------------------------------------------------------------------*/
void
BmPeopleList::AddPerson(const node_ref& nref, const entry_ref& eref)
{
	BNode node;
	BmString name;
	BmString nick;
	...
	if (!FindPersonByNodeRef(nref) && node.SetTo(&eref) == B_OK) {
		...
		BmPerson* newPerson = new BmPerson(this, nref, name, nick, email, groups, eref, foreign);
		for (int c = 1; c < 9; ++c) {
			email.Truncate(0);
...
\*------------------------------------------------------------------------------*/
void
BmPeopleList::RemovePerson(const node_ref& nref)
{
...
\*------------------------------------------------------------------------------*/
void
BmPeopleList::GetEmailsFromPeopleFile(const entry_ref& eref, BmStringVect& emails)
{
	emails.clear();
	...
	BmString addr;
	if (name.Length() && ThePrefs->GetBool("AddPeopleNameToMailAddr", true))
		addr << '"' << name << '"' << " <" << addrSpec << ">";
	else
		addr = addrSpec;
	if (addr.Length())
...
\*------------------------------------------------------------------------------*/
void
BmPeopleList::AddAsKnownAddress(const BmString& addr)
{
...
\*------------------------------------------------------------------------------*/
bool
BmPeopleList::IsAddressKnown(const BmString& addr) const
{
...
\*------------------------------------------------------------------------------*/
void
BmPeopleList::InstantiateItem(BMessage* archive)
{
	...
	BM_LOG3(BM_LogApp, BmString("PeopleList: known address <") << knownAddr << " read");
}
...
\*------------------------------------------------------------------------------*/
void
BmPeopleList::InstantiateItems(BMessage* archive)
{
	_FetchAllPeopleInfo();
...
\*------------------------------------------------------------------------------*/
void
BmPeopleList::ExecuteAction(BMessage* action)
{
	if (action) {
...
\*------------------------------------------------------------------------------*/
status_t
BmPeopleList::Archive(BMessage* archive, bool deep) const
{
	...
	if (deep && ret == B_OK) {
		...
		for (iter = mKnownAddrSet.begin(); iter != mKnownAddrSet.end() && ret == B_OK; ++iter) {
			BMessage msg;
...
\*------------------------------------------------------------------------------*/
void
BmPeopleList::_FetchAllPeopleInfo()
{
	...
	BmAutolockCheckGlobal lock(mModelLocker);
	if (!lock.IsLocked())
		BM_THROW_RUNTIME(ModelNameNC() << ":InitializeItems(): Unable to get lock");
	...
	if ((err = mPeopleQuery.SetTarget(BMessenger(ThePeopleMonitor))) != B_OK)
		BM_THROW_RUNTIME(
			BmString("BmPeopleList::InitializeItems(): could not set query target.\n\nError:")
			<< strerror(err));
...
\*------------------------------------------------------------------------------*/
BMenu*
BmPeopleList::_CreateSubmenuForPersonMap(const BmPersonMap& personMap, const BMessage& templateMsg,
	const char* addrField, BmString label, BFont* font, bool createAllEntry)
{
	BMessage* msg;
...
\*------------------------------------------------------------------------------*/
void
BmPeopleMonitor::MessageReceived(BMessage* msg)
{
	try {
		switch (msg->what) {
			case B_QUERY_UPDATE:
...
\*------------------------------------------------------------------------------*/
void
BmPeopleMonitor::HandleQueryUpdateMsg(BMessage* msg)
{
